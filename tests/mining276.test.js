...
test('player automatic mining has a 1.5 second cadence across batched heartbeats without worker overlap',()=>{
 const s=newMine(0);applyMineAction(s,presence('enter'),0);applyMineAction(s,presence('auto-start',2),0);
 advanceMine(s,1499,()=>0);assert.equal(s.ore,0);advanceMine(s,1500,()=>0);assert.equal(s.ore,1);
 advanceMine(s,4000,()=>0);applyMineAction(s,presence('heartbeat',3),4000);assert.equal(s.ore,2);
...
test('lost close notification expires player mode and calculates remaining time only at worker speed',()=>{
 const s=newMine(0);s.storage=5;applyMineAction(s,presence('enter'),0);applyMineAction(s,presence('auto-start',2),0);
