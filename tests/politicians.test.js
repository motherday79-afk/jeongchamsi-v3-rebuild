...
test('release metadata and browser cache keys identify JCS 0.0.22',async()=>{
  ...
  assert.match(pkg,/"name": "jcs-0-0-22"/);
  assert.match(pkg,/"version": "0\.0\.22"/);
  assert.doesNotMatch(index+app,/v=0\.0\.(?:12|13|14|15|16|17|18|19)/);
  assert.match(index,/pages\.css\?v=0\.0\.22/);
  assert.match(app,/politicians\.js\?v=0\.0\.22/);
  assert.match(gateway,/version:'JCS_0_0_22'/);
});
