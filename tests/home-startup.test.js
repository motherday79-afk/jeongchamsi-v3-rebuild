...
for(const bannerState of ['empty','failed','configured']){
 test('the real index entry exits loading with '+bannerState+' banner response',()=>{
  const result=spawnSync(process.execPath,[fileURLToPath(new URL('helpers/home-startup.mjs',import.meta.url)),new URL(entry.slice(1),root).href,bannerState],{cwd:root,encoding:'utf8',timeout:10000});
