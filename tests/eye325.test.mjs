...
for(const won of [true,false])test(`terminal ${won?'success':'failure'} cannot restart the game`,async t=>{
 ...
 try{cleanup=mountEye(host,{accept(){},audio:{play(){},unlock:async()=>{}},request:async()=>({ok:true,result:{eye:++requests===1?{id:'run',round:3,found:won?1:0,targets:[0],swaps:[],done:false}:{id:'run',round:3,found:won?2:0,done:true,won,last:[{slot:0,treasure:won},{slot:1,treasure:false}]}}})});
