...
test('home cage has two camp links over the same approved banner, for administrators too',()=>{
 const html=renderHomeCage(data,{authenticated:true,user:{role:'admin'}}).trim();
 assert.match(html,/^<div\b[^>]*class="home-cage-preview/);
 assert.equal((html.match(/data-layout-route=/g)||[]).length,2);
 assert.match(html,/href="\/community\/old\?camp=progressive&amp;compose=1"/);
 assert.match(html,/href="\/community\/old\?camp=conservative&amp;compose=1"/);
 assert.equal((html.match(/<svg class="cage-promo-art"/g)||[]).length,1);
...
test('changing the featured cage updates its real title, route and both percentages',()=>{
 ...
 assert.match(second,/href="\/community\/new\?camp=progressive&amp;compose=1"/);
...
test('saving the main cage then going Back renders the newly selected cage',async()=>{
 ...
 assert.match(screen,/href="\/community\/new\?camp=progressive&amp;compose=1"/);
});
