...
test('single-politician refresh remains private until approval and publication',async()=>{
  ...
  assert.equal((await service.getPublicIntelligence(person.id)).news[0].title,'고민정 기존 기사');
  await service.approvePersonRefresh({personId:person.id,reviewedBy:'admin'});
  assert.equal((await service.getPublicIntelligence(person.id)).news[0].title,'고민정 기존 기사');
