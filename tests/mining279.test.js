...
test('admin endpoints remain private before state reads or writes',async()=>{
 let calls=0;
 const service={run:async()=>{calls++;return {ok:true};},admin:async()=>{calls++;return {ok:true};},visit:async()=>{calls++;return 'https://example.com';}};
 for(const path of ['mine/admin'])for(const method of ['GET','POST']){
  ...
  assert.equal(response.status,403);assert.equal(response.data.error,'FORBIDDEN');
 }
