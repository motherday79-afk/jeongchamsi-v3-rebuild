...
assert.deepEqual(POLIMARBLE_HUD_LAYOUT.map(x=>x.id),['player-1','player-2','strategy-cards','today-ranking','dice-box']);
for(const x of [...POLIMARBLE_32_TILE_LAYOUT,...POLIMARBLE_HUD_LAYOUT]){
  for(const k of ['x','y','w','h']) assert.ok(Number.isFinite(x[k])&&x[k]>=0,`${k} must be valid`);
}
