...
test('admin reset invalidates run and preserves wallet; tower clear counts once per day',async()=>{
 ...
 const end=await service.run(admin,{action:'tower-finish',runId:start.result.towerRun.id,requestId:'finish-328'});assert.equal(end.quests.tower,true);assert.equal(end.quests.total,7);
