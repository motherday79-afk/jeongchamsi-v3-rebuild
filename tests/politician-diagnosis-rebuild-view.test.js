...
const context={peers:[{id:'r1',name:'이경쟁',party:'국민의힘',region:'서울',office:'국회의원'},{id:'r2',name:'박경쟁',party:'더불어민주당',region:'서울',office:'국회의원'},{id:'r3',name:'최경쟁',party:'무소속',region:'경기',office:'국회의원'}],ageSex:[{age:'20대',maleShare:49,femaleShare:51,totalCount:1200,totalShare:16},{age:'30대',maleShare:50,femaleShare:50,totalCount:1350,totalShare:18},{age:'40대',maleShare:51,femaleShare:49,totalCount:1500,totalShare:20},{age:'50대',maleShare:49,femaleShare:51,totalCount:1650,totalShare:22},{age:'60대 이상',maleShare:46,femaleShare:54,totalCount:1800,totalShare:24}]};
...
test('administrator detail renders all ten approved diagnosis-specific layouts',async()=>{
  ...
  for(const marker of ['NOW SIGNAL','BRAND INDICATORS','PAST RISK SIGNALS','BRAND TOTAL SIGN','서칭엔진 검색추이','JCS 연령·성별 지지구조 해석','기반 순위','지역 연령별 유권자 현황','코어 결집','유동층 이동','이탈 위험','이슈 확산 속도','매체 집중도','5대 메이저','정치 기반 흐름','정치 행보 구성','미디어 전환','JCS TOTAL'])assert.match(html,new RegExp(marker));
  assert.doesNotMatch(html,/SUPPORT COMPOSITION <span>합계 100%/);
  assert.match(html,/jcs-local-response-matrix/);
  assert.match(html,/jcs-persistence-32/);
...
test('approved final styles preserve local support media and compare visual contracts',async()=>{
  const css=(await readFile(new URL('../css/diagnosis-approved.css',import.meta.url),'utf8'))+(await readFile(new URL('../css/hotfix-31-32-mypage-diagnostics-photo.css',import.meta.url),'utf8'));
  for(const marker of ['.jcs-local-response-matrix','.jcs-support-index-ring.is-core','.jcs-support-index-ring.is-floating','.jcs-support-index-ring.is-exit','.jcs-treemap','.jcs-compare-report-admin .jcs-compare-matrix'])assert.match(css,new RegExp(marker.replaceAll('.','\\.')));
...
test('media panels expose full outlet names instead of ellipsized fragments',async()=>{
  ...
  assert.match(html,/jcs-media-name/);
...
test('approved demographic and support markup uses vertical age columns and three independent shapes',async()=>{
  const html=await adminHtml();
  assert.match(html,/JCS 연령·성별 지지구조 해석/);
  assert.match(html,/jcs-age-chart/);
  assert.match(html,/jcs-age-col/);
  assert.match(html,/jcs-support-item jcs-dx-core-shape/);
  assert.match(html,/jcs-support-item jcs-dx-floating-shape/);
  assert.match(html,/jcs-support-item jcs-dx-exit-shape/);
});

test('persistence and media markup preserves all calendar days and expands the full outlet list',async()=>{
  const html=await adminHtml();
  assert.equal((html.match(/data-persistence-raw-count=/g)||[]).length,30);
  assert.match(html,/30일 전/);
  assert.match(html,/오늘/);
  assert.match(html,/5대 메이저/);
  assert.match(html,/비메이저/);
  assert.match(html,/jcs-treemap/);
  assert.match(html,/class="jcs-media-list"/);
