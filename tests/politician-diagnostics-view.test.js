...
test('administrator detail renders all ten compact intelligence report modules',async()=>{
  ...
  for(const label of ['NOW SIGNAL','BRAND INDICATORS','SEARCH & NEWS SPREAD','PAST RISK SIGNALS','BRAND TOTAL SIGN','JCS 연령·성별 지지구조 해석','기반 순위','공식 선거 기반','코어','유동','이탈','이슈 확산 속도','매체 집중도','출마·당선 이력','정책 진행 단계','JCS TOTAL','실행 처방','실행 우선순위','예상 변화 및 추적 지표'])assert.match(html,new RegExp(label));
