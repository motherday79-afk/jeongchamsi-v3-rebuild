...
export function mountValley(host,{request,accept,audio}){
 ...
 function change(next){if(!active||paused||next===lane||time-(moves.at(-1)?.[0]??-120)<120)return;lane=next;moves.push([Math.round(time),lane]);audio.play('swing');}
