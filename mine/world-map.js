...
export function updateWorldMap(root,raid,quests){
 ...
 root.querySelector('[data-world-progress]').textContent=`${quests?.completed??Number(complete)}/6`;
 root.querySelector('[data-world]').classList.toggle('quest-complete',quests?.completed===6);
}
export function questMarkup(raid,quests){
 ...
 return `<section class="quest-scroll-page"><header class="quest-page-title"><small>오늘의 모험 기록</small><h3 class="quest-heading-art"><span class="quest-accessible">오늘의 퀘스트</span><b>${completed}/6</b></h3></header>
 ${entries.map((q,i)=>{const done=q.count>=q.goal;return `<article class="quest-entry ${done?'quest-entry-done':''}"><div class="quest-entry-heading"><span class="quest-seal" aria-hidden="true">${done?'✓':['Ⅰ','Ⅱ','Ⅲ','Ⅳ','Ⅴ','Ⅵ'][i]}</span><div class="quest-name-wrap"><small>${done?'완료한 퀘스트':'진행할 퀘스트'}</small><h4 class="quest-name-art quest-name-${q.art}"><span class="quest-accessible">${q.name}</span></h4></div><b class="quest-status">${q.count} / ${q.goal}</b></div><p>${q.description}</p><dl><div><dt>오늘 한 일</dt><dd>${q.count}회 완료</dd></div><div><dt>앞으로 할 일</dt><dd>${done?'이 퀘스트를 완료했습니다':`${q.goal-q.count}회 더 진행하세요`}</dd></div></dl><button class="quest-go quest-go-${q.art}" ${q.action} aria-label="${q.button}"><span class="${['forest','eye'].includes(q.art)?'quest-forest-go-label':'quest-accessible'}">${q.button}</span></button></article>`;}).join('')}
