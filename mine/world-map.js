export const worldMapMarkup=()=>`<section class="mine-world" data-world hidden aria-label="월드맵">
 ...
 <button class="world-ruins" data-panel="ruins" aria-label="오래된 폐허 입장"><img src="/assets/mine/ruins-337/title.png" alt="오래된 폐허"><small>왕묘 추리 · ★★★★☆</small></button>
 <button class="world-quest" data-panel="quests" aria-label="오늘의 퀘스트 자세히 보기"><span class="world-quest-label">오늘의 퀘스트</span><strong data-world-progress>0/8</strong></button>
 </section>`;
export function updateWorldMap(root,raid,quests){
 ...
 root.querySelector('[data-world-progress]').textContent=`${quests?.completed??Number(complete)}/8`;
 root.querySelector('[data-world]').classList.toggle('quest-complete',quests?.completed===8);
}
export function questMarkup(raid,quests){
 ...
 const entries=[
  {art:'ruins',name:'오래된 폐허',count:Number(!!quests?.ruins),goal:1,description:'단서를 연결해 진짜 왕묘를 한 번 찾아내세요.',action:'data-panel="ruins"',button:'폐허로 가기'},
 ...
 return `<section class="quest-scroll-page"><header class="quest-page-title"><small>오늘의 모험 기록</small><h3 class="quest-heading-art"><span class="quest-accessible">오늘의 퀘스트</span><b>${completed}/8</b></h3></header>
 ${entries.map((q,i)=>{const done=q.count>=q.goal;return `<article class="quest-entry ${done?'quest-entry-done':''}"><div class="quest-entry-heading"><span class="quest-seal" aria-hidden="true">${done?'✓':['Ⅰ','Ⅱ','Ⅲ','Ⅳ','Ⅴ','Ⅵ','Ⅶ','Ⅷ'][i]}</span><div class="quest-name-wrap"><small>${done?'완료한 퀘스트':'진행할 퀘스트'}</small><h4 class="quest-name-art quest-name-${q.art}"><span class="quest-accessible">${q.name}</span></h4></div><b class="quest-status">${q.count} / ${q.goal}</b></div><p>${q.description}</p><dl><div><dt>오늘 한 일</dt><dd>${q.count}회 완료</dd></div><div><dt>앞으로 할 일</dt><dd>${done?'이 퀘스트를 완료했습니다':`${q.goal-q.count}회 더 진행하세요`}</dd></div></dl><button class="quest-go quest-go-${q.art}" ${q.action} aria-label="${q.button}"><span class="quest-accessible">${q.button}</span></button></article>`;}).join('')}
