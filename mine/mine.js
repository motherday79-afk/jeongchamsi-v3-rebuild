...
function accept(data){
 ...
 if(data.raid)raid=data.raid;if(data.ad)ad=data.ad;if(data.jackpot)jackpot=data.jackpot;if(data.campaign)campaign=data.campaign;if(data.lottery)lottery=data.lottery;
...
function open(title,html){dialog.classList.toggle('raid-screen',panel==='raid');scratchCleanup?.();scratchCleanup=null;q('[data-dialog-title]').textContent=title;q('[data-dialog-body]').innerHTML=html;if(!dialog.open)dialog.showModal();}
...
function renderNavigation(which,scroll=0){
 ...
 if(!which||which==='world'){
  dialog.classList.remove('raid-screen');dialog.close();q('[data-world]').hidden=which!=='world';
