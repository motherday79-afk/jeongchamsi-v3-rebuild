...
export function raidMarkup({raid,gold,locked=false}){
 const amount=Math.floor(gold/10),last=raid.last;
 return `<section class="raid-game"><div class="raid-intro"><small>DAILY EXPEDITION</small><h3>후회없는 약탈</h3><p>세 광산 중 한 곳을 선택하세요.<br>빈틈은 단 하나. 나머지 두 곳에는 강한 광부가 기다립니다.</p></div><div class="raid-summary"><span>오늘 도전 <b>${raid.used} / 3</b></span><span class="${raid.complete?'quest-done':''}">일일퀘스트 <b>${raid.complete?'완료':'1회 도전'}</b></span></div><div class="raid-stakes"><span>성공 · 1장<strong>+${fmt(amount)} G</strong></span><span>실패 · 2장<strong>−${fmt(amount)} G</strong></span></div><div class="raid-cards">${['A','B','C'].map((c,i)=>`<button class="raid-card" data-raid-card="${i}" ${locked||!raid.remaining||gold<10?'disabled':''} aria-label="${c} 광산 선택"><span class="raid-card-letter">${c}</span><span class="raid-seal">⚒</span><span class="raid-card-caption">봉인된 광산</span></button>`).join('')}</div><p class="raid-instruction" role="status">${locked?'선택 결과를 확인하고 있습니다.':!raid.remaining?'오늘의 도전을 모두 마쳤습니다.':gold<10?'10 G부터 도전할 수 있습니다.':'카드를 고르면 즉시 골드가 정산됩니다.'}</p><div data-raid-result>${last?`<p class="raid-last">최근 결과 · ${last.won?'약탈 성공':'약탈 실패'} <b>${last.delta>0?'+':'−'}${fmt(Math.abs(last.delta))} G</b></p>`:''}</div><p class="raid-rule">현재 ${fmt(gold)} G · 성공 확률 1/3<br>매일 오전 0시(한국 시간) 3회 충전 · 참여는 선택<br>승패와 관계없이 1회 참여하면 퀘스트 완료<br>매회 보유 골드의 10% · 1 G 미만 버림</p></section>`;
}
export function animateRaid(host,result,{onNext}){
 let timer,finished=false,started=false;
 ...
 target.innerHTML=`<div class="raid-cinema ${result.won?'raid-victory':'raid-defeat'}"><img class="raid-cinema-art" src="/assets/mine/raid-${result.won?'success':'failure'}-305.webp" alt="${result.won?'금으로 가득한 수레를 끌고 광산을 나오는 광부':'강력한 경비 광부를 만나 금을 떨어뜨리며 철수하는 광부'}"><div class="raid-glints" aria-hidden="true">✦　✧　✦　✧　✦</div><div class="raid-cinema-title"><small>${result.won?'빈틈을 노렸다':'상대가 너무 강했다'}</small><strong>${result.won?'약탈 성공!':'철수하라!'}</strong></div></div><button class="raid-skip" data-raid-skip>결과 바로 보기</button>`;
 game.querySelector('.raid-instruction').textContent='선택한 광산에 진입했습니다…';
 const finish=()=>{
  if(finished)return;finished=true;clearTimeout(timer);dialog?.classList.remove('raid-cinema-dialog');game.classList.remove('raid-revealing');if(!host.contains(game))return;
 ...
 dialog?.scrollTo({top:0,behavior:'instant'});
 const image=target.querySelector('.raid-cinema-art');
 const start=()=>{if(finished||started)return;started=true;game.classList.add('raid-art-ready');timer=setTimeout(finish,matchMedia('(prefers-reduced-motion: reduce)').matches?0:3200);};
 image.onload=start;image.onerror=start;if(image.complete)start();
 return ()=>{finished=true;clearTimeout(timer);image.onload=null;image.onerror=null;dialog?.classList.remove('raid-cinema-dialog');};
}
