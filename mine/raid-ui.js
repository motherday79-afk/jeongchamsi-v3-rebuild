...
export function animateRaid(host,result,{onNext}){
 ...
 game.classList.add('raid-revealing');
 ...
 target.innerHTML=`<div class="raid-cinema ${result.won?'raid-victory':'raid-defeat'}"><div class="raid-cinema-art"></div><div class="raid-glints" aria-hidden="true">✦　✧　✦　✧　✦</div><div class="raid-cinema-title"><small>${result.won?'빈틈을 노렸다':'상대가 너무 강했다'}</small><strong>${result.won?'약탈 성공!':'철수하라!'}</strong></div></div><button class="raid-skip" data-raid-skip>결과 바로 보기</button>`;
 game.querySelector('.raid-instruction').textContent='선택한 광산에 진입했습니다…';
 const finish=()=>{
  clearTimeout(timer);if(!host.contains(game))return;
 ...
 timer=setTimeout(finish,matchMedia('(prefers-reduced-motion: reduce)').matches?0:2600);
