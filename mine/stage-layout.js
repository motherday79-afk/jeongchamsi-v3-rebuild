...
export function stageGeometry(width,height,consoleTop,sceneTop=120){
 ...
 // Additional shift uses the visible body width, excluding sprite padding.
 // Folded phone: half a body; unfolded/tablet: 1.5 bodies; PC: one body.
 const bodyWidth=size*.50,multiple=width<=600?.5:width<=1024?1.5:1;
 const requestedLeft=baseLeft+size*.14+bodyWidth*multiple;
 const left=Math.min(requestedLeft,width-size*.86-12);
