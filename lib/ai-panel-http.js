...
export async function aiPanelRequest(req,{service,user=null,url}={}){
 ...
 }catch(error){const code=String(error?.message||error?.code||'');if(/^AI_PANEL_[A-Z_]+$/.test(code))return failure(statusFor(code),code);if(/^STORAGE_(?:NETWORK|REQUEST|RATE_LIMIT|CAPACITY|BANDWIDTH_LIMIT|AUTH|MISSING)$/.test(code))return failure(503,'AI_PANEL_STORAGE_FAILED');return failure(500,'AI_PANEL_REQUEST_FAILED');}
}
