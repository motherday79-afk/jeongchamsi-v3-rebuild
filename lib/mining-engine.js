export const MINING_VERSION=276;
...
function advanceInterval(s,now,interval,rng){
 ...
 while(swings>0&&s.ore<st.capacity){
  ...
  if(wait>swings){s.cursor+=swings*interval;break;}
...
export function publicMine(s,now){
 ...
 return {gold,ore,pick,worker,storage,character,chosen,tool,cycle,claims,totalGold,cursor,lastStrike,mode:online?(s.presence.running?'player':'ready'):'offline',onlineToken:online?s.presence.token:null,onlineUntil:online?s.presence.until:0,serverNow:now,stats:mineStats(s)};
}
