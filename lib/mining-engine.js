...
export function applyMineAction(s,input,now,rng=Math.random,{allPicks=false}={}){
 ...
 if(a==='buy-pick'){
  if(allPicks)return applyMineAction(s,{...input,action:'tool'},now,rng,{allPicks});
 ...
 if(a==='tool'){
  if(!(allPicks&&PICKS.some(p=>p.id===input.tool))&&input.tool!=='trial'&&!(s.ownedPicks||['rust']).includes(input.tool)&&!(s.paidPicks||[]).includes(input.tool))fail('MINE_TOOL');
...
export function publicMine(s,now){
 ...
 return {gold,ore,pick,worker,storage,character,chosen,tool,cycle,claims,totalGold,cursor,lastStrike,resetVersion:s.resetVersion||0,ownedPicks:s.ownedPicks||['rust'],swings:(s.plannedSwings||[]).map(({at,hits,tool})=>({at,hits,tool})),costumes:s.costumes||{},campaignId:s.campaignId,paidPicks:s.paidPicks||[],mode:online?(s.presence.running?'player':'ready'):'offline',onlineToken:online?s.presence.token:null,onlineUntil:online?s.presence.until:0,serverNow:now,stats:mineStats(s)};
}
