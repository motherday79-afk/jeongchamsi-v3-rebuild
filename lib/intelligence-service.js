...
import {readNowRankSchedule} from './now-rank-schedule.js';
  ...
  async function runCollectionStep(){
    ...
    await concurrentMap(batch.ids,5,async personId=>{
      ...
      try{
        ...
        const collected=await collectRaw(person,context,{fetchImpl:options.fetchImpl,env,now,retryDelays:options.retryDelays,timeoutMs:options.timeoutMs||3500});
        if(options.requireRankingSources&&(!collected.searchAds||!collected.news||!collected.news.coverage?.length||collected.news.coverage.some(item=>item.collected!==true)||(collected.sourceErrors||[]).some(item=>['NAVER_SEARCH_ADS','GOOGLE_NEWS'].includes(item.source))))throw Object.assign(Error('RANKING_SOURCE_UNAVAILABLE'),{code:'RANKING_SOURCE_UNAVAILABLE'});
        const raw=await preserveNewsHistory(person.id,collected);
