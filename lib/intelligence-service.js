...
import { compactIntelligenceDraft, buildCompactHistory } from './intelligence-storage.js';
...
export function createIntelligenceService(options={}){
  ...
  async function runCollectionStep(){
    ...
    await concurrentMap(batch.ids,5,async personId=>{
      ...
      try{
        ...
        const draft=compactIntelligenceDraft(analyze(person,raw,{peers:people,ageSex:context.officialContext?.ageSex||null,source:population?.source||null,gallup:officialContext?.gallup||null},ALGORITHM_VERSION)),validation=validateDraft(draft);
  ...
  async function finalizePublication(job){
    ...
    const history=await repository.saveHistory(job.snapshotId,buildCompactHistory(drafts,rankings));
    return {ok:true,validation,rankings:{overall:rankings.overall},cleanup,history};
  }
