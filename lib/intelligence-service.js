...
const ALGORITHM_VERSION='JCS_INTELLIGENCE_V1';
...
function summaryRow(draft,profiles){
  const person=profiles.get(draft.id)||draft?.raw?.officialProfile||{};
  return {id:draft.id,name:person.name||draft.id,type:person.type||'assembly',party:person.party||'',jurisdiction:person.jurisdiction||'',score:Number(draft?.signal?.index)||0};
}
function buildRankings(drafts,profileMap,snapshot){
  const base=drafts.map(draft=>summaryRow(draft,profileMap)).filter(row=>row.id&&row.name).sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));
  const byId={},categories={};
  for(const type of POLITICIAN_TYPES){categories[type]=base.filter(row=>row.type===type).map((row,index)=>({...row,categoryRank:index+1}));}
  const overall=base.map((row,index)=>{const categoryRank=categories[row.type].findIndex(item=>item.id===row.id)+1,rank=index+1;byId[row.id]={rank,categoryRank,score:row.score,type:row.type};return {...row,rank,categoryRank};});
  return {snapshot,generatedAt:new Date().toISOString(),overall:overall.slice(0,30),categories,byId};
}
...
export function createIntelligenceService(options={}){
  ...
  const repository=createIntelligenceRepository(command,{now:options.now}),now=options.now||Date.now,env=options.env||process.env;
  const collectRaw=options.collectRaw||collectPoliticianRaw,analyze=options.analyze||buildIntelligenceDraft,validateDraft=options.validateDraft||validateIntelligenceDraft,validateAll=options.validateSnapshot||validateSnapshot;
  ...
  async function loadProfiles(){
    const rows=Array.isArray(options.profiles)?options.profiles:(await Promise.all(POLITICIAN_TYPES.map(type=>readPoliticianType(command,type)))).flat();
  ...
  async function runCollectionStep(){
    ...
    await concurrentMap(batch.ids,5,async personId=>{
      ...
      try{
        ...
        const draft=analyze(person,raw,{peers:people,ageSex:context.officialContext?.ageSex||null,source:population?.source||null,gallup:officialContext?.gallup||null},ALGORITHM_VERSION),validation=validateDraft(draft);
    ...
    if(terminal(job.status)){
      ...
      validation=validateAll(drafts,job.ids);await repository.setValidation(job.snapshotId,validation);
    }
  ...
  async function preview(){
    ...
    return {ok:true,snapshotId:draftId,validation:await repository.getValidation(draftId),top30:buildRankings(drafts,profileMap,draftId).overall,completed:drafts.length,total:collection?.total||profiles.length};
  }
  ...
  async function finalizePublication(job){
    ...
    const profiles=await loadProfiles(),profileMap=new Map(profiles.map(person=>[person.id,person])),rows=await repository.getPublishedBatch(job.snapshotId,job.ids),drafts=rows.map(item=>item.value).filter(Boolean),validation=validateAll(drafts,job.ids);
    if(!validation.ok){await repository.setValidation(job.snapshotId,{...validation,publish:true});return {ok:false,validation};}
    const rankings=buildRankings(drafts,profileMap,job.snapshotId);
    await repository.setRankings(job.snapshotId,rankings);
    await repository.setPublicPointer(job.snapshotId);
    return {ok:true,validation,rankings:{overall:rankings.overall}};
  }
  ...
  async function runPublishStep(){
    ...
    for(const {personId,value} of drafts){
      try{if(!value)throw Object.assign(new Error('DRAFT_MISSING'),{code:'DRAFT_MISSING'});await repository.putPublished(batch.job.snapshotId,personId,value);successIds.push(personId);}catch(error){failures.push(safeError(personId,Object.assign(error,{stage:'publish'})));}
    }
  ...
  async function getPublicIntelligence(personId){
    ...
    const [draft,rankings]=await Promise.all([repository.getPublished(pointer,personId),repository.getRankings(pointer)]);if(!draft)return null;
    return {...draft,rank:{overall:rankings?.byId?.[personId]?.rank||null,category:rankings?.byId?.[personId]?.categoryRank||null,temporary:false}};
  }
