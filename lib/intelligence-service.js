...
export function createIntelligenceService(options={}){
  ...
  async function publishPersonRefresh(input={}){const personId=String(input.personId||''),record=await repository.getPersonRefresh(personId),current=await repository.getPublicPointer();if(!record||!['DRAFT','APPROVED'].includes(record.status)||record.validation?.ok!==true)throw new Error('PERSON_REFRESH_NOT_READY');if(record.basePublicSnapshot!==current)throw new Error('PERSON_REFRESH_BASE_CHANGED');const pointer={personId,snapshotId:record.snapshotId,basePublicSnapshot:current,publishedAt:now(),publishedBy:String(input.reviewedBy||input.editorId||'admin')};await repository.setPublicPersonOverride(personId,pointer);const next={...record,status:'PUBLISHED',publishedAt:pointer.publishedAt,validationMode:'AUTOMATIC'};await repository.setPersonRefresh(personId,next);await adminPoliticians.log(pointer.publishedBy,'PERSON_REFRESH_PUBLISH',personId,{snapshotId:record.snapshotId,validationMode:'AUTOMATIC'});return {refresh:next,pointer};}
