...
export function createIntelligenceService(options={}){
  ...
  function hydrateStoredDraft(stored,person,peers){
    ...
    const input=stored.input,officialContext=input.officialContext||null,raw={
      ...
      searchAds:input.searchAds||null,news:input.news||{items:[]},officialContext,sourceErrors:Array.isArray(input.sourceErrors)?input.sourceErrors:[]
    };
  ...
  async function runCollectionStep(){
    ...
    if(terminal(job.status)){
      ...
      const newsIds=[...new Set(drafts.flatMap(draft=>(draft.input?.news?.items||[]).map(item=>item.url||item.title).filter(Boolean)))];
      await repository.updateVersion(job.snapshotId,{status:validation.ok?'draft':'failed',reviewStatus:validation.ok?'pending':'blocked',validationReport:validation,newsIds,eventClusters:[],politicianTypes:[],sourceFailures:job.failures||[],publishableCount:publishIds.length,failedAt:validation.ok?null:now()});
    }
  ...
  async function finalizePublication(job){
    ...
    try{
      ...
      await repository.cleanupObsoleteSnapshots([job.snapshotId]);
      await repository.clearHistory();
    }catch(error){
