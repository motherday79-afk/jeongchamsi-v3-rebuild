...
export function createIntelligenceService(options={}){
  ...
  async function publishPersonRefresh(input={}){const personId=String(input.personId||''),record=await repository.getPersonRefresh(personId),current=await repository.getPublicPointer();if(!record||record.status!=='APPROVED')throw new Error('PERSON_REFRESH_APPROVAL_REQUIRED');if(record.basePublicSnapshot!==current)throw new Error('PERSON_REFRESH_BASE_CHANGED');const pointer={personId,snapshotId:record.snapshotId,basePublicSnapshot:current,publishedAt:now(),publishedBy:String(input.reviewedBy||input.editorId||'admin')};await repository.setPublicPersonOverride(personId,pointer);const next={...record,status:'PUBLISHED',publishedAt:pointer.publishedAt};await repository.setPersonRefresh(personId,next);await adminPoliticians.log(pointer.publishedBy,'PERSON_REFRESH_PUBLISH',personId,{snapshotId:record.snapshotId});return {refresh:next,pointer};}
