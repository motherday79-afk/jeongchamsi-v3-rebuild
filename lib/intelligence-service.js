...
export function createIntelligenceService(options={}){
  ...
  async function finalizePublication(job){
    ...
    const earlier=previous?await repository.getRankings(previous):null;rankings.rising=previous===job.snapshotId&&earlier?.rising?earlier.rising:rankingChanges(rankings,earlier);rankings.keywordArticles=keywordArticles(drafts);
    try{
