...
export function createIntelligenceService(options={}){
  ...
  function hydrateStoredDraft(stored,person,peers){
    ...
    const input=stored.input,officialContext=input.officialContext||null,raw={
      ...
      searchAds:input.searchAds||null,news:{...(input.news||{items:[]}),aggregate:{articleCount:Number(stored.rankingInput?.articleCount)||input.news?.items?.length||0,sourceCount:Number(stored.rankingInput?.sourceCount)||0}},youtube:input.youtube||null,officialContext,sourceErrors:Array.isArray(input.sourceErrors)?input.sourceErrors:[]
    };
  ...
  async function status(){
    const [collection,publication,latestDraft,publicSnapshot,versions,youtube]=await Promise.all([repository.readJob('collect'),repository.readJob('publish'),repository.getLatestDraftId(),repository.getPublicPointer(),repository.listVersions(),youtubeChannels.status()]);
    const validation=latestDraft?await repository.getValidation(latestDraft):null;
    return {release:releaseMetadata(env),sources:{naverSearchAds:naverCredentialStatus(env),youtubeDataApi:youtube.credentials,googleNews:{configured:true,mode:'RSS_WEB'},officialPublicData:{configured:true,mode:'PUBLIC_WEB'}},youtube,collection,publication,latestDraft,publicSnapshot,validation,versions};
  }
  ...
  async function refreshPerson(input={}){
    ...
    const credentials=naverCredentialStatus(env);if(!credentials.configured)throw Object.assign(new Error('NAVER_CREDENTIALS_MISSING'),{code:'NAVER_CREDENTIALS_MISSING'});
    const basePublicSnapshot=await repository.getPublicPointer(),refreshId=`person-${personId}-${Number(now())}`,youtubeRegistry=await youtubeChannels.registryForCollection(),editorial=await adminPoliticians.editorialInputs(personId);
    let officialContext=null;try{officialContext=await loadOfficialContext();}catch{}
    const population=officialContext?.population||officialContext,election=officialContext?.electionsByPerson?.[personId]||officialContext?.officialElection||null,context={snapshotId:refreshId,peers:people,youtubeChannel:youtubeRegistry[personId]||null,newsExclusions:editorial.newsExclusions,officialContext:officialContext?{source:population?.source||null,ageSex:selectAgeSexForPerson(population,person),gallup:officialContext?.gallup||null,officialElection:election}:null};
    const raw=await preserveNewsHistory(person.id,await collectRaw(person,context,{fetchImpl:options.fetchImpl,env,now,retryDelays:options.retryDelays,timeoutMs:options.timeoutMs||3500})),generated=analyze(person,raw,{peers:people,ageSex:context.officialContext?.ageSex||null,source:population?.source||null,gallup:officialContext?.gallup||null,officialElection:election},ALGORITHM_VERSION),draft=await adminPoliticians.applyToDraft(personId,{...generated,algorithmVersion:generated?.algorithmVersion||ALGORITHM_VERSION}),validation=validateDraft(draft);
    if(!validation.ok)throw Object.assign(new Error('DRAFT_VALIDATION_FAILED'),{validation});
    await repository.putDraft(refreshId,personId,compactIntelligenceDraft(draft,{adminOverrides:editorial.pastRisks.length?{diagnoses:[{id:'01',pastRisks:editorial.pastRisks}]}:undefined}));
    const record={personId,snapshotId:refreshId,basePublicSnapshot,status:'DRAFT',createdAt:now(),createdBy:String(input.editorId||'admin'),validation};await repository.setPersonRefresh(personId,record);await adminPoliticians.log(input.editorId||'admin','PERSON_REFRESH',personId,{snapshotId:refreshId});return {refresh:record,draft};
  }
  async function approvePersonRefresh(input={}){const personId=String(input.personId||''),record=await repository.getPersonRefresh(personId);if(!record||record.status!=='DRAFT')throw new Error('PERSON_REFRESH_NOT_READY');const next={...record,status:'APPROVED',approvedAt:now(),approvedBy:String(input.reviewedBy||input.editorId||'admin')};await repository.setPersonRefresh(personId,next);return {refresh:next};}
  async function publishPersonRefresh(input={}){const personId=String(input.personId||''),record=await repository.getPersonRefresh(personId),current=await repository.getPublicPointer();if(!record||!['DRAFT','APPROVED'].includes(record.status)||record.validation?.ok!==true)throw new Error('PERSON_REFRESH_NOT_READY');if(record.basePublicSnapshot!==current)throw new Error('PERSON_REFRESH_BASE_CHANGED');const pointer={personId,snapshotId:record.snapshotId,basePublicSnapshot:current,publishedAt:now(),publishedBy:String(input.reviewedBy||input.editorId||'admin')};await repository.setPublicPersonOverride(personId,pointer);await createMediaSpreadService({command:storageCommand,env,now}).invalidate();const next={...record,status:'PUBLISHED',publishedAt:pointer.publishedAt,validationMode:'AUTOMATIC'};await repository.setPersonRefresh(personId,next);await adminPoliticians.log(pointer.publishedBy,'PERSON_REFRESH_PUBLISH',personId,{snapshotId:record.snapshotId,validationMode:'AUTOMATIC'});return {refresh:next,pointer};}
  ...
  async function runCollectionStep(){
    ...
    await concurrentMap(batch.ids,5,async personId=>{
      ...
      try{
        ...
        const editorial=await adminPoliticians.editorialInputs(person.id),election=officialContext?.electionsByPerson?.[person.id]||officialContext?.officialElection||null,context={snapshotId:batch.job.snapshotId,peers:people,youtubeChannel:youtubeRegistry[person.id]||null,newsExclusions:editorial.newsExclusions,officialContext:officialContext?{source:population?.source||null,ageSex:selectAgeSexForPerson(population,person),gallup:officialContext?.gallup||null,officialElection:election}:null};
  ...
  async function preview(){
    ...
    return {ok:true,snapshotId:draftId,version,validation:await repository.getValidation(draftId),top30:previewRankings.overall,completed:drafts.length,total:collection?.total||profiles.length,reviewTargets:profiles.filter(person=>hydratedMap.has(person.id)).map(person=>({id:person.id,name:person.name})),reviewSample:sample?{personId:sample.id,news:(sample.news||[]).slice(0,10),eventClusters:sample.eventClusters||[],politicianType:sample.politicianType||null,diagnoses:sample.diagnoses||[],prescriptions:sample.prescriptions||[]}:null};
  }
  ...
  async function startPublish(){
    ...
    await repository.clearHistory();
    await repository.compressDrafts(collection.snapshotId,(collection.successIds||[]).slice(0,10)).catch(error=>{error.stage='기존 수집 데이터 압축';throw error;});
    if(publicSnapshot){const previousRanks=await repository.getRankings(publicSnapshot);if(previousRanks)await repository.setRankings(publicSnapshot,previousRanks);}
    const version=await repository.getVersion(collection.snapshotId);
    if(version?.status==='draft')await repository.updateVersion(collection.snapshotId,{status:'approved',reviewStatus:'automatic',reviewedBy:'automatic-validation',approvedAt:now()});
  ...
  async function finalizePublication(job){
    ...
    const earlier=previous?await repository.getRankings(previous):null;rankings.rising=previous===job.snapshotId&&earlier?.rising?earlier.rising:rankingChanges(rankings,earlier);
    let publishStage='순위 저장';
    try{
      await repository.setRankings(job.snapshotId,rankings);
      publishStage='게시 버전 저장';
  ...
  async function runPublishStep(){
    ...
    const publicId=await repository.getPublicPointer();
    if(publicId&&publicId!==batch.job.snapshotId)await repository.compressDrafts(publicId,batch.ids);
  ...
  async function getPublicIntelligence(personId){
    ...
    const override=await repository.getPublicPersonOverride(personId),draftPointer=override?.basePublicSnapshot===pointer?override.snapshotId:pointer;
    const [draft,rankings,profiles]=await Promise.all([repository.getDrafts(draftPointer,[personId]).then(rows=>rows[0]?.value||null),repository.getRankings(pointer),loadProfiles()]);if(!draft)return null;
