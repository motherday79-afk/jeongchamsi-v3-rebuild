...
export function createIntelligenceService(options={}){
  ...
  async function finalizePublication(job){
    ...
    const profiles=await loadProfiles(),profileMap=new Map(profiles.map(person=>[person.id,person])),rows=await repository.getDrafts(job.snapshotId,job.ids),drafts=rows.map(item=>item.value).filter(Boolean),validation=validateAll(drafts,job.ids);
