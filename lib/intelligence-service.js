...
function competitorRow(report,profile,rankRow){
  const media=diagnosisDisplay(report,'07'),risk=diagnosisDisplay(report,'06'),campaign=diagnosisDisplay(report,'08'),agendaRows=Array.isArray(media.agendaPenetration)?media.agendaPenetration:[],agendaTotal=agendaRows.reduce((sum,row)=>sum+(Number(row.articles)||0),0);
  return {
    ...
    election:Array.isArray(campaign.elections)?campaign.elections[0]||null:null
  };
}
