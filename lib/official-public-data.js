...
const populationRow=(age,maleCount,femaleCount)=>{const totalCount=maleCount+femaleCount;return {age,maleCount,femaleCount,totalCount,maleShare:totalCount?Number((maleCount/totalCount*100).toFixed(2)):50,femaleShare:totalCount?Number((femaleCount/totalCount*100).toFixed(2)):50};};
const withTotalShares=rows=>{const total=rows.reduce((sum,row)=>sum+row.totalCount,0);let used=0;return rows.map((row,index)=>{const totalShare=total?(index===rows.length-1?Number((100-used).toFixed(2)):Number((row.totalCount/total*100).toFixed(2))):0;used+=totalShare;return {...row,totalShare};});};
...
export function parseMoisAgeSex(html){
  ...
  for(const match of String(html||'').matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)){
    ...
    regions[cells[1]]=withTotalShares(AGE_LABELS.map((age,index)=>populationRow(age,groups[index].reduce((sum,i)=>sum+male[i],0),groups[index].reduce((sum,i)=>sum+female[i],0))));
  }
...
export function selectAgeSexForPerson(context,person){
  if(!context?.regions)return null;
  const raw=String(person?.jurisdiction||person?.region||'').trim(),compact=value=>String(value||'').replace(/\s+/g,''),provinceKey=Object.keys(REGION_ALIASES).find(name=>raw.includes(name)),province=REGION_ALIASES[provinceKey]||'',tokens=raw.split(/\s+/).filter(Boolean),district=String(tokens.at(-1)||'').replace(/(갑|을|병|정)$/,'');
  const entries=Object.entries(context.regions),districtMatch=district&&district!==provinceKey?entries.filter(([name])=>compact(name).includes(compact(district))&&(!province||compact(name).includes(compact(province))||compact(name).includes(compact(provinceKey)))).sort((a,b)=>b[0].length-a[0].length)[0]:null;
  return districtMatch?.[1]||context.regions[province]||context.regions[provinceKey]||context.regions['전국']||null;
}
