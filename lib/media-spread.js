...
import { mediaLabel } from '../src/ui/intelligence-narratives.js';
...
export function publisherName(value){const label=mediaLabel(value);return ALIASES[normalized(label)]||ALIASES[clean(label).toLowerCase()]||label;}
...
export function buildMediaIndex(drafts=[],profiles=[]){
 ...
 for(const draft of drafts){
  ...
  for(const item of [...(news.items||draft.news||[]),...(news.evidenceItems||[]),...corpus]){
   const title=clean(item.title).slice(0,280),source=publisherName(item.source),date=clean(item.publishedAt||item.date),stamp=Date.parse(date);
   if(!title||!source)continue;publishers.add(source);if(!Number.isFinite(stamp))continue;
   const key=source+'|'+normalized(title),timed=item.timePrecision!=='unknown'&&/(?:T|\s)\d{2}:\d{2}/.test(date)&&/(?:Z|[+-]\d{2}:?\d{2}|GMT|UTC)\s*$/i.test(date),url=/^https?:\/\//i.test(item.url||'')?String(item.url).slice(0,1000):'';
   const identity=clean(item.articleKey)||mediaArticleKey(url),identityKey=identity?source+'|'+identity:'',sameTitle=articles.get(key)||[];
 ...
 return {version:1,people,articles:rows,issues:clusterArticles(rows,people),publishers:[...publishers].sort((a,b)=>a.localeCompare(b,'ko')),coverage};
}
...
export function analyzeMediaIndex(index,{query='',personId='',publisher='',period='latest',now=Date.now()}={}){
 ...
 const targetMatches=row=>row.people.some(id=>targetIds.has(id)),rows=index.articles.filter(inside),targetRows=rows.filter(targetMatches),outlets=new Map((index.publishers||[]).map(name=>[name,{name,count:0,total:0,firstCount:0,followupCount:0}]));
 for(const row of rows){const outlet=outlets.get(row.source);outlet.total++;if(targetMatches(row))outlet.count++;}
 const timelines=[];
 for(const issue of index.issues||[]){
  ...
  if(!first)continue;
  const uncertain=all.some(row=>!row.timed&&row.stamp<=first.stamp),firstSources=uncertain?[]:[...new Set(timed.filter(row=>row.stamp===first.stamp).map(row=>row.source))],current=all.filter(inside);
  if(!current.length)continue;
  // At least two outlets or two independently titled reports; one article alone never implies a scoop.
  const sourceRows=new Map();for(const row of all){if(!sourceRows.has(row.source))sourceRows.set(row.source,[]);sourceRows.get(row.source).push(row);}
  const perSource={};
  ...
  const laterSources=[...new Set(timed.filter(row=>row.stamp>first.stamp&&!firstSources.includes(row.source)).map(row=>row.source))];
  timelines.push({title:issue.title,firstAt:uncertain?'':new Date(first.stamp).toISOString(),firstSources,laterSources,people:[...new Set(all.flatMap(row=>row.people))],perSource,evidence:all.slice(0,8).map(row=>({title:row.title,source:row.source,date:new Date(row.stamp).toISOString(),url:row.url}))});
 }
 ...
 const selectedRows=rows.filter(row=>row.source===chosen),subjectCounts=new Map(),subjectFirst=new Map(),subjectFollowup=new Map();
 ...
 return {ok:true,period:days===30?'cumulative':'latest',days,from:new Date(start).toISOString(),to:new Date(end).toISOString(),matches,target:{kind:person?'person':isParty?'party':'none',name:person?.name|| (isParty?party:''),person:person||null,peopleCount:targetPeople.length,articleCount:targetRows.length,publisherCount:directory.filter(row=>row.count>0).length},publishers:directory,selected,coverage:{people:coverage.length,totalPeople:people.length,legacy:coverage.some(row=>row.legacy),targetLegacy:targetCoverage.some(row=>row.legacy),truncated:coverage.some(row=>row.truncated),latestAt:dates.length?new Date(Math.max(...dates)).toISOString():'',earliestAt:dates.length?new Date(Math.min(...dates)).toISOString():''}};
}
