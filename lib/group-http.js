const statusFor=code=>code==='ACTIVITY_RATE_LIMIT'?429:code==='LOGIN_REQUIRED'?401:code==='GROUP_NOT_FOUND'?404:code==='GROUP_CONFLICT'?409:['GROUP_FORBIDDEN','GROUP_MEMBERS_ONLY','GROUP_EXAMPLE_READ_ONLY','GROUP_ORIGIN_INVALID'].includes(code)?403:code.includes('STORAGE')||code==='GROUP_MEDIA_NOT_CONFIGURED'?503:400;
...
export async function groupRequest(req,{service,user=null,url}={}){
 ...
 try{
  ...
  const raw=String(error?.message||''),known=/^GROUP_[A-Z_]+$/.test(raw)||/^ACTIVITY_[A-Z_]+$/.test(raw)||raw==='INVALID_REQUEST_ID'||raw==='LOGIN_REQUIRED';
  const result=failure(known?statusFor(raw):500,known?raw:'GROUP_REQUEST_FAILED');if(error.retryAfterSeconds)result.data.retryAfterSeconds=error.retryAfterSeconds;return result;
 }
}
