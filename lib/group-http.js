...
export async function groupRequest(req,{service,user=null,url}={}){
 ...
 try{
  if(method==='GET'){
   ...
   const data=params.has('id')?await service.get(params.get('id'),user,{invite:params.get('invite')||''}):await service.list(user,{view:params.get('view')||'browse',category:params.get('category')||'all',q:params.get('q')||'',page:params.get('page')||1});
