...
export function createYouTubeChannelService(options={}){
  ...
  async function status(){
    ...
    return {credentials:youtubeCredentialStatus(env),registered:channels.length,total:profiles.filter(person=>person?.id&&person.isVacant!==true).length,quota:{day:registry.quota.day,used:Number(registry.quota.used)||0,limit:100},job,channels};
  }
