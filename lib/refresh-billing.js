...
export function createRefreshBilling({command,now=Date.now}){
 ...
 async function quote(user,personId){
  ...
  return {ok:true,...settings,personId,balance:Number(wallet.balance||0),lastRefreshedAt:last||null,nextAvailableAt:last?last+settings.cooldownMinutes*60000:null};
 }
 ...
 async function begin(user,input){
  ...
  return mutateParticipation(command,[walletKey(user),REFRESH_POLICY_KEY,K.personRefresh(personId)],([wallet,stored,person])=>{
   const done=paid(wallet,requestId);if(done){if(done.personId!==personId)throw Error('INVALID_REQUEST_ID');return {existing:done.result};}
 ...
 async function fail(user,requestId,error){
  return mutateParticipation(command,[walletKey(user)],([wallet])=>{
   const done=paid(wallet,requestId);if(done)return done.result;
