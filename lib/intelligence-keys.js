...
export const INTELLIGENCE_KEYS=Object.freeze({
  ...
  version:snapshotId=>`${PREFIX}:version:${clean(snapshotId)}`,
  versionIndex:`${PREFIX}:version:index`,
  revisions:snapshotId=>`${PREFIX}:revisions:${clean(snapshotId)}`,
  mediaIndex:snapshotId=>`${PREFIX}:media-index:${clean(snapshotId)}`,
  mediaRevision:`${PREFIX}:media-revision`,
  publicPointer:`${PREFIX}:public:pointer`,
  rankingWeights:`${PREFIX}:settings:ranking-weights`,
  latestDraft:`${PREFIX}:draft:latest`,
  personRefresh:personId=>`${PREFIX}:person-refresh:${clean(personId)}`,
  publicPersonOverride:personId=>`${PREFIX}:public-person:${clean(personId)}`,
});
