...
const signatures={jpg:bytes=>bytes[0]===0xff&&bytes[1]===0xd8&&bytes[2]===0xff,png:bytes=>bytes.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])),webp:bytes=>bytes.subarray(0,4).toString()==='RIFF'&&bytes.subarray(8,12).toString()==='WEBP',gif:bytes=>bytes.length>=13&&['GIF87a','GIF89a'].includes(bytes.subarray(0,6).toString())};
...
export function createHomeBannerService(options={}){
  ...
  async function save(input={},actor=''){
    ...
    let banner;
    try{const desktop=await upload(pc,'pc'),phone=await upload(mobile,'mobile'),wide=tablet?await upload(tablet,'tablet'):null;banner={...desktop,mobileUrl:phone.url,mobilePathname:phone.pathname,tabletUrl:wide?.url||previous?.tabletUrl||'',tabletPathname:wide?.pathname||previous?.tabletPathname||'',placement,designVersion:'upload',targetUrl:pc.targetUrl,alt:pc.alt,updatedAt:new Date(stamp).toISOString(),updatedBy:text(actor,24)};await command(['SET',key,JSON.stringify(banner)]);}
  ...
  const compareKey=TARGET_KEYS.homeBanner+':compare';
  async function getCompare(){const raw=await command(['GET',compareKey]);try{return raw?JSON.parse(raw):null;}catch{return null;}}
  async function saveCompare(ids,actor){
    if(!Array.isArray(ids)||ids.length!==2||ids.some(id=>typeof id!=='string'||!id.trim()||id.length>120)||ids[0]===ids[1])throw new Error('COMPARE_PAIR_INVALID');
    const value={ids:ids.map(id=>id.trim()),updatedAt:new Date(now()).toISOString(),updatedBy:text(actor,80)};
    if(value.ids[0]===value.ids[1])throw new Error('COMPARE_PAIR_INVALID');
    await command(['SET',compareKey,JSON.stringify(value)]);return value;
  }
  return {get,save,getCompare,saveCompare};
}
