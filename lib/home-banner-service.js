...
export function createHomeBannerService(options={}){
  ...
  async function save(input={},actor=''){
    ...
    if(pc.bytes.length+mobile.bytes.length>3_145_728)throw new Error('BANNER_PAIR_TOO_LARGE');
    ...
    let banner;
    try{const desktop=await upload(pc,'pc'),phone=await upload(mobile,'mobile');banner={...desktop,mobileUrl:phone.url,mobilePathname:phone.pathname,placement,targetUrl:pc.targetUrl,alt:pc.alt,updatedAt:new Date(stamp).toISOString(),updatedBy:text(actor,24)};}
    catch(error){await Promise.allSettled(created.map(row=>remove(row.url,token?{token}:{})));throw error;}
    await command(['SET',key,JSON.stringify(banner)]);
    // Only owned old paths are eligible, and only after committing the complete new pair.
    const old=[{url:previous?.url,path:previous?.pathname},{url:previous?.mobileUrl,path:previous?.mobilePathname}].filter(row=>row.url&&row.path?.startsWith('home-banners/')&&!created.some(item=>item.url===row.url));
