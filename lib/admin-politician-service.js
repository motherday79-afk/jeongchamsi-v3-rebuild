import {collectionProfileFor,validateCollectionProfile} from './collection-profile.js';
...
export function createAdminPoliticianService(options={}){
  ...
  async function save(personId,patch,actor,action){const profile=await requireProfile(personId);const record=await mutateParticipation(command,[TARGET_KEYS.adminPoliticianRecords],([value])=>{value.records||={};const record={...(value.records[profile.id]||{}),...patch,personId:profile.id,updatedAt:new Date(Number(now())).toISOString(),updatedBy:text(actor,24)};value.records[profile.id]=record;value.updatedAt=record.updatedAt;return record;});await log(actor,action,profile.id);return record;}
  async function saveCollectionProfile(personId,input,actor){const person=await requireProfile(personId);return {record:await save(personId,{collectionProfile:validateCollectionProfile(input,person)},actor,'COLLECTION_PROFILE_UPDATE')};}
  ...
  async function editorialInputs(personId){const {profile,record}=await get(personId);return {collectionProfile:collectionProfileFor(profile,record),pastRisks:Array.isArray(record.pastRisks)?record.pastRisks:[],newsExclusions:Array.isArray(record.newsExclusions)?record.newsExclusions:[]};}
  ...
  async function list(query=''){
    ...
    return {query:key,items:profiles.map(profile=>({profile,record:{...(value.records[profile.id]||{personId:profile.id,pastRisks:[],newsExclusions:[]}),collectionProfile:collectionProfileFor(profile,value.records[profile.id])},photo:photos[profile.id]||null,youtube:channels[profile.id]||null}))};
  }
  async function audit(){return readJson(command,TARGET_KEYS.adminAudit,emptyAudit());}
  return {list,get,saveCollectionProfile,savePastRisks,saveNewsExclusions,editorialInputs,applyToDraft,completeness,audit,log};
}
