...
function copiedDiagnosisFields(diagnoses){
  const values=diagnoses.map(row=>normalized(row?.headline||row?.display?.nowSignal)).filter(Boolean),counts=new Map();
  for(const value of values)counts.set(value,(counts.get(value)||0)+1);
  return Math.max(0,...counts.values())>=Math.max(3,Math.ceil(diagnoses.length*.6));
}
...
export function validateIntelligenceConsistency(report,options={}){
  ...
  const frameSummary=report?.newsNarrative?.frameSummary||{},hasArticleFrames=['positive','neutral','negative'].some(key=>Number.isFinite(Number(frameSummary[key]))),negativeCount=hasArticleFrames?Number(frameSummary.negative)||0:events.filter(event=>event.direction==='negative'||event.politicalFrame==='부정·위기').length,positiveCount=hasArticleFrames?Number(frameSummary.positive)||0:events.filter(event=>event.direction==='positive'||event.politicalFrame==='긍정·성과').length,neutralCount=hasArticleFrames?Number(frameSummary.neutral)||0:0,negativeDominant=negativeCount>0&&negativeCount>=positiveCount&&negativeCount>=neutralCount;
