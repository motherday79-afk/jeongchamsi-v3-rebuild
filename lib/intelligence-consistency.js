...
export function validateIntelligenceConsistency(report,options={}){
  ...
  const negativeCount=events.filter(event=>event.direction==='negative'||event.politicalFrame==='부정·위기').length,positiveCount=events.filter(event=>event.direction==='positive'||event.politicalFrame==='긍정·성과').length,negativeDominant=negativeCount>0&&negativeCount>=positiveCount;
