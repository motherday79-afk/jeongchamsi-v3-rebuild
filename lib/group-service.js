import { activityKeys, activityRetry, awardActivity, revokeOwnActivities } from './activity-points.js';
...
return 1`;
// Content, index membership and reward documents are compared before any write.
export const GROUP_ACTIVITY_COMMIT_LUA=`-- GROUP_ACTIVITY_COMMIT_V1
local u=tonumber(ARGV[5])
local d=#KEYS-2-u
local start=6+u
if (redis.call('GET',KEYS[1]) or '')~=ARGV[1] then return 0 end
for i=1,d do if (redis.call('GET',KEYS[2+u+i]) or '')~=ARGV[start+i-1] then return 0 end end
local indexType=redis.call('TYPE',KEYS[2]).ok
if indexType~='none' and indexType~='hash' then return redis.error_reply('GROUP_STORAGE_FAILED') end
for i=1,u do local t=redis.call('TYPE',KEYS[2+i]).ok;if t~='none' and t~='set' then return redis.error_reply('GROUP_STORAGE_FAILED') end end
local writes={KEYS[1],ARGV[2]}
for i=1,d do table.insert(writes,KEYS[2+u+i]);table.insert(writes,ARGV[start+d+i-1]) end
redis.call('MSET',unpack(writes))
redis.call('HSET',KEYS[2],ARGV[4],ARGV[3])
for i=1,u do if ARGV[5+i]=='1' then redis.call('SADD',KEYS[2+i],ARGV[4]) else redis.call('SREM',KEYS[2+i],ARGV[4]) end end
return 1`;
const fail=code=>{throw new Error(code);};
const signed=user=>{if(!user?.id||user.status==='suspended')fail('LOGIN_REQUIRED');};
...
export function createGroupService({command,now=Date.now,idFor=randomUUID,examples=true,putImpl=put,getImpl=getBlob,delImpl=del,env=process.env}={}){
 ...
 async function commit(previous,raw,next,rewards=null){
  ...
  const flags=users.map(id=>after.has(id)?'1':'0');
  if(rewards)keys.push(...rewards.keys);
  const result=await command(rewards?['EVAL',GROUP_ACTIVITY_COMMIT_LUA,String(keys.length),...keys,raw||'',serialized,JSON.stringify(summary),next.id,String(users.length),...flags,...rewards.raw,...rewards.docs.map(JSON.stringify)]:['EVAL',GROUP_COMMIT_LUA,String(keys.length),...keys,raw||'',serialized,JSON.stringify(summary),next.id,...flags]);
 ...
 async function list(user,{view='browse',category='all',q='',page=1,visibility='all'}={}){
  ...
  if(view==='mine'){
   ...
   if(view==='browse')rows=rows.filter(row=>row.status==='approved'&&(visibility==='public'?row.visibility==='public':['public','members'].includes(row.visibility)));
  }
 ...
 async function saveOnce(user,payload={}){
  ...
  let rewards=null,activityReward,activityReversal;
  const isCreate=operation==='comment'||operation==='post'&&!input.postId,isDelete=['post-delete','comment-delete'].includes(operation),scope=operation==='comment'?`group:${id}:comment:${input.postId}`:`group:${id}:post`;
  if(isCreate||isDelete){const keys=activityKeys(user.id),raw=await Promise.all(keys.map(key=>command(['GET',key]).then(v=>v||''))),docs=raw.map(v=>{const parsed=v?parse(v):{};if(!object(parsed))fail('GROUP_STORAGE_FAILED');return parsed;});rewards={keys,raw,docs};
   if(isCreate){const old=activityRetry(docs[1],{requestId:input.requestId,scope,body:input.body});if(old){const found=operation==='post'?previous.item.posts.find(p=>p.id===old.contentId&&!p.deleted):previous.item.posts.find(p=>p.id===input.postId&&!p.deleted)?.comments.find(c=>c.id===old.contentId&&!c.deleted);if(!found)fail('ACTIVITY_CONTENT_REMOVED');return {ok:true,item:groupDetail(previous.item,user,{invite}),activityReward:docs[1].events[old.eventId].receipt};}}
  }
  ...
  if(isCreate){const content=operation==='post'?next.posts[0]:next.posts.find(p=>p.id===input.postId).comments.at(-1);activityReward=awardActivity(rewards.docs,{user,kind:operation==='post'?'post':'comment',body:content.body,requestBody:input.body,scope,contentId:content.id,requestId:input.requestId,eligible:next.status==='approved'&&!next.isExample&&(operation!=='post'||content.kind!=='notice'),at:new Date(instant).getTime()});}
  if(isDelete){const post=previous.item.posts.find(p=>p.id===input.postId),targets=operation==='post-delete'?[...(post.authorId===user.id?[{scope:`group:${id}:post`,contentId:post.id}]:[]),...post.comments.filter(c=>c.authorId===user.id).map(c=>({scope:`group:${id}:comment:${post.id}`,contentId:c.id}))]:post.comments.filter(c=>c.id===input.commentId&&c.authorId===user.id).map(c=>({scope:`group:${id}:comment:${post.id}`,contentId:c.id}));if(targets.length)activityReversal=revokeOwnActivities(rewards.docs,{user,targets,at:new Date(instant).getTime()});}
  await commit(previous.item,previous.raw,next,rewards);
  ...
  return {ok:true,item:groupDetail(next,user,{invite}),...(activityReward?{activityReward}:{}),...(activityReversal?{activityReversal}: {})};
 }
 async function save(user,payload={}){for(let attempt=0;attempt<8;attempt++){try{return await saveOnce(user,payload);}catch(error){if(error.message!=='GROUP_CONFLICT'||attempt===7)throw error;}}}
