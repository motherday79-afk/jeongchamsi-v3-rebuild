...
export async function campaignRequest(req,{service,user=null,url}={}){
  ...
  try{
    if(method==='GET'){
      const data=params.has('id')?await service.get(params.get('id'),user,{edit:params.get('edit')==='1'}):await service.list(user,{view:params.get('view')||'current',page:params.get('page')||1});
