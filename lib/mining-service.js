import {ruinsAction} from './mining-ruins.js';
...
export function createMiningService({command,now=()=>Date.now(),rng=()=>randomInt(0,0x100000000)/0x100000000,uuid=randomUUID}={}){
 ...
 async function run(user,input={action:'sync'}){
  ...
  const sponsor=await ad(),roll=['ruins-start','lottery-buy','raid-play','valley-start','eye-start','eye-choose','tower-start'].includes(input.action)?rng():null;
  for(let attempt=0;attempt<12;attempt++){
   ...
   try{
    ...
    }else if(['ruins-start','ruins-clue','ruins-ask','ruins-finish'].includes(input.action)){
     result={...result,...ruinsAction(s,input,instant,Math.floor(roll*4294967296))};
    }else if(input.action==='tower-start'||input.action==='tower-finish'||input.action==='tower-reset'){
