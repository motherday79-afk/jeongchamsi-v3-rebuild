...
export function createMiningService({command,now=()=>Date.now(),rng=()=>randomInt(0,0x100000000)/0x100000000,uuid=randomUUID}={}){
 ...
 async function run(user,input={action:'sync'}){
  ...
  for(let attempt=0;attempt<12;attempt++){
   ...
   migrateCampaign(s,g.current,instant);
   const response=async(result,error)=>({ok:!error,...(error?{error}:{}),state:publicMine(s,instant),result,ad:sponsor,jackpot:await jackpot(),campaign:publicCampaign(g.current),lottery:publicLottery(s,g)});
