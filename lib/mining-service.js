...
export function createMiningService({command,now=()=>Date.now(),rng=()=>randomInt(0,0x100000000)/0x100000000,uuid=randomUUID}={}){
 ...
 async function run(user,input={action:'sync'}){
  ...
  for(let attempt=0;attempt<4;attempt++){
   ...
   if(prior)return {ok:!prior.error,error:prior.error,state:publicMine(s,instant),result:prior.result,ad:sponsor,jackpot:await jackpot()};
   ...
   const cas=failures>0?['EVAL',MINE_CAS,'2',key(user.id),JACKPOT_FAILURES,raw||'',serialized,String(failures)]:['EVAL',MINE_CAS,'1',key(user.id),raw||'',serialized];
   if(raw===serialized||Number(await command(cas))===1){
    return {ok:!error,...(error?{error}:{}),state:publicMine(s,instant),result,ad:sponsor,jackpot:await jackpot()};
   }
  }
  fail('MINE_BUSY');
 }
