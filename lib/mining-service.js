...
export function createMiningService({command,now=()=>Date.now(),rng=()=>randomInt(0,0x100000000)/0x100000000,uuid=randomUUID}={}){
 ...
 async function run(user,input={action:'sync'}){
  ...
  const sponsor=await ad(),roll=input.action==='lottery-buy'?rng():null;
  for(let attempt=0;attempt<12;attempt++){
   ...
   const response=async(result,error)=>({ok:!error,...(error?{error}:{}),state:{...publicMine(s,instant),adminAllPicks:user.role==='admin'},result,ad:sponsor,jackpot:await jackpot(),campaign:publicCampaign(g.current),lottery:publicLottery(s,g)});
  ...
  if(input?.action==='reset-preview'||input?.action==='reset-member'){
   ...
   for(let attempt=0;attempt<12;attempt++){
    ...
    const next={...newMine(instant),resetVersion:version+1,cycle:(old.cycle||1)+1,campaignId:old.campaignId,
     lottery:old.lottery,prizeHistory:old.prizeHistory||[],ledger:old.ledger||[],
