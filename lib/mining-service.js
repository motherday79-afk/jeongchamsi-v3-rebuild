...
export function createMiningService({command,now=()=>Date.now(),rng=()=>randomInt(0,0x100000000)/0x100000000,uuid=randomUUID}={}){
 ...
 async function run(user,input={action:'sync'}){
  ...
  const sponsor=await ad(),roll=['lottery-buy','raid-play','valley-start','eye-start','eye-choose'].includes(input.action)?rng():null;
  for(let attempt=0;attempt<12;attempt++){
   ...
   const response=async(result,error)=>({ok:!error,...(error?{error}:{}),state:{...publicMine(s,instant),adminAllPicks:user.role==='admin'},result,ad:sponsor,jackpot:await jackpot(),campaign:publicCampaign(g.current),lottery:publicLottery(s,g),raid:publicRaid(s,instant),quests:publicQuests(s,instant)});
