...
function setup(run,now,seed){
 ...
 run.targets=ids.slice(0,3-run.found);run.layout=Array.from({length:20},(_,i)=>i);run.swaps=[];
...
export function eyeAction(s,input,now,seed){
 if(input.action==='eye-start'){const r={id:input.requestId,round:1,found:0,rules:334,startedAt:now};setup(r,now,seed);s.eyeRun=r;return {eye:pub(r)};}
 const r=s.eyeRun;if(!r||r.rules!==334||r.id!==input.runId||r.done||r.round!==input.round||now-r.startedAt>1800000)throw Error('MINE_EYE_RUN');
 ...
 r.won=r.found===3;r.done=r.won||r.round===3;
