...
export function compactSearchMetrics(source,referenceAt){
 ...
 return {...(source.keywordMode==='specified'?{keywordMode:'specified',keywords:source.keywords,confirmedVolume:source.confirmedVolume}:{}),volume,collectedAt:clean(source.collectedAt||referenceAt),...(source.keyword?{keyword:clean(source.keyword).slice(0,100)}:{}),...(source.volumePrecision===1||source.provider==='NAVER_SEARCH_ADS'?{volumePrecision:1}:{}),...(source.keywordMatched!==undefined?{keywordMatched:source.keywordMatched===true}:{})};
}
