...
export async function handleLayout(req,{command,user}){
 ...
 try{const raw=await command(['GET',KEY])||'';const d=raw?JSON.parse(raw):{version:0,layout:defaults(),previous:null};if(!Number.isInteger(d.version)||d.version<0)throw Error();d.layout=validate(d.layout);if(d.previous)d.previous=validate(d.previous);
  ...
  let b=req.body;if(typeof b==='string'){if(b.length>24000)return out(413,{ok:false,error:'TOO_LARGE'});try{b=JSON.parse(b);}catch{return out(400,{ok:false,error:'INVALID_JSON'});}}
  if(!b||JSON.stringify(b).length>24000||!['publish','rollback'].includes(b.action)||!Number.isInteger(b.version))return out(400,{ok:false,error:'INVALID_INPUT'});
