...
function newsMetrics(news){
  const items=Array.isArray(news?.items)?news.items:[],sources=unique(items.map(item=>safeText(item.source))),count=Math.max(items.length,Number(news?.aggregate?.articleCount)||0),sourceCount=Math.max(sources.length,Number(news?.aggregate?.sourceCount)||0);
