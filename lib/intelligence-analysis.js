...
export function buildIntelligenceDraft(person,raw,sharedContext={},algorithmVersion='JCS_INTELLIGENCE_V1'){
  ...
  const cohorts=AGE_LABELS.map((age,index)=>{
    ...
    return {age,male:Number(maleWeight.toFixed(2)),female:Number(femaleWeight.toFixed(2)),note:`${party.label}·개인 NOW 신호·뉴스 프레임·연령 및 성별 직접 언급·공식 인구를 결합한 JCS 연령·성별 지지구조 해석`};
  });
