...
function newsMetrics(news){
  ...
  return {items,sources,count,sourceCount,score:clamp(20+count*5+sourceCount*6)};
}
const TOPICS=[
...
export function buildIntelligenceDraft(person,raw,sharedContext={},algorithmVersion='JCS_INTELLIGENCE_V1'){
  ...
  const cohorts=AGE_LABELS.map((age,index)=>{
    ...
    return {age,male:clamp(cohortBase+ageSignal-sexDelta),female:clamp(cohortBase+ageSignal+sexDelta),note:'검색 기기 비중·공식 연령×성별 인구 맥락의 JCS 관심지수'};
  });
