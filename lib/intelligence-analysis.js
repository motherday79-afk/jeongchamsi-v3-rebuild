...
export function buildIntelligenceDraft(person,raw,sharedContext={},algorithmVersion='JCS_INTELLIGENCE_V1'){
  ...
  const ageChannel=[12,8,2,-4,-10],partyAge=[party.young,party.young*.6,0,party.older*.6,party.older];
  ...
  const cohorts=AGE_LABELS.map((age,index)=>{
    ...
    const ageSignal=ageChannel[index]*channelTilt/10+partyAge[index],sexDelta=(Number(official.femaleShare||50)-Number(official.maleShare||50))*.35;
    return {age,male:clamp(base+ageSignal+party.male-sexDelta),female:clamp(base+ageSignal+party.female+sexDelta),note:`검색 기기 비중·${party.label}·공식 연령×성별 인구 맥락의 JCS 지수`};
  });
