...
export function buildIntelligenceDraft(person,raw,sharedContext={},algorithmVersion='JCS_INTELLIGENCE_V1'){
  ...
  const ageBase=[19,19,21,22,19],deviceShape=[4,3,0,-2,-4],partyShape=[party.young,party.young*.62,0,party.older*.62,party.older],channelTilt=(search.mobileShare-.5)*2;
  ...
  const cohorts=AGE_LABELS.map((age,index)=>{
    ...
    const ageWeight=Math.max(3,ageBase[index]+deviceShape[index]*channelTilt+partyShape[index]*1.3+Math.min(6,ageMentions[index]*2));
    const officialMale=Number(official.maleShare||50),officialFemale=Number(official.femaleShare||50),genderNews=Math.max(-4,Math.min(4,(maleMentions-femaleMentions)*1.5));
    const ageDirection=[2,1,0,-1,-2][index],partyGenderGap=party.female-party.male,ageGenderSignal=ageDirection*(channelTilt*1.8+partyGenderGap*.45);
    const maleWeight=ageWeight*Math.max(.3,(officialMale+party.male*1.5+genderNews-ageGenderSignal)/50),femaleWeight=ageWeight*Math.max(.3,(officialFemale+party.female*1.5-genderNews+ageGenderSignal)/50);
    return {age,male:Number(maleWeight.toFixed(2)),female:Number(femaleWeight.toFixed(2)),note:`검색 기기 비중·${party.label}·공식 연령×성별 인구를 결합한 JCS 지지구조 해석`};
  });
