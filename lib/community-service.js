import { activityKeys, activityRetry, awardActivity, revokeOwnActivity, revokeOwnActivities } from './activity-points.js';
import { validateCageTitleLayout, normalizeCageTitle } from '../src/core/cage-title-layout.js';
...
const eligibleCages=data=>items(data).filter(x=>published(x)&&x.cageEnabled&&!x.cageParentId).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')));
...
export function createCommunityService({command,now=()=>new Date().toISOString(),makeId=()=>randomUUID()}={}){
  ...
  function parentCheck(data,post){const root=post.cageParentId?pick(data,post.cageParentId):post;if(post.cageParentId&&(!root?.cageEnabled||root.cageParentId))fail('CAGE_NOT_FOUND',404);if(root?.cageEnabled&&root.cageEndsAt&&Date.parse(now())>=Number(root.cageEndsAt))fail('CAGE_CLOSED');}
  ...
  return {
    async featureCage(id,user,{titleLayout}={}){login(user);if(user.role!=='admin')fail('ADMIN_REQUIRED',403);return mutate([key('community')],([data])=>{const featured=id?pick(data,id):eligibleCages(data)[0];if(id&&(!featured?.cageEnabled||featured.cageParentId))fail('CAGE_NOT_FOUND',404);if(titleLayout!==undefined){
        if(!featured||String(titleLayout?.cageId||'')!==String(featured.id))fail('CAGE_TITLE_CHANGED',409);
        const target=featured;
        if(normalizeCageTitle(target.title)!==normalizeCageTitle(titleLayout.title))fail('CAGE_TITLE_CHANGED',409);
        const layouts=Object.assign(Object.create(null),data.cageTitleLayouts||{});
        if(titleLayout.enabled===true)layouts[String(target.id)]=validateCageTitleLayout(target.title,titleLayout);else if(titleLayout.enabled===false)delete layouts[String(target.id)];else fail('CAGE_TITLE_INPUT_INVALID');
        data.cageTitleLayouts=layouts;
      }data.featuredCageId=id;return {ok:true};});},
    async createPost(domain,input={},user){login(user);validDomain(domain);if(['columns','news'].includes(domain)&&!['admin','partner'].includes(user.role))fail('EDITOR_WRITE_FORBIDDEN',403);
      ...
      return mutate([key(domain),...activityKeys(user.id)],([data,...reward])=>{const scope=`${domain}:post`,old=activityRetry(reward[1],{requestId:input.requestId,scope,body:input.body});if(old){const existing=pick(data,old.contentId);if(!existing)fail('ACTIVITY_CONTENT_REMOVED',409);return {...existing,activityReward:reward[1].events[old.eventId].receipt};}const post=fields(input);if(input.cageParentId){if(domain!=='community')fail('CAGE_NOT_FOUND',404);post.cageParentId=text(input.cageParentId,160);parentCheck(data,post);if(!camps.has(input.camp))fail('CAMP_REQUIRED');post.camp=input.camp;}pin(post,domain,input,user);Object.assign(post,{id:`${domain}-${makeId()}`,ownerId:user.id,author:text(user.nickname||user.id,40),published:true,likes:0,views:0,createdAt:now(),updatedAt:now()});const activityReward=awardActivity(reward,{user,kind:'post',body:post.body,requestBody:input.body,scope,contentId:post.id,requestId:input.requestId,eligible:['community','itsme'].includes(domain)&&!post.cageEnabled,at:Date.parse(now())});data.items=[post,...items(data)];return {...post,activityReward};});
    },
    ...
    async deletePost(domain,id,user){login(user);validDomain(domain);return mutate([key(domain),key('comments'),...activityKeys(user.id)],([data,comments,...reward])=>{const post=pick(data,id);if(!post)fail('POST_NOT_FOUND',404);if(!manage(user,post))fail('POST_EDIT_FORBIDDEN',403);const removed=new Set([String(id),...items(data).filter(x=>String(x.cageParentId||'')===String(id)).map(x=>String(x.id))]);const targets=[...items(data).filter(x=>removed.has(String(x.id))&&String(x.ownerId)===String(user.id)).map(x=>({scope:`${domain}:post`,contentId:x.id})),...items(comments).filter(x=>x.domain===domain&&removed.has(String(x.postId))&&String(x.ownerId)===String(user.id)).map(x=>({scope:`${domain}:comment:${x.postId}`,contentId:x.id}))];const activityReversal=targets.length?revokeOwnActivities(reward,{user,targets,at:Date.parse(now())}):null;data.items=items(data).filter(x=>!removed.has(String(x.id)));comments.items=items(comments).filter(x=>x.domain!==domain||!removed.has(String(x.postId)));return {ok:true,parentId:post.cageParentId||'',...(activityReversal?{activityReversal}:{})};});},
    async addComment(domain,postId,input={},user){login(user);validDomain(domain);if(!text(input.text,1000))fail('INVALID_COMMENT');return mutate([key(domain),key('comments'),...activityKeys(user.id)],([posts,data,...reward])=>{const scope=`${domain}:comment:${postId}`,old=activityRetry(reward[1],{requestId:input.requestId,scope,body:input.text});if(old){const existing=items(data).find(c=>c.id===old.contentId&&!c.deleted);if(!existing)fail('ACTIVITY_CONTENT_REMOVED',409);return {...existing,activityReward:reward[1].events[old.eventId].receipt};}const post=pick(posts,postId);if(!post)fail('POST_NOT_FOUND',404);parentCheck(posts,post);if((post.cageEnabled||post.cageParentId)&&!camps.has(input.camp))fail('CAMP_REQUIRED');let parentId='';if(input.parentId){const parent=items(data).find(x=>String(x.id)===String(input.parentId)&&x.domain===domain&&String(x.postId)===String(postId)&&published(x));if(!parent)fail('COMMENT_PARENT_INVALID');parentId=parent.parentId||parent.id;}const c={id:`comment-${makeId()}`,domain,postId:String(postId),ownerId:user.id,author:text(user.nickname||user.id,40),text:text(input.text,1000),createdAt:now(),published:true,...(parentId?{parentId}:{}),...((post.cageEnabled||post.cageParentId)?{camp:input.camp}:{})};const activityReward=awardActivity(reward,{user,kind:'comment',body:c.text,requestBody:input.text,scope,contentId:c.id,requestId:input.requestId,eligible:!post.isExample&&!post.isDemo&&!post.demo?.enabled,at:Date.parse(now())});data.items=[...items(data),c];return {...c,activityReward};});},
    ...
    async deleteComment(domain,postId,id,user){login(user);validDomain(domain);return mutate([key(domain),key('comments'),...activityKeys(user.id)],([posts,data,...reward])=>{if(!pick(posts,postId))fail('POST_NOT_FOUND',404);const c=commentFor(data,postId,domain,id);if(!manage(user,c))fail('COMMENT_EDIT_FORBIDDEN',403);const activityReversal=String(c.ownerId)===String(user.id)?revokeOwnActivity(reward,{user,scope:`${domain}:comment:${postId}`,contentId:id,at:Date.parse(now())}):null;if(items(data).some(x=>String(x.parentId)===String(id)&&published(x))){c.text='';c.deleted=true;c.likes=0;delete c.likedBy;}else data.items=items(data).filter(x=>String(x.id)!==String(id));return {ok:true,...(activityReversal?{activityReversal}:{})};});},
    async likeComment(domain,postId,id,user){login(user);validDomain(domain);return mutate([key(domain),key('comments')],([posts,data])=>{const post=pick(posts,postId);if(!post)fail('POST_NOT_FOUND',404);parentCheck(posts,post);const c=commentFor(data,postId,domain,id),liked=new Set(c.likedBy||[]),active=!liked.has(user.id);active?liked.add(user.id):liked.delete(user.id);c.likedBy=[...liked];c.likes=liked.size;return {ok:true,active,likes:c.likes};});},
