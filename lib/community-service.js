...
export function createCommunityService({command,now=()=>new Date().toISOString(),makeId=()=>randomUUID()}={}){
  ...
  return {
    async featureCage(id,user){login(user);if(user.role!=='admin')fail('ADMIN_REQUIRED',403);return mutate([key('community')],([data])=>{if(id){const post=pick(data,id);if(!post?.cageEnabled||post.cageParentId)fail('CAGE_NOT_FOUND',404);}data.featuredCageId=id;return {ok:true};});},
