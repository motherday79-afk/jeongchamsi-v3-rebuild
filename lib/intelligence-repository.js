...
export function createIntelligenceRepository(command,options={}){
  ...
  async function createJob(kind,snapshotId,personIds){
    ...
    const job={
      ...
      storageMode:kind==='collect'?'LATEST_ONLY_V3':null,
    };
  ...
  async function prepareCompactCollection(){
    ...
    if(!job||job.status!=='RUNNING'||job.storageMode==='LATEST_ONLY_V3')return job;
    ...
    Object.assign(job,{cursor:0,completed:0,succeeded:0,failed:0,successIds:[],failures:[],activeBatch:null,completedAt:null,updatedAt:now(),storageMode:'LATEST_ONLY_V3',recoveredFromLegacy:true});
