...
export function createIntelligenceRepository(command,options={}){
  ...
  async function setJobError(kind,error){
    ...
    job.lastError=String(error?.code||error?.message||error||'INTELLIGENCE_OPERATION_FAILED').slice(0,160);
  ...
  async function cleanupObsoleteSnapshots(protectedSnapshotIds=[]){
    ...
    await write(INTELLIGENCE_KEYS.versionIndex,nextVersionIds);
  ...
  return {getNewsHistory,putNewsHistory,createJob,readJob,setJobError,claimNextBatch,completeBatch,prepareFailedRetry,prepareDraftPointerPublication,prepareCompactCollection,cleanupObsoleteSnapshots,clearHistory,putDraft,getDrafts,putPublished,getPublished,getPublishedBatch,setPublicPointer,getPublicPointer,setRankings,getRankings,setValidation,getValidation,getLatestDraftId,putVersion,getVersion,listVersions,updateVersion,archiveVersion,appendRevision,getRevisions,getPersonRefresh,setPersonRefresh,getPublicPersonOverride,setPublicPersonOverride};
}
