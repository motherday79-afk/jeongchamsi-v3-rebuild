...
export function createIntelligenceRepository(command,options={}){
  ...
  async function createJob(kind,snapshotId,personIds){
    ...
    const job={
      ...
      storageMode:kind==='collect'?'COMPACT_V2':null,
    };
  ...
  async function prepareCompactCollection(){
    ...
    if(!job||job.status!=='RUNNING'||job.storageMode==='COMPACT_V2')return job;
    ...
    Object.assign(job,{cursor:0,completed:0,succeeded:0,failed:0,successIds:[],failures:[],activeBatch:null,completedAt:null,updatedAt:now(),storageMode:'COMPACT_V2',recoveredFromLegacy:true});
  ...
  async function saveHistory(snapshotId,value,limits={}){
    const maxEntries=Math.max(1,Number(limits.maxEntries)||730),maxBytes=Math.max(1024,Number(limits.maxBytes)||64*1024*1024),serialized=JSON.stringify(value),bytes=Buffer.byteLength(serialized);
    await command(['SET',INTELLIGENCE_KEYS.history(snapshotId),serialized]);
    const current=parseJson(await command(['GET',INTELLIGENCE_KEYS.historyIndex]),{items:[]}),items=(current?.items||[]).filter(item=>item.snapshot!==snapshotId);
    items.push({snapshot:snapshotId,bytes,createdAt:now()});
    let total=items.reduce((sum,item)=>sum+Number(item.bytes||0),0),removed=[];
    while(items.length>maxEntries||total>maxBytes){const oldest=items.shift();if(!oldest)break;total-=Number(oldest.bytes||0);removed.push(INTELLIGENCE_KEYS.history(oldest.snapshot));}
    if(removed.length)await deleteKeys(removed);
    await write(INTELLIGENCE_KEYS.historyIndex,{items,totalBytes:total,maxBytes,maxEntries});
    return {entries:items.length,totalBytes:total,removed:removed.length};
  }

  return {createJob,readJob,claimNextBatch,completeBatch,prepareDraftPointerPublication,prepareCompactCollection,cleanupObsoleteSnapshots,putDraft,getDrafts,putPublished,getPublished,getPublishedBatch,setPublicPointer,getPublicPointer,setRankings,getRankings,setValidation,getValidation,getLatestDraftId,saveHistory};
}
