...
export function createIntelligenceRepository(command,options={}){
  ...
  async function claimNextBatch(kind){
    ...
    const workIds=job.retryMode?job.retryIds||[]:job.ids,start=Math.max(0,Number(job.retryMode?job.retryCursor:job.cursor)||0),ids=workIds.slice(start,start+MAX_POLITICIAN_BATCH);
    if(!ids.length){
