...
export const decodeStored=(raw)=>{const value=typeof raw==='string'?JSON.parse(raw):raw;return value?.codec==='JCS_GZIP_V1'?JSON.parse(gunzipSync(Buffer.from(value.data,'base64'),{maxOutputLength:64*1024*1024}).toString('utf8')):value;};
export const encodeStored=value=>{const plain=JSON.stringify(value);if(Buffer.byteLength(plain)<2048)return plain;const packed=JSON.stringify({codec:'JCS_GZIP_V1',data:gzipSync(plain).toString('base64')});return Buffer.byteLength(packed)<Buffer.byteLength(plain)?packed:plain;};
const parseJson=(raw,fallback=null)=>{if(raw===null||raw===undefined||raw==='')return fallback;try{return decodeStored(raw);}catch{return fallback;}};
...
export function createIntelligenceRepository(command,options={}){
  ...
  async function setJobError(kind,error){
    const job=await readJob(kind);if(!job)return null;
    job.lastError=[error?.stage,error?.code||error?.message||error||'INTELLIGENCE_OPERATION_FAILED',error?.diagnostic].filter(Boolean).join(' · ').slice(0,300);
    job.updatedAt=now();await write(INTELLIGENCE_KEYS.job(kind),job);return clone(job);
  }

  async function createJob(kind,snapshotId,personIds,options={}){
    if(!['collect','publish'].includes(String(kind||'')))throw new Error('JOB_KIND_INVALID');
    const existingRaw=String(await command(['GET',INTELLIGENCE_KEYS.job(kind)])||''),existing=parseJson(existingRaw,null);
    if(existing?.status==='RUNNING'||existing?.publicationPending)return existing;
    const ids=[...new Set((Array.isArray(personIds)?personIds:[]).map(String).filter(Boolean))];
    const timestamp=now();
    const job={
      id:`${kind}-${snapshotId}`,
      kind,
      ...(kind==='publish'?{publicationPending:true}:{}),
      ...
      storageMode:kind==='collect'?'INPUT_ONLY_V5':null,
      ...(kind==='publish'&&options.rankingWeights?{rankingWeights:clone(options.rankingWeights)}:{}),
    };
    if(kind==='publish'){if(Number(await command(['EVAL',COMMUNITY_CAS_LUA,'1',INTELLIGENCE_KEYS.job(kind),existingRaw,JSON.stringify(job)]))!==1)throw Error('PUBLICATION_BUSY');}
    else await write(INTELLIGENCE_KEYS.job(kind),job);
    if(kind==='collect')await command(['SET',INTELLIGENCE_KEYS.latestDraft,String(snapshotId||'')]);
    return clone(job);
  }

  async function claimNextBatch(kind,size=MAX_POLITICIAN_BATCH){
    ...
    const workIds=job.retryMode?job.retryIds||[]:job.ids,start=Math.max(0,Number(job.retryMode?job.retryCursor:job.cursor)||0),ids=workIds.slice(start,start+Math.min(MAX_POLITICIAN_BATCH,Math.max(1,Number(size)||MAX_POLITICIAN_BATCH)));
    if(!ids.length){
  ...
  async function completeBatch(kind,result={}){
    ...
    const failures=(result.failures||[]).filter(item=>activeSet.has(String(item?.personId||''))).map(item=>({
      personId:String(item.personId),name:String(item.name||item.personId).slice(0,60),stage:String(item.stage||'unknown').slice(0,60),code:String(item.code||'SOURCE_ERROR').slice(0,100),details:String(item.details||'').slice(0,240),at:String(item.at||new Date(now()).toISOString()).slice(0,40),attempts:Math.max(1,Number(item.attempts)||1),retryable:item.retryable!==false
    }));
    ...
    if(job.retryMode)job.retryCursor=active.start+active.ids.length;else job.cursor=active.start+active.ids.length;
    job.completed=job.retryMode?Math.max(0,job.total-job.failures.length):Math.min(job.total,job.cursor);
    job.succeeded=job.successIds.length;
    job.failed=job.failures.length;
    job.activeBatch=null;
    job.updatedAt=now();
    const exhausted=job.retryMode?job.retryCursor>=(job.retryIds||[]).length:job.cursor>=job.total;
    if(exhausted){job.status=job.failed?'COMPLETED_WITH_ERRORS':'COMPLETED';job.completedAt=now();job.retryMode=false;}
    await write(INTELLIGENCE_KEYS.job(kind),job);
    return clone(job);
  }

  async function prepareFailedRetry(kind='collect'){
    const job=await readJob(kind);if(!job)throw new Error('JOB_NOT_FOUND');
    if(job.status==='RUNNING')return clone(job);
    const retryIds=[...new Set((job.failures||[]).filter(item=>item.retryable!==false).map(item=>String(item.personId)).filter(Boolean))];
    if(!retryIds.length)throw new Error('NO_RETRYABLE_FAILURES');
    Object.assign(job,{status:'RUNNING',retryMode:true,retryIds,retryCursor:0,activeBatch:null,completedAt:null,updatedAt:now()});
    await write(INTELLIGENCE_KEYS.job(kind),job);return clone(job);
  }
  ...
  async function prepareCompactCollection(){
    ...
    if(!job||job.status!=='RUNNING'||job.storageMode==='INPUT_ONLY_V5')return job;
    ...
    Object.assign(job,{cursor:0,completed:0,succeeded:0,failed:0,successIds:[],failures:[],activeBatch:null,completedAt:null,updatedAt:now(),storageMode:'INPUT_ONLY_V5',recoveredFromLegacy:true});
  ...
  async function cleanupObsoleteSnapshots(protectedSnapshotIds=[]){
    ...
    for(const key of keys){
      if(key===INTELLIGENCE_KEYS.versionIndex||key.startsWith(`${INTELLIGENCE_KEYS.prefix}:draft:person-`))continue;
      ...
      if(!match)match=key.match(new RegExp(`^${INTELLIGENCE_KEYS.prefix}:(?:validation|rankings|version|revisions|media-index):([^:]+)$`));
      if(match&&!protectedIds.has(match[1]))remove.push(key);
    }
    const removed=await deleteKeys(remove),versionIds=parseJson(await command(['GET',INTELLIGENCE_KEYS.versionIndex]),[]),nextVersionIds=(Array.isArray(versionIds)?versionIds:[]).map(String).filter(id=>protectedIds.has(id));
    if(JSON.stringify(versionIds)!==JSON.stringify(nextVersionIds))await write(INTELLIGENCE_KEYS.versionIndex,nextVersionIds);
    return {removed,scanned:keys.length};
  }
  ...
  const getNewsHistory=personId=>read(`jcs:news-observations:${personId}`);
  const putNewsHistory=(personId,value)=>command(['SET',`jcs:news-observations:${personId}`,JSON.stringify(value),'EX',String(45*86400)]);
  const putDraft=async(snapshotId,personId,value)=>{await command(['SET',INTELLIGENCE_KEYS.draft(snapshotId,personId),encodeStored(value)]);return value;};
  async function compressDrafts(snapshotId,ids){if(!snapshotId)return;for(const chunk of chunkKeys(ids,10)){const rows=await getDrafts(snapshotId,chunk);for(const row of rows)if(row.value)await putDraft(snapshotId,row.personId,row.value);}}
  ...
  const setRankings=async(snapshotId,value)=>{await command(['SET',INTELLIGENCE_KEYS.rankings(snapshotId),encodeStored(value)]);return value;};
  const getRankings=snapshotId=>read(INTELLIGENCE_KEYS.rankings(snapshotId));
  const getRankingWeights=()=>read(INTELLIGENCE_KEYS.rankingWeights);
  const setRankingWeights=value=>write(INTELLIGENCE_KEYS.rankingWeights,value);
  ...
  const getPersonRefresh=personId=>read(INTELLIGENCE_KEYS.personRefresh(personId));
  const setPersonRefresh=(personId,value)=>write(INTELLIGENCE_KEYS.personRefresh(personId),value);
  const getPublicPersonOverride=personId=>read(INTELLIGENCE_KEYS.publicPersonOverride(personId));
  const setPublicPersonOverride=(personId,value)=>write(INTELLIGENCE_KEYS.publicPersonOverride(personId),value);
  return {getRankingWeights,setRankingWeights,compressDrafts,getNewsHistory,putNewsHistory,createJob,readJob,setJobError,claimNextBatch,completeBatch,prepareFailedRetry,prepareDraftPointerPublication,prepareCompactCollection,cleanupObsoleteSnapshots,clearHistory,putDraft,getDrafts,putPublished,getPublished,getPublishedBatch,setPublicPointer,getPublicPointer,setRankings,getRankings,setValidation,getValidation,getLatestDraftId,putVersion,getVersion,listVersions,updateVersion,archiveVersion,appendRevision,getRevisions,getPersonRefresh,setPersonRefresh,getPublicPersonOverride,setPublicPersonOverride};
}
