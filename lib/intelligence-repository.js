...
import { compactIntelligenceDraft } from './intelligence-storage.js';
...
export function createIntelligenceRepository(command,options={}){
  ...
  async function createJob(kind,snapshotId,personIds){
    ...
    const job={
      ...
      storageMode:kind==='collect'?'INPUT_ONLY_V4':null,
    };
  ...
  async function recoverCollectionFailures(job,failures=[]){
    if(job?.kind!=='collect'||!failures.length)return {recoveredIds:[],recoveries:[],failures};
    const publicSnapshot=await getPublicPointer();
    if(!publicSnapshot||publicSnapshot===job.snapshotId)return {recoveredIds:[],recoveries:[],failures};
    const recoveredIds=[],recoveries=[],remaining=[];
    for(const failure of failures){
      const personId=String(failure.personId||'');
      let previous=await read(INTELLIGENCE_KEYS.draft(publicSnapshot,personId));
      if(!previous)previous=await read(INTELLIGENCE_KEYS.published(publicSnapshot,personId));
      if(!previous){remaining.push(failure);continue;}
      const recoveryError={source:'JCS_RECOVERY',code:'PREVIOUS_PUBLISHED_FALLBACK',attempts:Math.max(1,Number(failure.attempts)||1)};
      let recovered=null;
      if(previous?.storageMode==='INPUT_ONLY_V4'&&previous?.input&&previous?.rankingInput){
        recovered=clone(previous);
        recovered.snapshot=job.snapshotId;
        recovered.input={...recovered.input,sourceErrors:[...(Array.isArray(recovered.input?.sourceErrors)?recovered.input.sourceErrors:[]),recoveryError].slice(-4)};
      }else if(previous?.raw){
        recovered=compactIntelligenceDraft({...previous,snapshot:job.snapshotId,raw:{...previous.raw,sourceErrors:[...(Array.isArray(previous.raw?.sourceErrors)?previous.raw.sourceErrors:[]),recoveryError]}});
      }
      if(!recovered?.id||!recovered?.input||!recovered?.rankingInput){remaining.push(failure);continue;}
      await write(INTELLIGENCE_KEYS.draft(job.snapshotId,personId),recovered);
      recoveredIds.push(personId);
      recoveries.push({personId,fromSnapshot:publicSnapshot,stage:failure.stage,code:failure.code,recoveredAt:now()});
    }
    return {recoveredIds,recoveries,failures:remaining};
  }
  ...
  async function completeBatch(kind,result={}){
    ...
    let failures=(result.failures||[]).filter(item=>activeSet.has(String(item?.personId||''))).map(item=>({
    ...
    const recovery=await recoverCollectionFailures(job,failures);
    failures=recovery.failures;
    successIds.push(...recovery.recoveredIds.filter(id=>!successIds.includes(id)));
    if(recovery.recoveries.length){
      const prior=new Map((job.recoveries||[]).map(item=>[item.personId,item]));
      for(const item of recovery.recoveries)prior.set(item.personId,item);
      job.recoveries=[...prior.values()];
      job.recovered=job.recoveries.length;
    }
  ...
  async function prepareCompactCollection(){
    ...
    if(job.storageMode==='INPUT_ONLY_V4')return clone(job);
    if(job.storageMode==='VERSIONED_V3'){
      const firstCompletedId=(job.successIds||[])[0]||job.ids?.[0]||'';
      const sample=firstCompletedId?await read(INTELLIGENCE_KEYS.draft(job.snapshotId,firstCompletedId)):null;
      if((Number(job.cursor)||0)===0||sample?.storageMode==='INPUT_ONLY_V4'){
        job.storageMode='INPUT_ONLY_V4';job.updatedAt=now();job.normalizedStorageMode=true;
    ...
    Object.assign(job,{cursor:0,completed:0,succeeded:0,failed:0,successIds:[],failures:[],activeBatch:null,completedAt:null,updatedAt:now(),storageMode:'INPUT_ONLY_V4',recoveredFromLegacy:true});
    await write(INTELLIGENCE_KEYS.job('collect'),job);
    return clone(job);
  }
