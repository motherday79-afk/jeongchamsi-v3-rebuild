...
function uploadWindows(videos,nowValue){
  const current=Number(nowValue),within=days=>videos.filter(item=>{const stamp=Date.parse(item.publishedAt);const age=current-stamp;return Number.isFinite(stamp)&&age>=0&&age<=days*DAY;}).length;
  ...
  return {h24:within(1),d7:within(7),d30:within(30),daily};
}
