...
export function createAiPanelService({command,now=Date.now,idFor=randomUUID}={}){
 ...
 async function save(user,payload={}){
  ...
  if(operation==='create'){
   ...
   if(input.sourceRunId){source=(await stored(input.sourceRunId)).item;if(source.week!==input.week||typeof input.rerunReason!=='string'||!input.rerunReason.trim())fail('RERUN_REQUIRED');}
   if(state&&!source)fail('RERUN_REQUIRED');if(!state&&source)fail('STORAGE_FAILED');
   const item=createRun(input,{...ctx,id:idFor(),panel,runNumber:(state?.runNumber||0)+1});await commit('run','',item,{raw,next:{runNumber:item.runNumber,latestId:item.id}});return {ok:true,item};
  }
