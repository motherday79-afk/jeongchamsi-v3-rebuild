...
export async function collectOfficialPolls({fetch=globalThis.fetch,now=Date.now,limit=4}={}){
 ...
 const results=await Promise.all(SOURCES.map(async provider=>{
  let items=[],errors=0;try{const index=await fetchOfficialHtml(provider.url,{fetch,signal:deadline}),urls=provider.discover(index).slice(0,Math.max(1,Math.min(4,Number(limit)||4)));if(!urls.length){console.warn('HUMAN_POLL_INDEX',JSON.stringify({provider:provider.id,length:index.length,title:htmlText(index.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||'').slice(0,100),anchors:(index.match(/<a\b/gi)||[]).length,notice:htmlText(index).slice(0,240)}));bad();}
