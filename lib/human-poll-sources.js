...
export function parseRealmeterFeed(xml){
 ...
 for(const match of xml.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)){
  ...
  if(!/주간\s*동향/.test(title)||!/대통령|국정수행/.test(title))continue;
  const raw=entities(item.match(/<link>([^<]+)<\/link>/i)?.[1]||'').trim();let url;
  try{url=allowed(raw);if(url.hostname!=='www.realmeter.net')continue;}catch{continue;}
  const stamp=Date.parse(item.match(/<pubDate>([^<]+)<\/pubDate>/i)?.[1]||'');if(!Number.isFinite(stamp))bad();
  const published=new Date(stamp+9*60*60*1000).toISOString().slice(0,10);
  const content=item.match(/<content:encoded>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/content:encoded>/i)?.[1];if(!content)bad();
  const parsed=parseRealmeter(`<time datetime="${published}T00:00:00+09:00"></time>${content}`,url.href);
  parsed.provenance.contentHash=hash(content);records.set(parsed.id,parsed);
 }
...
export async function collectOfficialPolls({fetch=globalThis.fetch,now=Date.now,limit=4}={}){
 ...
 const results=await Promise.all(SOURCES.map(async provider=>{
  ...
  if(provider.id==='realmeter')try{items=parseRealmeterFeed(await fetchOfficialHtml('http://www.realmeter.net/feed/',{fetch,signal:deadline})).slice(0,count).map(p=>{if(p.publishedDate>instant.slice(0,10))bad();return {...p,fetchedAt:instant};});}catch(error){diagnostic(provider.id,'feed',error);}
