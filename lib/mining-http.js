...
export async function miningRequest(req,{service,user,url}){
 try{
  ...
  if(user.status&&user.status!=='active')return {status:403,data:{ok:false,error:'MINE_FORBIDDEN',message:'권한이 없습니다. 관리자에게 권한을 요청해 주세요.'}};
  const image=url.pathname.endsWith('/image')||url.searchParams.get('path')==='mine/image'||req.query?.path==='mine/image';
  ...
  if((admin||image)&&user.role!=='admin')return {status:403,data:{ok:false,error:'FORBIDDEN'}};
  if(!['GET','POST'].includes(req.method)||visit&&req.method!=='GET'||image&&req.method!=='POST')return {status:405,data:{ok:false,error:'METHOD_NOT_ALLOWED'}};
  if(req.method==='POST'){
  ...
  let body=req.body||{};if(typeof body==='string'){if(body.length>(image?1500000:6000))throw new Error('MINE_INPUT');try{body=JSON.parse(body);}catch{throw new Error('MINE_INPUT');}}
  if(!body||Array.isArray(body)||typeof body!=='object')throw new Error('MINE_INPUT');
  if(image)return {status:201,data:await service.upload(body)};
