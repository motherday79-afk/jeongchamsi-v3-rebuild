...
export function createCampaignService({command,now=Date.now,putImpl=put,env=process.env,examples=true}={}){
  ...
  async function get(id,user,{edit=false}={}){
    ...
    const record=parse(await command(['GET',CAMPAIGN_KEYS.item(id)]))||(examples?campaignExample(id):null);
  ...
  async function list(user,{view='current',page=1,category='all'}={}){
    ...
    let rows=examples?[...CAMPAIGN_EXAMPLES.map(row=>overrides.get(row.id)||row),...stored.filter(row=>!campaignExample(row.id))]:stored;
  ...
  async function save(user,{id='',version=0,operation='save',input={}}={}){
    ...
    const previous=parse(existing)||base,key=id||randomUUID(),instant=now();
