...
if ARGV[2]=='publish' and (doc.number==nil or doc.number==cjson.null) then
...
export function createCampaignService({command,now=Date.now,putImpl=put,env=process.env}={}){
  ...
  async function get(id,user,{edit=false}={}){
    ...
    const record=parse(await command(['GET',CAMPAIGN_KEYS.item(id)]));
  ...
  async function list(user,{view='current',page=1}={}){
    if(!['current','archive','manage'].includes(view))view='current';if(view==='manage')admin(user);
    const instant=now(),rows=(await command(['HVALS',CAMPAIGN_KEYS.index])||[]).map(parse).filter(Boolean);
    ...
    const all=view==='manage'?rows.sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt))).map(row=>({...row.draft,id:row.id,number:row.number,version:row.version,state:campaignState(row,instant),updatedAt:row.updatedAt})):view==='archive'?archive:current.filter(row=>row.id!==selected?.id);
  ...
  async function save(user,{id='',version=0,operation='save',input={}}={}){
    ...
    const existing=id?await command(['GET',CAMPAIGN_KEYS.item(id)]):null;
    if(id&&!existing)fail('CAMPAIGN_NOT_FOUND');
    const previous=parse(existing),key=id||randomUUID(),instant=now();
    const record=prepareCampaignMutation(previous,input,{id:key,operation,version,actor:user.id,now:instant});
