...
export function createFavoriteService({command,getPerson,getPost,listDomain}){
  ...
  return {
    ...
    async dashboard(user){
      if(!user?.id)return {ok:false,error:'LOGIN_REQUIRED'};
      const activity=await readActivity(command,user.id),keys=[...new Set(Array.isArray(activity.favorites)?activity.favorites:[])],targets=keys.map(parseFavoriteTarget).filter(Boolean);
      ...
      const favoritePosts=(await Promise.all(targets.filter(item=>item.kind==='post').map(async item=>{const post=await getPost?.(item.domain,item.id);return post&&post.published!==false?{...post,domain:item.domain,route:postRoute(item.domain)}:null;}))).filter(Boolean);
      const domainRows=await Promise.all(POST_DOMAINS.map(async domain=>[domain,await listDomain?.(domain)||[]]));
      const authoredPosts=domainRows.flatMap(([domain,rows])=>(Array.isArray(rows)?rows:[]).filter(item=>item?.published!==false&&String(item?.ownerId||'')===String(user.id)).map(item=>({...item,domain,route:postRoute(domain)}))).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')));
