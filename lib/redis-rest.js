...
export function createNativeRedisCommand(url){
  ...
  return async function command(args){
    try{
      ...
      return await nativeClientState.client.sendCommand((Array.isArray(args)?args:[]).map(value=>String(value)));
    }catch(cause){
      if(nativeClientState?.client && !nativeClientState.client.isOpen) nativeClientState=null;
