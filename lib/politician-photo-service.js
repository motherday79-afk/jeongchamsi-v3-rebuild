...
export function createPoliticianPhotoService(options={}){
  ...
  const command=options.command,putImpl=options.putImpl||put,now=options.now||Date.now;
  ...
  async function save(input={},actor=''){
    ...
    const blob=await putImpl(path,valid.bytes,{access:'public',contentType:valid.contentType,addRandomSuffix:true});
