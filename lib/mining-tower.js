...
export function towerAction(s,input,now,seed,admin=false){
 if(input.action==='tower-start'){
  ...
  const floor=publicTower(s).floor,target=Math.ceil(10*Math.pow(1.015,Math.min(floor-1,2000)));let v=seed>>>0,total=0;const events=[];
 ...
 r.done=true;if(r.won)s.towerFloor=r.floor+1;return {towerRun:{...r}};
}
