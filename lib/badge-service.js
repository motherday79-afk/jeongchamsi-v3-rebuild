...
export function createBadgeService(command,{now=()=>new Date()}={}){
  ...
  async function domains(){
    if(!domainSnapshotPromise)domainSnapshotPromise=Promise.all(['community','itsme','columns','news','comments'].map(name=>readDomainWithViews(command,name,emptyDomain()))).then(([community,itsme,columns,news,comments])=>({community,itsme,columns,news,comments}));
