...
export function issueSessionToken(userId,secret,now=Date.now(),sessionVersion=0){
  if(!secret || String(secret).length<16) throw new Error('SESSION_SECRET_MISSING');
  const payload=b64(JSON.stringify({userId:String(userId||'').slice(0,24),iat:now,sessionVersion:Number(sessionVersion)||0}));
...
export function readSessionToken(token,secret,now=Date.now(),maxAgeMs=1000*60*60*24*30){
  try{
    ...
    return {userId:String(data.userId),sessionVersion:Number(data.sessionVersion)||0};
  }catch{return null;}
}
