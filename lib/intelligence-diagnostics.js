...
function campaignFallback(ctx,diagnosisScores,localTopics,aggregateSourceCount,totalSearch,elections){
  const currentIndex=clamp(diagnosisScores['08']),regionalStatus=currentIndex>=62?'우세':currentIndex<42?'취약':'경합',career=elections[0]?.result||clean(ctx.person?.terms)||clean(ctx.person?.office)||'현재 공식 역할';
  return {mode:'jcs-current',elections,currentSignals:[{label:'출마·당선 이력',value:career,basis:'공식 프로필'},{label:'현재 캠페인 경쟁력',value:currentIndex,unit:'JCS',basis:'검색·뉴스·정치 구조'},{label:'언론 확산 기반',value:aggregateSourceCount,unit:'개 매체',basis:'수집 뉴스'},{label:'검색 관심 기반',value:totalSearch,unit:'회',basis:'네이버 검색량'}],trajectory:[{label:'공식 경력',value:contextFactors(ctx,ctx.score).career},{label:'지역 기반',value:clamp(diagnosisScores['03'])},{label:'지지 기반',value:clamp(diagnosisScores['04'])},{label:'현재 경쟁력',value:currentIndex}],regionalStructure:(localTopics.length?localTopics.map(row=>({name:row.label,status:row.share>=45?'우세':row.share<20?'취약':'경합',value:row.share,basis:'지역 기사 점유'})):[{name:ctx.region,status:regionalStatus,value:currentIndex,basis:'JCS 지역·인구·검색·뉴스 결합'}]).slice(0,6)};
}
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const policyNews=news.filter(isPolicyEvidence),stages=['발표','제안','검토','통과','시행','완료'],policyBasis=policyNews.length?policyNews:news.slice(0,3),policyMode=policyNews.length?'policy':'issue',policyRows=policyBasis.slice(0,5).map(row=>{const stage=policyMode==='policy'?policyStage(row.title):'관측',stageIndex=policyMode==='policy'?stages.indexOf(stage):0,issueName=`${clean(row.agendaTag)||'정치 활동'} · ${clean(row.frame)||'중립·정보'}`;return {name:policyMode==='policy'?compactTitle(row.title):issueName,stage,stageIndex,articles:1,outlets:1,frame:row.frame||'중립·정보',pc:null,mobile:null,share:Math.round(100/Math.max(1,policyBasis.length)),isPolicy:policyMode==='policy',evidenceTitle:policyMode==='policy'?compactTitle(row.title):issueName,evidenceUrl:clean(row.url),evidenceSource:clean(row.source),specificity:{beneficiary:/청년|노인|소상공인|주민|가구|기업/.test(row.title),budget:/예산|억|조원/.test(row.title),deadline:/년|월|일까지/.test(row.title),owner:/정부|국회|위원회|부처|서울시|도|시|군|구/.test(row.title)}};});
  const frequencyBands={repeat:sources.filter(row=>row.count>=3).length,double:sources.filter(row=>row.count===2).length,single:sources.filter(row=>row.count===1).length},officialElectionRows=elections.filter(row=>!row.profileOnly&&row.voteRate!==null),campaign=officialElectionRows.length?{kind:'campaign',mode:'official',elections,status:'공식 선거 기록 연결'}:{kind:'campaign',...campaignFallback(ctx,diagnosisScores,localTopics,aggregateSourceCount,totalSearch,elections)};
  const contracts={
    ...
    '04':support,
    '05':{kind:'competitor',people,metrics:['NOW 순위','PC·모바일 검색량','대표 뉴스량·매체 다양성','뉴스 프레임','주요 의제 점유율','최근 선거·현재 경쟁 신호']},
    ...
    '07':{kind:'media',articleCount:aggregateNewsCount,sourceCount:sources.length,sourceSpread:sources,frequencyBands,topSources:sources.slice(0,5),remainingSources:sources.slice(5),concentration:sources,search:{pc:Number(search.pc)||0,mobile:Number(search.mobile)||0,mobileShare:totalSearch?Math.round(Number(search.mobile||0)/totalSearch*100):0},ownership:{led,external},agendaPenetration:topics.map(row=>({label:row.label,articles:row.count,outlets:row.sourceCount}))},
    '08':campaign,
    '09':{kind:'policy',mode:policyMode,title:policyMode==='policy'?'정책·공약 반응':'주요 의제 반응',policies:policyRows,status:policyRows.length?(policyMode==='policy'?'정책·공약 보도 연결':'정책 근거 미확인 · 주요 의제 반응으로 표시'):'수집 뉴스 없음'},
