...
function demographicDisplay(cohorts){
  const cells=normalizedHundred(list(cohorts).flatMap(row=>[{age:row.age,gender:'male',value:row.male},{age:row.age,gender:'female',value:row.female}]));
  const rows=list(cohorts).map(row=>({age:row.age,male:cells.find(cell=>cell.age===row.age&&cell.gender==='male')?.value||0,female:cells.find(cell=>cell.age===row.age&&cell.gender==='female')?.value||0}));
  const rank=gender=>rows.map(row=>({age:row.age,value:row[gender]})).sort((a,b)=>b.value-a.value||a.age.localeCompare(b.age,'ko'));
  return {kind:'demographic',cohorts:rows,maleRank:rank('male'),femaleRank:rank('female'),total:100};
}
function dateBuckets(items){
  const dated=list(items).map(row=>Date.parse(row.date||row.publishedAt||'')).filter(Number.isFinite),reference=Math.max(0,...dated);
  const within=days=>list(items).filter(row=>{const stamp=Date.parse(row.date||row.publishedAt||'');return reference&&Number.isFinite(stamp)&&reference-stamp<=days*86400000;}).length;
...
function verifiedElections(person,input){
  ...
  return rows.filter(row=>Number.isFinite(Number(row.voteRate??row.voteShare))).map(row=>({
    year:clean(row.year||row.date),election:clean(row.election||row.name||person.electionLabel),result:clean(row.result),voteRate:Number(row.voteRate??row.voteShare),opponent:clean(row.opponent),opponentRate:Number.isFinite(Number(row.opponentRate))?Number(row.opponentRate):null,margin:Number.isFinite(Number(row.margin))?Number(row.margin):null,regions:list(row.regions).slice(0,30)
  }));
}
...
function supportDisplay(ctx){
  ...
  const composition=normalizedHundred([{key:'core',label:'CORE',value:career+Math.max(0,50-ctx.negativeShare)},{key:'floating',label:'FLOATING',value:Math.max(10,100-(spread.length?Math.max(...spread)-Math.min(...spread):0))},{key:'exit',label:'EXIT',value:Math.max(5,ctx.negativeShare+Math.abs(50-ctx.score))}]);
...
function localCoverage(items,person){
  ...
  for(const row of rows){
    const label=clean(row.agendaTag)||'지역·현장',current=grouped.get(label)||{label,count:0,sources:new Set(),led:0};
    current.count+=1;
    if(row.source)current.sources.add(row.source);
    if(row.agency==='정치인 주도')current.led+=1;
    grouped.set(label,current);
  }
  return [...grouped.values()].map(row=>({label:row.label,count:row.count,sourceCount:row.sources.size,share:Math.round(row.count/Math.max(1,rows.length)*100),messageShare:Math.round(row.led/Math.max(1,row.count)*100)})).sort((a,b)=>b.count-a.count||a.label.localeCompare(b.label,'ko')).slice(0,5);
}
function displayContract(id,ctx,input,diagnosisScores){
  const news=list(ctx.newsNarrative.items),search=input.searchMetrics||{},frames=ctx.newsNarrative.frameSummary||{positive:0,neutral:0,negative:0},buckets=dateBuckets(news),elections=verifiedElections(ctx.person,input),demographic=demographicDisplay(ctx.cohorts),support=supportDisplay(ctx),sources=[...countBy(news,'source')].map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value||a.name.localeCompare(b.name,'ko'));
  const aggregateNewsCount=Math.max(news.length,Number(input.newsMetrics?.count)||0),aggregateSourceCount=Math.max(ctx.sourceDiversity,Number(input.newsMetrics?.sourceCount)||0),topics=list(ctx.newsNarrative.topics).filter(row=>row.basis==='direct').slice(0,5),localTopics=localCoverage(news,ctx.person),led=Number(ctx.newsNarrative.agencySummary?.led)||0,external=Number(ctx.newsNarrative.agencySummary?.external)||0,totalSearch=Number(search.pc||0)+Number(search.mobile||0),subject={name:ctx.person.name,party:ctx.person.party,office:ctx.roleTitle,region:ctx.region,overallRank:null,categoryRank:null,pc:Number(search.pc)||0,mobile:Number(search.mobile)||0,newsCount:aggregateNewsCount,sourceCount:aggregateSourceCount,frames};
  const people=[subject,...ctx.competitors.slice(0,3).map(row=>({name:row.name,party:row.party,office:row.office,region:row.jurisdiction||row.region,overallRank:null,categoryRank:null,pc:null,mobile:null,newsCount:null,sourceCount:null,frames:null}))];
  const issueTotal=Math.max(1,frames.positive+frames.neutral+frames.negative),direction=Math.round((frames.positive-frames.negative)/issueTotal*50),dominant=list(input.eventClusters)[0],persistence=dominant?{from:dominant.dateRange?.from,to:dominant.dateRange?.to,sourceCount:new Set(list(dominant.evidence).map(row=>row.source)).size,recurrences:Math.max(0,list(dominant.relatedNewsIds).length-1),events:list(dominant.evidence).map(row=>({date:row.date,source:row.source}))}:{from:'',to:'',sourceCount:0,recurrences:0,events:[]};
  const policyRows=news.filter(row=>['민생·경제','정책·입법','성과·행정'].includes(row.agendaTag)).slice(0,5).map(row=>({name:compactTitle(row.title),stage:policyStage(row.title),articles:1,outlets:1,frame:row.frame,pc:null,mobile:null,share:Math.round(100/Math.max(1,news.filter(item=>['민생·경제','정책·입법','성과·행정'].includes(item.agendaTag)).length)),specificity:{beneficiary:/청년|노인|소상공인|주민|가구|기업/.test(row.title),budget:/예산|억|조원/.test(row.title),deadline:/년|월|일까지/.test(row.title),owner:/정부|국회|위원회|부처|서울시|도|시|군|구/.test(row.title)}}));
  const contracts={
    ...
    '02':demographic,
    '03':{kind:'local',elections,status:elections.length?'공식 선거 데이터 연결':'비교 가능한 공식 선거 데이터 부족',population:[],populationStatus:list(input.officialPopulation).length?'공식 지역 인구 연결':'지역 연령별 인구 데이터 부족',issues:localTopics.map(row=>({label:row.label,count:row.count,share:row.share})),messageFit:localTopics.map(row=>({label:row.label,localShare:row.share,messageShare:row.messageShare}))},
    '04':support,
