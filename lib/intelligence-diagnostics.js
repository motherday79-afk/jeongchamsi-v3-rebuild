...
function localCoverage(items,person){
  ...
  const rows=list(items).filter(row=>{const title=clean(row.title),hasPlace=placeTerms.some(term=>term.length>=2&&title.includes(term)),hasLocalIssue=localCue.test(title);return (hasPlace||hasLocalIssue)&&!(organizationOnly.test(title)&&!hasPlace&&!/예산|교통|주거|교육|복지|개발|현안/.test(title));});
