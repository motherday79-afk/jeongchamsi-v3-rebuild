...
const MONITORING_BY_TOPIC=Object.freeze({
  ...
  '09':['정치 활동의 기사 전환','본인 주도 보도 비중'],
  '10':['우선 처방 실행 상태','진단별 추적 지표 변화']
});
...
const DISPLAY_KINDS=Object.freeze({
  ...
  '06':'risk','07':'media','08':'campaign','09':'action','10':'summary'
});
...
const DISTRIBUTION_DOMAINS=/^(?:v|news)\.daum\.net$|^(?:n\.)?news\.naver\.com$|^news\.google\.com$/i;
function normalizedOutlet(value){
  const name=clean(value).replace(/^https?:\/\//i,'').replace(/\/$/,'');
  return !name||DISTRIBUTION_DOMAINS.test(name)?'':mediaLabel(name);
}
...
function demographicDisplay(cohorts){
  ...
  const maleRank=rank('male'),femaleRank=rank('female');
  return {kind:'demographic',label:'JCS 연령·성별 지지구조 해석',cohorts:rows,maleRank,femaleRank,total:100,interpretation:`남성 기반은 ${subjectWord(maleRank[0]?.age||'분석 중')} 가장 강하고, 여성 기반은 ${subjectWord(femaleRank[0]?.age||'분석 중')} 가장 강합니다. 가장 약한 확장 구간은 남성 ${maleRank.at(-1)?.age||'분석 중'}·여성 ${femaleRank.at(-1)?.age||'분석 중'}입니다.`};
}
function dateBuckets(items,referenceAt){
  const dated=list(items).map(row=>Date.parse(row.publishedAt||row.date||'')).filter(Number.isFinite),requested=Date.parse(referenceAt||''),reference=Number.isFinite(requested)?requested:Math.max(0,...dated);
  const within=days=>list(items).filter(row=>{const stamp=Date.parse(row.publishedAt||row.date||'');const age=reference-stamp;return reference&&Number.isFinite(stamp)&&age>=0&&age<=days*86400000;}).length;
  return [{label:'24H',value:within(1)},{label:'7D',value:within(7)},{label:'30D',value:within(30)}];
}
const MEDIA_PERIOD_FIELDS=Object.freeze({'24H':'h24','7D':'d7','30D':'d30'});
function periodNews(items,label,referenceAt){
  const days=label==='24H'?1:label==='7D'?7:30,rows=list(items),requested=Date.parse(referenceAt||''),dated=rows.map(row=>Date.parse(row.publishedAt||row.date||'')).filter(Number.isFinite),reference=Number.isFinite(requested)?requested:Math.max(0,...dated);
  return rows.filter(row=>{const stamp=Date.parse(row.publishedAt||row.date||''),age=reference-stamp;return reference&&Number.isFinite(stamp)&&age>=0&&age<=days*86400000;});
}
function mediaPeriod(label,items,sourceCounts,buckets,referenceAt){
  const field=MEDIA_PERIOD_FIELDS[label],stored=list(sourceCounts),hasStoredWindow=stored.some(row=>Object.prototype.hasOwnProperty.call(row||{},field)),windowItems=periodNews(items,label,referenceAt),observed=[...countBy(windowItems,'source')].map(([name,value])=>({name,value})),rows=hasStoredWindow?stored.map(row=>({name:row.name,value:Number(row[field])||0})):label==='30D'&&stored.length?stored.map(row=>({name:row.name,value:Number(row.count)||0})):observed;
  const ledger=[...rows.reduce((map,row)=>{const name=normalizedOutlet(row.name),count=Math.max(0,Number(row.value)||0);if(name&&count)map.set(name,(map.get(name)||0)+count);return map;},new Map())].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name,'ko')),shares=normalizedHundred(ledger.map(row=>({name:row.name,count:row.count,value:row.count}))),allSources=shares.map(row=>({name:row.name,count:row.count,value:row.count,share:row.value})),majorCount=allSources.filter(row=>MAJOR_OUTLET_NAMES.includes(row.name)).reduce((sum,row)=>sum+row.count,0),sourceTotal=allSources.reduce((sum,row)=>sum+row.count,0),majorShare=sourceTotal?Math.round(majorCount/sourceTotal*100):0;
  return {label,articleCount:Number(list(buckets).find(row=>row.label===label)?.value)||0,sourceCount:allSources.length,allSources,concentration:Number(allSources[0]?.share)||0,majorShare,nonMajorShare:100-majorShare};
}
...
function verifiedElections(person,input){
  ...
  return [{year:year?`제${year}대`:'현재',election,result:/당선|선출/.test(election)?'당선':'출마 기록',voteRate:null,opponent:'',opponentRate:null,margin:null,regions:[],profileOnly:true}];
}
...
function periodFrameSummary(items,label,referenceAt){
  const rows=periodNews(items,label,referenceAt),positive=rows.filter(row=>row.frame==='긍정·성과').length,negative=rows.filter(row=>row.frame==='부정·위기').length,neutral=rows.length-positive-negative;
  return {label,positive,neutral,negative,items:rows.slice(0,6).map(row=>({title:compactTitle(row.title),url:clean(row.url),source:clean(row.source),frame:clean(row.frame)||'중립·정보'}))};
}
function policyConnectionScore(items,person,referenceAt){
  const all=list(items),rows=all.filter(isPolicyEvidence),empty={direct:0,action:0,local:0,recency:0};if(!rows.length)return {value:0,components:empty,basis:'관측된 정책·공약 근거 없음'};
  const direct=Math.round(Math.min(1,rows.length/Math.max(1,all.length)*2)*35),actionRows=rows.filter(row=>/법안|조례|예산|입법|발의|제안|추진|시행|통과|의결|가결|집행|착수|확보|완료|달성/.test(clean(row.title))),jurisdiction=clean(person?.jurisdiction),region=clean(person?.region),localRows=rows.filter(row=>/지역|지역구|주민|현장|교통|주거|교육|복지|개발/.test(clean(row.title))||(jurisdiction&&clean(row.title).includes(jurisdiction))||(region&&clean(row.title).includes(region))),recentRows=periodNews(rows,'30D',referenceAt),components={direct,action:Math.round(actionRows.length/rows.length*30),local:Math.round(localRows.length/rows.length*20),recency:Math.round(recentRows.length/rows.length*15)};
  return {value:Object.values(components).reduce((sum,value)=>sum+value,0),components,basis:'직접 연결 35 · 실행 30 · 지역 20 · 최근성 15'};
}
function persistenceShape(daily,reignitionCount=0){
  const rows=list(daily),active=rows.filter(row=>Number(row.count)>0);if(!active.length)return '데이터 부족';
  const gaps=active.slice(1).map((row,index)=>row.index-active[index].index),recent=rows.slice(-7).reduce((sum,row)=>sum+Number(row.count||0),0),prior=rows.slice(-14,-7).reduce((sum,row)=>sum+Number(row.count||0),0),span=active.at(-1).index-active[0].index;
  if(reignitionCount>0||gaps.some(gap=>gap>=7))return '재점화 이슈';
  if(recent>0&&recent>prior*1.4)return '상승 이슈';
  if(span>=7||active.length>=4)return '지속 이슈';
  return '단기 이슈';
}
function supportDisplay(ctx){
  ...
  const composition=[
    {key:'core',label:'코어 결집',value:clamp(career+Math.max(0,50-ctx.negativeShare))},
    {key:'floating',label:'유동층 이동',value:clamp(Math.max(10,100-(spread.length?Math.max(...spread)-Math.min(...spread):0)))},
    {key:'exit',label:'이탈 위험',value:clamp(Math.max(5,ctx.negativeShare+Math.abs(50-ctx.score)))}
  ];
  return {kind:'support',scale:'independent-index',composition};
}
function localCoverage(items,person){
  ...
  const localCue=/지역구|지역\s*(?:예산|현안|민심|주민|현장|교통|주거|교육|복지|개발)|구민|군민|도민|시정|도정|군정|구정|시청|도청|군청|구청/;
  const organizationOnly=/창당|해산|탈당|복당|입당|전당대회|당대표|지도부|공천/;
  const rows=list(items).slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).filter(row=>{const title=clean(row.title),hasPlace=placeTerms.some(term=>term.length>=2&&title.includes(term)),hasLocalIssue=localCue.test(title);return (hasPlace||hasLocalIssue)&&!(organizationOnly.test(title)&&!hasPlace&&!/예산|교통|주거|교육|복지|개발|현안/.test(title));});
  ...
  return [...grouped.values()].map(row=>({label:row.label,count:row.count,ledCount:row.led,sourceCount:row.sources.size,share:Math.round(row.count/Math.max(1,rows.length)*100),messageShare:Math.round(row.led/Math.max(1,row.count)*100),evidence:row.evidence})).sort((a,b)=>b.count-a.count||a.label.localeCompare(b.label,'ko')).slice(0,5);
}
export function persistenceSeries(items,referenceAt,stored=[],coverage=[]){
  const day=86400000,kst=9*3600000,parsed=list(items).map(row=>({...row,stamp:Date.parse(row.publishedAt||row.date||'')})).filter(row=>Number.isFinite(row.stamp)),requested=Date.parse(referenceAt||''),reference=Number.isFinite(requested)?requested:Math.max(0,...parsed.map(row=>row.stamp)),today=Math.floor((reference+kst)/day)*day-kst,start=today-30*day,history=new Map(stored.map(row=>[row.date,row])),scanned=new Map(coverage.map(row=>[row.date,row]));
  const daily=Array.from({length:31},(_,index)=>{const from=start+index*day,date=new Date(from+kst).toISOString().slice(0,10),old=history.get(date),count=parsed.filter(row=>row.stamp>=from&&row.stamp<from+day&&row.stamp<=reference).length,collected=(old?.collected===true&&!old.truncated)||(scanned.get(date)?.collected===true&&!scanned.get(date)?.truncated);return {index,offset:index-30,date,count:old?.count!=null?Math.max(old.count,count):count?count:collected?0:null,collected,truncated:old?.truncated===true||scanned.get(date)?.truncated===true};});
  const trend7=daily.map((row,index)=>{const window=daily.slice(Math.max(0,index-6),index+1);return {index:row.index,date:row.date,value:window.some(item=>item.count===null||item.truncated)?null:Number((window.reduce((sum,item)=>sum+item.count,0)/window.length).toFixed(2))};});
  return {daily,trend7,maxDaily:Math.max(0,...daily.map(row=>row.count||0))};
}

function imageConsistencySeries(items,referenceAt,coverage=[]){
  const rows=list(items).map(row=>({...row,stamp:Date.parse(row.publishedAt||row.date||'')})).filter(row=>Number.isFinite(row.stamp)),requested=Date.parse(referenceAt||''),reference=Number.isFinite(requested)?requested:Math.max(0,...rows.map(row=>row.stamp)),day=86400000,start=Math.floor((reference+9*3600000)/day)*day-9*3600000-29*day;
  const bars=Array.from({length:10},(_,index)=>{
    const from=start+index*3*day,to=Math.min(reference+1,from+3*day),bucket=rows.filter(row=>row.stamp>=from&&row.stamp<to),count=bucket.length;
    const collected=Array.from({length:3},(_,offset)=>new Date(from+offset*day+9*3600000).toISOString().slice(0,10)).every(date=>coverage.some(row=>row.date===date&&row.collected&&!row.truncated));
    if(!count)return {index,label:index===0?'과거':index===9?'현재':'',value:0,count:0,observed:false,collected};
    const ratio=field=>Math.max(0,...[...countBy(bucket,field).values()])/count*100,agenda=ratio('agendaTag'),frame=ratio('frame'),agency=ratio('agency'),policy=bucket.filter(isPolicyEvidence).length/count*100;
    return {index,label:index===0?'과거':index===9?'현재':'',value:clamp(Math.round(agenda*.5+frame*.2+agency*.15+policy*.15)),count,observed:true};
  }),observed=bars.filter(row=>row.observed);
  return {bars,average:observed.length?Math.round(observed.reduce((sum,row)=>sum+row.value,0)/observed.length):0};
}
function campaignFoundation(ctx,diagnosisScores,localTopics){
  const currentIndex=clamp(diagnosisScores['08']),regionalStatus=currentIndex>=62?'강함':currentIndex<42?'약함':'경합',career=contextFactors(ctx,ctx.score).career;
  const foundations=[
    {key:'career',label:'공식 경력',value:career,visual:'seal'},
    {key:'regional',label:'지역 기반',value:clamp(diagnosisScores['03']),visual:'map'},
    {key:'support',label:'지지 기반',value:clamp(diagnosisScores['04']),visual:'people'},
    {key:'competition',label:'현재 경쟁력',value:currentIndex,visual:'signal'}
  ];
  const regionalStructure=(localTopics.length?localTopics.map(row=>({name:row.label,status:row.share>=45?'강함':row.share<20?'약함':'경합',value:row.share,basis:'지역 보도·메시지 결합'})):[{name:ctx.region,status:regionalStatus,value:currentIndex,basis:'JCS 지역·인구·검색·뉴스 결합'}]).slice(0,6);
  return {kind:'campaign',mode:'foundation',foundations,regionalStructure};
}
function actionCategory(row){
  const title=clean(row?.title);
  if(/정책|공약|법안|입법|예산|조례|제안|발의|추진/.test(title))return '정책·의제 선점';
  if(/방문|참석|간담회|행사|현장|봉사|기념식|대회/.test(title))return '현장·행사 참여';
  if(/창당|해산|탈당|복당|입당|전당대회|당대표|지도부|공천/.test(title))return '조직·정당 활동';
  return '발언·논평';
}
function actionDisplay(news,topics,led,external){
  const activities=list(news).slice(0,8).map(row=>({title:compactTitle(row.title),category:actionCategory(row),frame:row.frame||'중립·정보',agency:row.agency||'외부 서사',date:clean(row.date),evidenceUrl:clean(row.url),source:clean(row.source)})).filter(row=>row.title&&row.evidenceUrl);
  const counts=[...activities.reduce((map,row)=>map.set(row.category,(map.get(row.category)||0)+1),new Map())].map(([label,value])=>({label,value}));
  const composition=normalizedHundred((counts.length?counts:[{label:'관측 활동 없음',value:1}])).map(row=>({label:row.label,value:row.value}));
  const firstMover=activities.filter(row=>row.agency==='정치인 주도').length,joined=activities.length-firstMover;
  return {kind:'action',title:'정치 활동·미디어 전환 진단',composition,activityCount:activities.length,activities,initiative:{firstMover,joined},conversion:{led,external,total:led+external},agendaConversion:topics.map(row=>({label:row.label,articles:row.count,outlets:row.sourceCount,share:row.share}))};
}
function displayContract(id,ctx,input,diagnosisScores){
  const news=list(ctx.newsNarrative.items),history=input.historicalAggregates||{},search=input.searchMetrics||{},frames=ctx.newsNarrative.frameSummary||{positive:0,neutral:0,negative:0},storedBuckets=list(input.newsMetrics?.periodCounts),buckets=storedBuckets.length===3?storedBuckets.map(row=>({label:clean(row.label),value:Math.max(0,Number(row.value)||0)})):dateBuckets(news,input.referenceAt),elections=verifiedElections(ctx.person,input),demographic=demographicDisplay(ctx.cohorts),support=supportDisplay(ctx),observedSources=[...countBy(news,'source')].map(([name,value])=>({name,value})),rawSources=list(input.sourceCounts).length?list(input.sourceCounts).map(row=>({name:row.name,value:Number(row.count)||0})):observedSources,sourceLedger=[...rawSources.reduce((map,row)=>{const name=normalizedOutlet(row.name),count=Number(row.value)||0;if(name&&count>0)map.set(name,(map.get(name)||0)+count);return map;},new Map())].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name,'ko')),sourceShares=normalizedHundred(sourceLedger.map(row=>({name:row.name,count:row.count,value:row.count}))),sources=sourceShares.map(row=>({name:row.name,count:row.count,value:row.count,share:row.value}));
  const aggregateNewsCount=Math.max(news.length,Number(input.newsMetrics?.count)||0),aggregateSourceCount=Math.max(ctx.sourceDiversity,Number(input.newsMetrics?.sourceCount)||0),topics=list(ctx.newsNarrative.topics).filter(row=>row.basis==='direct').slice(0,5),localTopics=Array.isArray(history.localTopics)?history.localTopics:localCoverage(list(input.consistencyNews).length?input.consistencyNews:news,ctx.person),led=Number(ctx.newsNarrative.agencySummary?.led)||0,external=Number(ctx.newsNarrative.agencySummary?.external)||0,totalSearch=Number(search.pc||0)+Number(search.mobile||0),storedFrames=list(history.framePeriods),subject={id:ctx.person.id,name:ctx.person.name,party:ctx.person.party,office:ctx.roleTitle,region:ctx.region,overallRank:null,categoryRank:null,pc:Number(search.pc)||0,mobile:Number(search.mobile)||0,newsCount:aggregateNewsCount,sourceCount:aggregateSourceCount,frames,framePeriods:storedFrames.length===3?storedFrames:['24H','7D','30D'].map(label=>periodFrameSummary(news,label,input.referenceAt)),agendas:topics,election:elections[0]||null};
  const people=[subject,...ctx.competitors.slice(0,3).map(row=>({id:row.id,name:row.name,party:row.party,office:row.office,region:row.jurisdiction||row.region,overallRank:row.overallRank??null,categoryRank:row.categoryRank??null,pc:row.pc??null,mobile:row.mobile??null,newsCount:row.newsCount??null,sourceCount:row.sourceCount??null,newsPeriods:list(row.newsPeriods),frames:row.frames||null,framePeriods:list(row.framePeriods),agendas:list(row.agendas),election:row.election||null}))].map((row,index)=>({...row,competition:{index:index===0?clamp(diagnosisScores['05']):clamp(ctx.competitors[index-1]?.score),basis:index===0?'검색·뉴스·정치 구조':'공식 경력·직책 구조'}}));
  const subjectCompetition=people[0]?.competition?.index||0;people.forEach(row=>{row.competition.gap=Math.round((row.competition.index-subjectCompetition)*10)/10;});
  const issueTotal=Math.max(1,frames.positive+frames.neutral+frames.negative),direction=Math.round((frames.positive-frames.negative)/issueTotal*50),dominant=list(input.eventClusters)[0],eventDates=list(dominant?.evidence).map(row=>Date.parse(row.date||'')).filter(Number.isFinite).sort((a,b)=>a-b),durationDays=eventDates.length?Math.max(1,Math.ceil((eventDates.at(-1)-eventDates[0])/86400000)+1):0,reignitionCount=eventDates.slice(1).filter((stamp,index)=>stamp-eventDates[index]>7*86400000).length,persistence=dominant?{from:dominant.dateRange?.from,to:dominant.dateRange?.to,durationDays,sourceCount:new Set(list(dominant.evidence).map(row=>row.source)).size,recurrences:Math.max(0,list(dominant.relatedNewsIds).length-1),reignitionCount,events:list(dominant.evidence).map(row=>({date:clean(row.date),source:clean(row.source),title:compactTitle(row.title),url:clean(row.url)})).filter(row=>row.date&&row.title&&row.url)}:{from:'',to:'',durationDays:0,sourceCount:0,recurrences:0,reignitionCount:0,events:[]};
  const storedDaily=list(history.persistence?.daily),series=persistenceSeries(list(input.newsItems).length?input.newsItems:news,input.referenceAt,storedDaily,input.newsCoverage||[]);
  Object.assign(persistence,series);
  persistence.shape=persistenceShape(persistence.daily,persistence.reignitionCount);
  const officialPopulation=list(input.officialPopulation).filter(row=>clean(row.age)&&Number(row.totalCount)>0&&Number.isFinite(Number(row.totalShare)));
  const voterPopulation=officialPopulation.map(row=>{const split=normalizedHundred([{value:Number(row.maleShare)||0},{value:Number(row.femaleShare)||0}]);return {age:clean(row.age),totalCount:Number(row.totalCount)||0,totalShare:Number(row.totalShare)||0,maleShare:split[0]?.value||0,femaleShare:split[1]?.value||0};});
  const localEvidenceCount=localTopics.reduce((sum,row)=>sum+Number(row.count||0),0),localLedCount=localTopics.reduce((sum,row)=>sum+Math.round(Number(row.count||0)*Number(row.messageShare||0)/100),0),regionalBase=clamp(diagnosisScores['03']),localVisibility=clamp(localEvidenceCount?35+localEvidenceCount*12:regionalBase*.58),messageConnection=clamp(localEvidenceCount?42+localLedCount*15:average([regionalBase,diagnosisScores['01'],diagnosisScores['09']]));
  const regionalReach=clamp(average([regionalBase,localVisibility,messageConnection]));
  const messagePath=[{key:'voter',label:'지역 유권자 기반',value:regionalBase},{key:'visibility',label:'지역 의제 노출',value:localVisibility},{key:'connection',label:'정치인 메시지 연결',value:messageConnection},{key:'reach',label:'지역 최종 도달',value:regionalReach}];
  const localJudgment=`JCS 지역 진단 · ${ctx.region}에서 지역 기반은 ${regionalBase>=62?'강한 편':regionalBase<42?'보강이 필요한 편':'경합 구간'}이며, 현재 메시지 연결력은 ${messageConnection>=62?'지역 의제로 전환되는 흐름':messageConnection<42?'전국 이슈에 가려진 흐름':'지역과 전국 의제가 맞서는 흐름'}이다.`;
  const frequencyBands={repeat:sources.filter(row=>row.count>=3).length,double:sources.filter(row=>row.count===2).length,single:sources.filter(row=>row.count===1).length},majorCount=sources.filter(row=>MAJOR_OUTLET_NAMES.includes(row.name)).reduce((sum,row)=>sum+row.count,0),allSourceArticles=sources.reduce((sum,row)=>sum+row.count,0),majorShare=allSourceArticles?Math.round(majorCount/allSourceArticles*100):0,mediaPeriods=['24H','7D','30D'].map(label=>mediaPeriod(label,news,input.sourceCounts,buckets,input.referenceAt)),campaign=campaignFoundation(ctx,diagnosisScores,localTopics),action=actionDisplay(news,topics,led,external);
  const clarityComponents=[
    topics.length?{key:'agenda',label:'대표 의제 집중도',value:clamp(topics[0]?.share),basis:'최근 보도 핵심 의제 점유율'}:null,
    news.length?{key:'policy',label:'정책·공약 연결도',value:clamp(ctx.policyConnection?.value),basis:ctx.policyConnection?.basis}:null,
    news.length?{key:'frame',label:'보도 프레임 일관성',value:clamp(Math.max(frames.positive,frames.neutral,frames.negative)/issueTotal*100),basis:'긍정·중립·부정 보도 프레임 집중도'}:null,
    led+external?{key:'agency',label:'정치인 주도 메시지',value:clamp(led/Math.max(1,led+external)*100),basis:'정치인 주도 보도 비율'}:null
  ].filter(Boolean),brandClarity={components:clarityComponents,average:clarityComponents.length?Math.round(average(clarityComponents.map(row=>row.value))):0},imageConsistency=list(history.imageConsistency?.bars).length===10?history.imageConsistency:imageConsistencySeries(list(input.consistencyNews).length?input.consistencyNews:news,input.referenceAt,input.newsCoverage||[]),cohortTotals=list(demographic.cohorts).map(row=>Number(row.total)||0),generationExpansion=clamp(100-(cohortTotals.length?Math.max(...cohortTotals)-Math.min(...cohortTotals):100)),expansionAxes=[{key:'generation',label:'세대',value:generationExpansion},{key:'regional',label:'지역',value:clamp(diagnosisScores['03'])},{key:'online',label:'온라인',value:clamp(diagnosisScores['01'])},{key:'policy',label:'정책',value:clamp(ctx.policyConnection?.value)},{key:'competition',label:'경쟁',value:clamp(diagnosisScores['05'])}],expansionPercent=Math.round(average(expansionAxes.map(row=>row.value))),expansion={axes:expansionAxes,percent:expansionPercent,label:expansionPercent>=85?'다방면 확장 가능성 기대':expansionPercent>=70?'확장 기대':expansionPercent>=50?'확장 가능성 중립':'확장 기반 보완'};
  const signalTitle=compactTitle(ctx.newsNarrative.dominantEvent?.title||`${ctx.person.name} 현재 정치 활동`),signalEvidence=history.signalEvidence||news.find(row=>clean(row.title)===clean(ctx.newsNarrative.dominantEvent?.title))||news[0]||{};
  const summarySourceIds=['01','02','03','04','05','06','09'],leadingGender=(demographic.maleRank[0]?.value||0)>=(demographic.femaleRank[0]?.value||0)?{label:'남성',value:demographic.maleRank[0]?.value||0}:{label:'여성',value:demographic.femaleRank[0]?.value||0},leadingAge=[...demographic.cohorts].sort((a,b)=>b.total-a.total)[0],leadingSupport=[...support.composition].sort((a,b)=>b.value-a.value)[0],resilience=clamp(average([contextFactors(ctx,ctx.score).career,100-ctx.negativeShare,ctx.policyLink||0]));
  const summaryItems=[
    {sourceId:'01',label:'브랜드 상태',value:clamp(diagnosisScores['01']),detail:ctx.newsNarrative.attentionQuality||'현재 브랜드 신호'},
    {sourceId:'02',label:'지지 우위 세대·성별',value:leadingAge?.total||0,detail:`${leadingAge?.age||'분석 중'} · ${leadingGender.label}`},
    {sourceId:'03',label:'지역 적합도',value:clamp(diagnosisScores['03']),detail:ctx.region},
    {sourceId:'04',label:'지지층 구조',value:leadingSupport?.value||0,detail:leadingSupport?.label||'분석 중'},
    {sourceId:'05',label:'경쟁 위치',value:clamp(diagnosisScores['05']),detail:clamp(diagnosisScores['05'])>=60?'우위':clamp(diagnosisScores['05'])<40?'열세':'경합'},
    {sourceId:'06',label:'이슈 회복력',value:resilience,detail:resilience>=65?'회복 기반 강함':resilience<40?'선제 대응 필요':'관리 가능'},
    {sourceId:'09',label:'정치 활동·미디어 전환',value:action.activityCount?clamp((led/Math.max(1,led+external))*70+Math.min(30,action.activityCount*4)):0,detail:`주도 ${led} · 외부 ${external}`}
  ];
  const contracts={
    '01':{kind:'brand',nowSignal:signalTitle,evidenceUrl:clean(signalEvidence.url),evidenceTitle:clean(signalEvidence.title)||signalTitle,evidenceSource:clean(signalEvidence.source),indicators:diagnosisVisualization('01',ctx.score,ctx).bars.map(row=>({label:row.label==='차별성'?'경쟁자 차별성':row.label,value:signed(row.value)})),brandClarity,imageConsistency,expansion,policyConnection:ctx.policyConnection,search:{pc:Number(search.pc)||0,mobile:Number(search.mobile)||0,mobileShare:totalSearch?Math.round(Number(search.mobile||0)/totalSearch*100):0},newsPeriods:buckets,news:buckets,pastRisks:riskTags(news),totalSign:{risk:Math.min(50,Math.round(frames.negative/issueTotal*50)),opportunity:Math.min(50,Math.round((frames.positive+led*.5)/issueTotal*50))}},
    '02':demographic,
    '03':{kind:'local',agenda:Array.isArray(history.localAgenda)?history.localAgenda:buildLocalAgenda(ctx.person,voterPopulation,input.newsItems||[]),population:voterPopulation,populationBasis:voterPopulation.length?'행정안전부 공식 지역 인구 구조':'',issues:localTopics.map(row=>({label:row.label,count:row.count,share:row.share,evidence:row.evidence})),messageFit:localTopics.map(row=>({label:row.label,localShare:row.share,messageShare:row.messageShare,gap:Math.abs(row.share-row.messageShare),count:row.count,ledCount:row.ledCount,sourceCount:row.sourceCount,evidence:row.evidence})),messagePath,localJudgment},
    '04':support,
    '05':{kind:'competitor',people:people.map((row,index)=>({...row,newsPeriods:index===0?buckets:list(row.newsPeriods),frames:index===0?frames:row.frames,framePeriods:index===0?subject.framePeriods:list(row.framePeriods)})),metrics:['NOW 순위','PC·모바일 검색량','24H·7D·30D 뉴스량','기간별 긍정·중립·부정','기간별 대표 뉴스','주요 의제 점유율','최근 선거·현재 경쟁 신호']},
    '06':{kind:'risk',lifecycle:Array.isArray(history.lifecycle)?restoreIssueLifecycle(history.lifecycle,input.referenceAt):buildIssueLifecycle(ctx.person,input.mediaTimelineItems||input.newsItems||[],input.referenceAt),nowSignal:compactTitle(ctx.newsNarrative.dominantEvent?.title||`${ctx.person.name} 현재 정치 활동`),direction,frames,velocity:buckets,persistence},
    '07':{kind:'media',issueObservations:Array.isArray(history.issueObservations)?history.issueObservations:buildMediaObservations(ctx.person,input.mediaTimelineItems||input.newsItems||[],input.referenceAt),observationBasis:history.observationBasis||input.mediaTimelineBasis||'최근 30일 수집 기사 기준',articleCount:aggregateNewsCount,sourceCount:sources.length,sourceSpread:sources,allSources:sources,periods:mediaPeriods,newsBasis:clean(input.newsBasis)||'Google News RSS · 최근 1일/7일/30일',newsCollectedAt:clean(input.newsCollectedAt||input.referenceAt),majorOutletNames:[...MAJOR_OUTLET_NAMES],majorShare,nonMajorShare:100-majorShare,frequencyBands,topSources:sources.slice(0,5),remainingSources:sources.slice(5),concentration:sources,ownership:{led,external},agendaPenetration:topics.map(row=>({label:row.label,articles:row.count,outlets:row.sourceCount})),...(input?.youtube?.channel?.id?{youtube:input.youtube}:{})},
    '08':campaign,
    '09':action,
    '10':{kind:'summary',sourceIds:summarySourceIds,items:summaryItems,totalScore:clamp(average(summarySourceIds.map(key=>diagnosisScores[key])))}
  };
...
function makeDiagnoses(person,input){
  ...
  const terms=termsCount(person.terms),role=roleWeight(person),representativeNewsCount=newsNarrative.items.length,newsCount=Math.max(representativeNewsCount,Number(input.newsMetrics?.count)||0),policyConnection=policyConnectionScore(newsNarrative.items,person,input.referenceAt),policyLink=policyConnection.value,sourceDiversity=Math.max(newsNarrative.sourceDiversity,Number(input.newsMetrics?.sourceCount)||0),search=input.searchMetrics||{},frames=newsNarrative.frameSummary||{positive:0,neutral:0,negative:0};
  ...
  const ctxBase={person,newsNarrative,topTopic,secondTopic,strongAge,weakAge,competitor,region,roleTitle,committee,policyLink,policyConnection,sourceDiversity,newsCount,negativeShare,cohorts,competitors,riskFrame:newsNarrative.riskFrames[0]||`${secondTopic} 이미지 충돌`,issueLabel,isMay18},decisionMap=new Map(list(input.politicalStates).map(row=>[row.id,row]));
  return POLITICAL_DIAGNOSIS_TOPICS.map(([id,title,type])=>{
    ...
    if(id==='07'&&input.youtube?.channel?.id)sourceTypes.push('YouTube Data API');
...
function prescriptionContent(id,ctx){
  ...
  const rows={
    ...
    '09':[`${topTopic} 정치 활동을 본인 주도 보도로 전환한다.`,[`${topTopic} 활동의 원문과 실행 근거를 먼저 공개`,`이슈 합류보다 직접 선점할 활동의 순서를 앞당김`,`관련 기사 이후 후속 결과를 같은 메시지로 반복`],`${companionWord(strongAge)} 뉴스 유입층`,`${topTopic} 활동의 시작·실행·결과를 한 흐름으로 연결`,['공식 발표','현장 브리핑','보도자료','검색형 콘텐츠']],
    '10':[`진단 우선순위에 따라 실행 순서를 하나로 통합한다.`,[`가장 시급한 처방부터 즉시 실행`,`30일·90일 처방의 책임 행동과 확인 지표를 고정`,`중장기 처방은 진단값 갱신 때마다 우선순위를 재평가`],`핵심 실행 조직과 의사결정 주체`,`진단 근거 → 우선 처방 → 실행 확인의 단일 기준`,['실행 브리핑','진단 갱신 보고','성과 점검 회의']]
  };
  return rows[id];
}
...
function makePrescriptions(person,diagnoses,input){
  ...
  const expectedImpactFor=id=>({
    ...
    '09':`${topTopic} 정치 활동이 관련 기사와 본인 주도 보도로 이어지는 흐름이 강화된다.`,
    '10':`우선 처방의 실행 상태와 연결 진단의 추적 지표를 같은 기준으로 점검할 수 있다.`
  }[id]);
  const rows=PRESCRIPTION_META.map(([id,title,type,linkedDiagnosisIds])=>{
    ...
    const timing=id==='06'?'0~6시간 · 24시간 · 72시간 · 7일':id==='08'?'30일 · 60일 · 90일':id==='10'?'즉시 · 30일 · 90일 · 중장기':id==='01'||id==='03'?'즉시 착수 · 30일 집중 운영':'즉시 착수 · 30일 · 90일 점검';
