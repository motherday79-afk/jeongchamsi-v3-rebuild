...
function periodNews(items,label,referenceAt){
  const days=label==='24H'?1:label==='7D'?7:30,rows=list(items),requested=Date.parse(referenceAt||''),dated=rows.map(row=>Date.parse(row.date||row.publishedAt||'')).filter(Number.isFinite),reference=Number.isFinite(requested)?requested:Math.max(0,...dated);
  return rows.filter(row=>{const stamp=Date.parse(row.date||row.publishedAt||''),age=reference-stamp;return reference&&Number.isFinite(stamp)&&age>=0&&age<=days*86400000;});
}
...
function persistenceSeries(items,referenceAt){
  const parsed=list(items).map(row=>({...row,stamp:Date.parse(row.date||row.publishedAt||'')})).filter(row=>Number.isFinite(row.stamp)),requested=Date.parse(referenceAt||''),reference=Number.isFinite(requested)?requested:Math.max(0,...parsed.map(row=>row.stamp)),day=86400000,start=reference-29*day;
  const daily=Array.from({length:30},(_,index)=>{const from=start+index*day,to=from+day,count=parsed.filter(row=>row.stamp>=from&&row.stamp<to).length;return {index,offset:index-29,date:new Date(from).toISOString().slice(0,10),count};});
  const trend7=daily.map((row,index)=>{const window=daily.slice(Math.max(0,index-6),index+1);return {index:row.index,date:row.date,value:Number((window.reduce((sum,item)=>sum+item.count,0)/window.length).toFixed(2))};});
  return {daily,trend7,maxDaily:Math.max(0,...daily.map(row=>row.count))};
}

function imageConsistencySeries(items,referenceAt){
  const rows=list(items).map(row=>({...row,stamp:Date.parse(row.date||row.publishedAt||'')})).filter(row=>Number.isFinite(row.stamp)),requested=Date.parse(referenceAt||''),reference=Number.isFinite(requested)?requested:Math.max(0,...rows.map(row=>row.stamp)),day=86400000,start=reference-30*day;
  const bars=Array.from({length:10},(_,index)=>{
    const from=start+index*3*day,to=index===9?reference+day:from+3*day,bucket=rows.filter(row=>row.stamp>=from&&row.stamp<to),count=bucket.length;
    if(!count)return {index,label:index===0?'과거':index===9?'현재':'',value:0,count:0,observed:false};
...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const storedDaily=list(history.persistence?.daily),series=storedDaily.length===30?{daily:storedDaily,trend7:storedDaily.map((row,index)=>{const window=storedDaily.slice(Math.max(0,index-6),index+1);return {index:row.index,date:row.date,value:Number((window.reduce((sum,item)=>sum+Number(item.count||0),0)/window.length).toFixed(2))};}),maxDaily:Math.max(0,...storedDaily.map(row=>Number(row.count)||0))}:persistenceSeries(list(input.newsItems).length?input.newsItems:(list(dominant?.evidence).length?dominant.evidence:news),input.referenceAt);
