...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const contracts={
    ...
    '04':support,
    '05':{kind:'competitor',people:people.map((row,index)=>({...row,newsPeriods:index===0?buckets:list(row.newsPeriods),frames:index===0?frames:row.frames})),metrics:['NOW 순위','PC·모바일 검색량','24H·7D·30D 뉴스량','긍정·중립·부정','주요 의제 점유율','최근 선거·현재 경쟁 신호']},
