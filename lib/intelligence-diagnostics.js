...
function mediaPeriod(label,items,sourceCounts,buckets,referenceAt){
  ...
  return {label,articleCount:Number(list(buckets).find(row=>row.label===label)?.value)||0,sourceCount:allSources.length,allSources,majorShare,nonMajorShare:100-majorShare};
}
...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  Object.assign(persistence,persistenceSeries(list(input.newsItems).length?input.newsItems:(list(dominant?.evidence).length?dominant.evidence:news),input.referenceAt));
