...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const contracts={
    ...
    '07':{kind:'media',articleCount:aggregateNewsCount,sourceCount:sources.length||aggregateSourceCount,sourceSpread:sources,frequencyBands,topSources:sources.slice(0,5),remainingSources:sources.slice(5),concentration:sources.map(row=>({...row,share:Math.round(row.value/Math.max(1,sources.reduce((sum,item)=>sum+item.value,0))*100)})),search:{pc:Number(search.pc)||0,mobile:Number(search.mobile)||0,mobileShare:totalSearch?Math.round(Number(search.mobile||0)/totalSearch*100):0},ownership:{led,external},agendaPenetration:topics.map(row=>({label:row.label,articles:row.count,outlets:row.sourceCount}))},
    '08':campaign,
