...
function makeDiagnoses(person,input){
  ...
  const dominant=newsNarrative.dominantEvent||{title:`${roleTitle} 공식 활동`,date:'현재',source:'공식 프로필'},dominantTitle=clean(dominant.title),isMay18=/5\s*[\.·]?\s*18/.test(dominantTitle),issueLabel=isMay18?'5·18 역사 인식':negativeShare?`${newsNarrative.riskFrames[0]||topTopic} 부정 이슈`:topTopic;
