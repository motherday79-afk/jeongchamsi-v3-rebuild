...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const voterPopulation=demographic.cohorts.map(row=>{const official=list(input.officialPopulation).find(item=>clean(item.age)===row.age),split=official?normalizedHundred([{value:Number(official.maleShare)||0},{value:Number(official.femaleShare)||0}]):[{value:row.male},{value:row.female}];return {age:row.age,totalShare:row.total,maleShare:split[0]?.value||0,femaleShare:split[1]?.value||0};});
