...
function displayContract(id,ctx,input,diagnosisScores){
  ...
  const stages=['발표','제안','검토','통과','시행','완료'],policyRows=news.filter(row=>['민생·경제','정책·입법','성과·행정'].includes(row.agendaTag)).slice(0,5).map(row=>{const stage=policyStage(row.title);return {name:compactTitle(row.title),stage,stageIndex:stages.indexOf(stage),articles:1,outlets:1,frame:row.frame,pc:null,mobile:null,share:Math.round(100/Math.max(1,news.filter(item=>['민생·경제','정책·입법','성과·행정'].includes(item.agendaTag)).length)),specificity:{beneficiary:/청년|노인|소상공인|주민|가구|기업/.test(row.title),budget:/예산|억|조원/.test(row.title),deadline:/년|월|일까지/.test(row.title),owner:/정부|국회|위원회|부처|서울시|도|시|군|구/.test(row.title)}};});
  const contracts={
