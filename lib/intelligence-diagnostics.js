...
function makeDiagnoses(person,input){
  ...
  const ctxBase={person,newsNarrative,topTopic,secondTopic,strongAge,weakAge,competitor,region,roleTitle,committee,policyLink,sourceDiversity,newsCount,negativeShare,cohorts,competitors,riskFrame:newsNarrative.riskFrames[0]||`${secondTopic} 이미지 충돌`,issueLabel,isMay18};
...
function makePrescriptions(person,diagnoses,input){
  ...
  const rows=PRESCRIPTION_META.map(([id,title,type,linkedDiagnosisIds])=>{
    ...
    const diagnosisBasis=linked.map(item=>`${item.id} · ${item.headline}`),evidenceIds=unique(linked.flatMap(item=>item.evidenceIds||[])),monitoringIndicators=unique([`${topTopic} 뉴스 점유`,`${region} 메시지 반응`,...linked.slice(0,2).map(item=>`${item.title} JCS 지수`)]);
    return {id,linkedDiagnosisIds,sourceFindings:diagnosisBasis,diagnosisBasis,title,objective:`${person.name}의 ${linked.map(item=>item.title.replace(/ 진단| 분석/g,'')).join('·')} 결과를 실행 가능한 변화로 전환`,strategicJudgment:judgment,recommendedActions:actions,actions,targetGroups:[target],target,messageDirection,channels,timing,immediateActions:actions.slice(0,1),actionsWithin30Days:actions.slice(1,2),actionsWithin90Days:actions.slice(2,3),longTermActions:[`분기마다 ${monitoringIndicators[0]}를 재측정하고 전략을 갱신`],priority:'',expectedImpact:`${linked[0].headline} 현재 판단을 ${topTopic} 실행 성과로 전환`,monitoringIndicators,evidenceIds,visualization:{type,...visualData},updatedAt:clean(input.snapshot)||'2026-09-03',algorithmVersion:input.algorithmVersion,urgency:clamp(100-average+(id==='06'?12:0))};
  });
