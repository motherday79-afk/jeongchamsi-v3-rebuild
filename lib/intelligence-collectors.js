...
import { fetchYouTubeChannelData } from './youtube-data.js';
...
export async function collectPoliticianRaw(person,context={},options={}){
  const shared={fetchImpl:options.fetchImpl||fetch,env:options.env||process.env,now:options.now||Date.now,timeoutMs:options.timeoutMs,exclusions:context?.newsExclusions||[],collectionProfile:context.collectionProfile};
  const delays=options.retryDelays===undefined?[0,250,750]:options.retryDelays;
  const youtubeCollector=options.fetchYouTubeChannelData||fetchYouTubeChannelData;
  const [searchAds,news,youtube]=await Promise.all([
    ...
    context?.youtubeChannel?.channelId?withRetry('YOUTUBE_DATA_API',()=>youtubeCollector(context.youtubeChannel,shared),delays):Promise.resolve({value:null,error:null,attempts:0})
  ]);
  ...
  return {
    ...
    officialProfile,collectionProfile:context.collectionProfile||null,
    searchAds:searchAds.value,news:news.value,youtube:youtube.value,
    officialContext:context?.officialContext||null,
    sources:[searchAds.value?.source,news.value?.source,youtube.value?.source,{type:'OFFICIAL_PROFILE',label:String(person?.source||'정참시 공식 프로필')}].filter(Boolean),
    sourceErrors:[searchAds.error,news.error,youtube.error].filter(Boolean)
  };
}
