...
export async function collectPoliticianRaw(person,context={},options={}){
  const shared={fetchImpl:options.fetchImpl||fetch,env:options.env||process.env,now:options.now||Date.now,timeoutMs:options.timeoutMs,exclusions:context?.newsExclusions||[]};
