import { spendActivityCredit } from './activity-points.js';
...
import {grantPersonAnalysisAccess,paidResultWithAccess} from './person-analysis-access.js';
...
export async function publishOnePerson({command,repository,profiles,personId,refreshId,now,billing}){
 for(let attempt=0;attempt<8;attempt++){
  ...
  if(billing){const wallet=parse(before[8])||{},paid=(wallet.ledger||[]).find(x=>x.type==='person-refresh'&&x.requestId===billing.requestId);if(paid){if(paid.personId!==personId)throw Error('INVALID_REQUEST_ID');return paidResultWithAccess(paid.result,wallet,now());}}
  ...
  if(billing){
   ...
   wallet.balance-=billing.fee;spendActivityCredit(wallet,billing.fee);
   const analysisAccess=grantPersonAnalysisAccess(wallet,personId,billing.requestId,next.publishedAt);
   const paidResult={ok:true,personId,name:person.name,points:billing.fee,publishedAt:next.publishedAt,rank:ranks.byId[personId]?.rank,requestId:billing.requestId,analysisAccess};
