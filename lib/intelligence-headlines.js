...
export function analyzeNewsHeadlines(person,items=[]){
  ...
  const enriched=selected.map(row=>{
    const title=clean(row.title),description=clean(row.description).replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').slice(0,360),agendaTag=topicFor(`${title} ${description}`),source=clean(row.source)||'출처 미표기',date=clean(row.publishedAt||row.date).slice(0,10);
