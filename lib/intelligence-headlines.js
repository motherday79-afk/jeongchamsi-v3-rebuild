...
const NEGATIVE=/논란|의혹|수사|비판|공격|사과|징계|고발|갈등|반발|실패|도발|왜곡|폄훼|망언|모독|손배소|손해배상|소송|피소/;
...
const isoDate=value=>{const time=stamp(value);return time?new Date(time).toISOString():'';};
...
export function analyzeNewsHeadlines(person,items=[],options={}){
  const selected=options.all?(Array.isArray(items)?items:[]):representativeRows(Array.isArray(items)?items:[]),reference=Math.max(0,...selected.map(item=>stamp(item.publishedAt||item.date)));
  const enriched=selected.map(row=>{
    const title=clean(row.title),description=clean(row.description).replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').slice(0,360),agendaTag=topicFor(`${title} ${description}`),source=clean(row.source)||'출처 미표기',date=isoDate(row.publishedAt||row.date);
    return {id:stableId(`${normalizedTitle(title)}|${date}|${source}`),date,publishedAt:clean(row.publishedAt||row.date),source,title,description,url:clean(row.url),agendaTag,frame:frameFor(`${title} ${description}`),agency:agencyFor(title,person),diagnosisRefs:REF_BY_TOPIC[agendaTag]||['01','07']};
  });
  ...
  const politicalMeaning=attentionQuality==='정치적 부담'?`${top.label} 대표 뉴스 ${top.count||0}건이 부정 프레임으로 분류돼 노출 확대보다 정치적 부담이 크다.`:attentionQuality==='정치 자산'?`${top.label} 대표 뉴스 ${top.count||0}건이 성과 프레임으로 분류돼 대표 의제로 연결된다.`:`${top.label} 대표 뉴스 ${top.count||0}건의 성과와 부담을 기사별로 구분한다.`;
