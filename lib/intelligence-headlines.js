...
export function analyzeNewsHeadlines(person,items=[]){
  ...
  const enriched=selected.map(row=>{
    ...
    return {id:stableId(`${normalizedTitle(title)}|${date}|${source}`),date,source,title,description,url:clean(row.url),agendaTag,frame:frameFor(`${title} ${description}`),agency:agencyFor(title,person),diagnosisRefs:REF_BY_TOPIC[agendaTag]||['01','07']};
  });
