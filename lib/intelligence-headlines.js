const TOPIC_RULES=Object.freeze([
  ...
  {label:'논란·위기',words:/논란|의혹|수사|비판|공격|사과|해명|징계|고발|위기/},
...
const NEGATIVE=/논란|의혹|수사|비판|공격|사과|징계|고발|갈등|반발|실패/;
...
export function analyzeNewsHeadlines(person,items=[]){
  ...
  const dominantEvent=dominantRow?Object.fromEntries(['title','date','source','agendaTag','frame','agency'].map(key=>[key,dominantRow[key]])):{title:`${structuralCue||clean(person?.name)} 현재 정치 역할`,date:'현재',source:'공식 프로필',agendaTag:top.label,frame:'중립·정보',agency:'정치인 주도'};
  const politicalMeaning=attentionQuality==='정치적 부담'?`${dominantEvent.title} 보도가 관심을 만들고 있으나 부정 프레임이 우세해 노출 확대보다 정치적 부담의 통제가 먼저다.`:attentionQuality==='정치 자산'?`${dominantEvent.title} 보도가 관심을 만들고 있으며 긍정·성과 프레임이 우세해 대표 의제로 축적할 수 있다.`:`${dominantEvent.title} 보도는 긍정과 부정 효과가 함께 나타나 사건별 정치적 의미를 분리해 관리해야 한다.`;
