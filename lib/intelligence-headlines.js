...
export function analyzeNewsHeadlines(person,items=[]){
  ...
  const counts=new Map(),sources=new Map(),recent=new Map(),prior=new Map();
  ...
  const topics=[...counts].map(([label,count])=>({
    ...
    basis:'direct'
  })).sort((a,b)=>b.count-a.count||b.sourceCount-a.sourceCount||a.label.localeCompare(b.label,'ko'));
  ...
  const mediaImage=enriched.length?`${top.label} 의제가 ${top.share}%로 뉴스 프레임을 주도한다.`:`${structuralCue}를 중심으로 정치 서사의 기준축이 형성된다.`;
  return {
    items:enriched,
    topics:finalTopics,
    ...
    narratives:{
      days30:enriched.length?`${clean(person?.name)}의 최근 30일 노출은 ${top.label}을 중심으로 형성됐고, ${led.length>=dragged.length?'직접 주도한 메시지가 서사를 견인한다.':'외부 사건에 반응한 보도가 직접 주도 메시지보다 많다.'}`:`${clean(person?.name)}의 최근 서사는 ${structuralCue}의 현재 역할과 정치 구조를 중심으로 해석된다.`,
