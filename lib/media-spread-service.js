import {buildMediaIndex,analyzeMediaIndex} from './media-spread.js';
...
export function createMediaSpreadService({command,profiles,now=Date.now,scope,env=process.env}={}){
 ...
 async function load(){
  ...
  const token=pointer+'|'+revision,old=cache.get(scopeKey);if(old?.token===token&&old.until>Date.now())return old.promise;
  const promise=(async()=>{
   const people=await loadProfiles();
   const stored=decode(await command(['GET',K.mediaIndex(pointer)]));
   if(stored?.revision===revision&&stored.index?.version===1){return {...stored.index,people:buildMediaIndex([],people).people};}
