...
export function createPointService({command,clock=()=>Date.now()}={}){
 ...
 return {
 async member(user,id){admin(user);const users=await read(TARGET_KEYS.users);if(!users[id])throw new Error('USER_NOT_FOUND');const [wallet,orders]=await Promise.all([read(walletKey(id)),read(POINT_ORDERS)]);return {ok:true,userId:id,wallet,orders:(orders.items||[]).filter(o=>o.userId===id).sort((a,b)=>b.createdAt-a.createdAt)};},
