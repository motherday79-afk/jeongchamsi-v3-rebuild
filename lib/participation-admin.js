import { ACTIVITY_KEYS, activityKeys, activityPolicy, activityStatus, validateActivityPolicy, revokeActivity, spendActivityCredit } from './activity-points.js';
...
function optionImage(value){const url=clean(value,2048);if(!url)return '';if(!/^https:\/\//i.test(url)&&!/^\/(?!\/)/.test(url))throw new Error('INVALID_OPTION_IMAGE_URL');return url;}
...
export function editParticipationPost(domain,source,itemId,input={},remove=false,now=new Date().toISOString()){
  ...
  if(domain==='polls'){if(Array.isArray(input.optionImages)){if(input.optionImages.length!==item.options.length)throw new Error('OPTION_IMAGE_COUNT_MISMATCH');item.options=item.options.map((row,index)=>({...row,imageUrl:optionImage(input.optionImages[index])}));}item.question=title;item.description=clean(input.body,2000);if(Array.isArray(input.optionLabels)&&input.optionLabels.length===item.options.length)item.options=item.options.map((row,index)=>({...row,label:clean(input.optionLabels[index],80)||row.label}));}
...
export function createPointService({command,clock=()=>Date.now()}={}){
 ...
 const rewards=async id=>{const [p,r,w]=await Promise.all([read(ACTIVITY_KEYS.policy),read(ACTIVITY_KEYS.record(id)),read(walletKey(id))]);return activityStatus(p,r,w,clock());};
 const reasonOf=input=>{const reason=clean(input.reason,300);if(!reason)throw new Error('REASON_REQUIRED');return reason;};
 const requestId=x=>{const id=String(x||'');if(!/^[a-zA-Z0-9-]{12,80}$/.test(id))throw new Error('INVALID_REQUEST_ID');return id;};
 return {
 async member(user,id){admin(user);const users=await read(TARGET_KEYS.users);if(!users[id])throw new Error('USER_NOT_FOUND');const [wallet,orders]=await Promise.all([read(walletKey(id)),read(POINT_ORDERS)]);return {ok:true,userId:id,wallet,activityRewards:await rewards(id),activityEvents:Object.values((await read(ACTIVITY_KEYS.record(id))).events||{}).sort((a,b)=>b.at-a.at),orders:(orders.items||[]).filter(o=>o.userId===id).sort((a,b)=>b.createdAt-a.createdAt)};},
 async grant(user,input){admin(user);const uid=clean(input.userId,24),amount=Number(input.amount),token=requestId(input.requestId),reason=clean(input.reason,200);if(!POINT_PACKAGES.includes(amount)||!reason)throw new Error('INVALID_GRANT');if(input.confirmed!==true)throw new Error('TRANSFER_CONFIRM_REQUIRED');const id=randomUUID();return mutateParticipation(command,[TARGET_KEYS.users,POINT_ORDERS,walletKey(uid)],([users,orders,wallet])=>{const target=users[uid];if(!target||target.status==='suspended')throw new Error('USER_NOT_FOUND');orders.items||=[];const old=orders.items.find(o=>o.userId===uid&&o.requestId===token);if(old)return {ok:true,order:old};const bonus=pointBonus(amount,!wallet.firstPaid),points=amount*(100+bonus)/100,at=clock(),order={id,requestId:token,userId:uid,nickname:clean(target.nickname||uid,60),amount,points,bonus,status:'approved',source:'admin',reason,createdAt:at,reviewedAt:at,reviewedBy:user.id};wallet.balance=Number(wallet.balance||0)+points;wallet.firstPaid=true;wallet.ledger||=[];wallet.ledger.push({id,type:'charge',points,amount,bonus,at,source:'admin',reason,by:user.id});orders.items.push(order);return {ok:true,order};});},
 async wallet(user){requireUser(user);return {ok:true,wallet:await read(walletKey(user.id)),activityRewards:await rewards(user.id)};},
 async status(user){const bank=await read(POINT_BANK);if(!user?.id)return {ok:true,bank,packages:POINT_PACKAGES,activityRewards:activityStatus(await read(ACTIVITY_KEYS.policy),{},{},clock()),serverNow:clock()};requireUser(user);const [wallet,orders]=await Promise.all([read(walletKey(user.id)),read(POINT_ORDERS)]);const activityRewards=await rewards(user.id),audit=user.role==='admin'?await read(ACTIVITY_KEYS.audit):null;return {ok:true,bank,wallet,activityRewards,...(audit?{activityAdmin:{policy:activityRewards.policy,suspects:audit.suspects||[],audit:audit.items||[]}}:{}),packages:POINT_PACKAGES,orders:(orders.items||[]).filter(o=>user.role==='admin'||o.userId===user.id).sort((a,b)=>b.createdAt-a.createdAt),serverNow:clock()};},
 async activityPolicy(user,input){admin(user);const policy=validateActivityPolicy(input.policy),reason=reasonOf(input);return mutateParticipation(command,[ACTIVITY_KEYS.policy,ACTIVITY_KEYS.audit],([doc,audit])=>{doc.policy={...activityPolicy(doc),...policy};audit.items||=[];audit.items.unshift({action:'activity-policy',by:user.id,reason,at:clock(),policy:doc.policy});return {ok:true,policy:doc.policy};});},
 async activityRevoke(user,input){admin(user);const uid=clean(input.userId,100),eventId=clean(input.eventId,500),reason=reasonOf(input),token=requestId(input.requestId);if(!uid||!eventId)throw new Error('INVALID_ACTIVITY_REVOKE');return mutateParticipation(command,activityKeys(uid),([policy,record,wallet,audit])=>{audit.requests||={};if(audit.requests[token]){const old=audit.requests[token];if(old.action!=='activity-revoke'||old.userId!==uid||old.eventId!==eventId)throw new Error('INVALID_REQUEST_ID');return old.result;}if(!record.events?.[eventId])throw new Error('ACTIVITY_EVENT_NOT_FOUND');const result={ok:true,...revokeActivity(record,wallet,eventId,{at:clock(),reason,by:user.id,requestId:token})};audit.items||=[];audit.items.unshift({action:'activity-revoke',by:user.id,userId:uid,eventId,reason,at:clock(),recovered:result.recovered,debtAdded:result.debtAdded});audit.requests[token]={action:'activity-revoke',userId:uid,eventId,result};return result;});},
 async activityRestrict(user,input){admin(user);const uid=clean(input.userId,100),hours=Number(input.hours),reason=reasonOf(input),token=requestId(input.requestId);if(!uid||!Number.isFinite(hours)||hours<0||hours>720)throw new Error('INVALID_ACTIVITY_RESTRICTION');return mutateParticipation(command,[ACTIVITY_KEYS.record(uid),ACTIVITY_KEYS.audit],([record,audit])=>{audit.requests||={};if(audit.requests[token]){const old=audit.requests[token];if(old.action!=='activity-restrict'||old.userId!==uid||old.hours!==hours)throw new Error('INVALID_REQUEST_ID');return old.result;}record.restrictedUntil=hours?clock()+hours*3600000:0;record.restrictionReason=hours?reason:'';const result={ok:true,userId:uid,restrictedUntil:record.restrictedUntil,restrictionReason:record.restrictionReason};audit.items||=[];audit.items.unshift({action:'activity-restrict',by:user.id,userId:uid,hours,reason,at:clock()});audit.requests[token]={action:'activity-restrict',userId:uid,hours,result};return result;});},
