import {collectionScope} from './collection-profile.js';
...
export async function fetchGoogleNews(person,options={}){
  ...
  const regions=options.collectionProfile?.mode==='specified'?options.collectionProfile.newsRegions:[],searchTerm=regions.length?`"${name}" (${regions.map(term=>`"${term}"`).join(' OR ')})`:name;
  const fetchImpl=options.fetchImpl||fetch,nowValue=Number((options.now||Date.now)()),windows=['1d','7d','30d'],today=koreanDay(nowValue),start=Date.parse(`${today}T00:00:00+09:00`)-30*DAY,ranges=Array.from({length:5},(_,index)=>({from:start+index*7*DAY,to:Math.min(start+(index+1)*7*DAY,Date.parse(`${today}T00:00:00+09:00`)+DAY)})).filter(row=>row.from<row.to),queries=ranges.map(row=>`${searchTerm} after:${koreanDay(row.from-DAY)} before:${koreanDay(row.to)}`),merged=[],coverage=[];
  ...
  async function collectQuery(queryIndex){
    ...
    try{
      ...
      const currentRange=ranges[queryIndex];if(rows.length>=Math.min(50,options.limit||50)&&currentRange.to-currentRange.from>DAY){for(let from=currentRange.from;from<currentRange.to;from+=DAY){ranges.push({from,to:from+DAY});queries.push(`${searchTerm} after:${koreanDay(from-DAY)} before:${koreanDay(from+DAY)}`);}}
  ...
  for(const item of merged){
    ...
    const identity=normalized(`${item.title} ${item.description}`);if(!identity.includes(nameKey))continue;if(regions.length&&!regions.some(term=>identity.includes(normalized(term)))){excludedCount+=1;continue;}
  ...
  return {collectionScope:collectionScope(person,options.collectionProfile,options.exclusions),provider:'GOOGLE_NEWS_RSS',personId:String(person?.id||''),query:queries[0],queries,coverage:[...resolvedCoverage.values()],queryBasis:'Google News RSS · 한국 날짜 기준 30일 전~오늘, 기간 분할 수집',collectedAt:new Date(nowValue).toISOString(),periodCounts:{h24:count(1),d7:count(7),d30:count(30)},excludedCount,items,source:{url:`https://news.google.com/search?q=${encodeURIComponent(`${searchTerm} when:30d`)}&hl=ko&gl=KR&ceid=KR:ko`,label:'Google News RSS'}};
}
...
export function mergeNewsHistory(previous={},news={},referenceAt){
  ...
  const scope=news.collectionScope||previous.scope||'';
  const prior=new Map((news.collectionScope&&news.collectionScope!==previous.scope&&!(!previous.scope&&news.collectionScope.startsWith('name:'))?[]:previous.daily||[]).map(row=>[row.date,row]));
  ...
  return {version:1,scope,daily:Array.from({length:31},(_,index)=>{const date=koreanDay(start+index*DAY),old=prior.get(date)||{},scan=coverage.get(date),all=[...new Set([...(old.keys||[]),...(incoming.get(date)||[])])].sort(),keys=all.slice(0,64),collected=old.collected===true||scan?.collected===true,overflow=old.overflow===true||all.length>64,truncated=overflow||(scan?.collected===true?scan.truncated===true:old.truncated===true);return {date,keys,collected,truncated,...(overflow?{overflow:true}:{}),count:keys.length?keys.length:collected&&!truncated?0:null};})};
}
