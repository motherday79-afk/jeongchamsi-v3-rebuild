...
export function parseGoogleNewsRss(xml,limit=20){
  ...
  return blocks.slice(0,Math.min(50,Math.max(1,Number(limit)||20))).map(block=>({
    title:tag(block,'title'),url:tag(block,'link'),publishedAt:tag(block,'pubDate'),source:tag(block,'source')||'Google 뉴스'
...
export async function fetchGoogleNews(person,options={}){
  ...
  try{
    ...
    const items=parseGoogleNewsRss(await response.text(),options.limit||20);
