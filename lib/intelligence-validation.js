...
export function validateIntelligenceDraft(draft){
  ...
  const narrativeDiagnoses=(draft?.diagnoses||[]).map(({display,...diagnosis})=>diagnosis);
  if(/데이터 부족|분석 준비 중|분석 불가|판단 불가|비교 불가|알 수 없음|추가 데이터 필요|N\/A|TODO|TBD|추후 제공/.test(JSON.stringify({diagnoses:narrativeDiagnoses,prescriptions:draft?.prescriptions})))errors.push('PROHIBITED_INTELLIGENCE_COPY');
