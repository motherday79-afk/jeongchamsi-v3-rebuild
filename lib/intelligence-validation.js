...
import { validateIntelligenceConsistency } from './intelligence-consistency.js';
...
export function validateIntelligenceDraft(draft){
  ...
  if(cohorts.length>1&&new Set(pairs).size===1)errors.push('COHORT_VECTOR_CLONED');
  ...
  for(const diagnosis of draft?.diagnoses||[])if(!diagnosis?.headline||!diagnosis?.coreEvent||!diagnosis?.politicalMeaning||!diagnosis?.changeReason||!diagnosis?.pastPresentConnection||!diagnosis?.supportingData?.length||!score(diagnosis?.score)||!diagnosis?.visualization?.type||!diagnosis?.interpretation?.length)errors.push(`DIAGNOSIS_INVALID:${diagnosis?.id||''}`);
  for(const prescription of draft?.prescriptions||[])if(!prescription?.strategicJudgment||!prescription?.diagnosisBasis?.length||!prescription?.visualization?.type||!prescription?.actions?.length||!prescription?.linkedDiagnosisIds?.every(id=>diagnosisIds.has(id)))errors.push(`PRESCRIPTION_INVALID:${prescription?.id||''}`);
  const activeDiagnoses=(draft?.diagnoses||[]).map(diagnosis=>({headline:diagnosis?.headline,nowSignal:diagnosis?.display?.nowSignal})),activePrescriptions=(draft?.prescriptions||[]).map(row=>({strategicJudgment:row?.strategicJudgment,actions:row?.actions,messageDirection:row?.messageDirection}));
  if(/분석 불가|판단 불가|알 수 없음|N\/A|TODO|TBD|추후 제공/.test(JSON.stringify({diagnoses:activeDiagnoses,prescriptions:activePrescriptions})))errors.push('PROHIBITED_INTELLIGENCE_COPY');
  if(!draft?.diagnosisSummary||!draft?.prescriptionPriorities)errors.push('INTELLIGENCE_SUMMARY_MISSING');
  if(String(draft?.algorithmVersion||'').includes('V3')){
    const requiredV3=['eventClusters','politicalAssetMatrix','pastPresentConnections','politicianType'];
    for(const key of requiredV3)if(!draft?.[key]||(Array.isArray(draft[key])&&!draft[key].length&&!(key==='eventClusters'&&draft.raw?.news&&Array.isArray(draft.raw.news.items)&&draft.raw.news.items.length===0&&draft.raw.news.coverage?.length&&draft.raw.news.coverage.every(x=>x.collected===true&&!x.truncated))))errors.push(`${key.replace(/([A-Z])/g,'_$1').toUpperCase()}_MISSING`);
    const consistency=validateIntelligenceConsistency(draft);if(!consistency.ok)errors.push(...consistency.errors);
  }
