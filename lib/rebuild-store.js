...
export async function authenticateUser(command,id,password){const user=await getUser(command,id);if(!user||user.status==='suspended'||!verifyPasswordHash(password,user.passwordHash))return null;return publicUser(user);}
