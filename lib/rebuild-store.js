...
export async function registerUser(command,input={}){
  const id=text(input.id,24), password=String(input.password||''),name=text(input.name,40),nickname=text(input.nickname,40),email=text(input.email,120),birthYear=text(input.birthYear,4),regionProvince=text(input.regionProvince,40),regionCity=text(input.regionCity||input.regionDistrict,40),phone=normalizeRegistrationPhone(input.phoneDigits||input.phone),currentYear=new Date().getFullYear();
  ...
  const user={id,name,nickname,email,phone,birthYear,regionProvince,regionCity,regionDistrict:'',region:`${regionProvince} ${regionCity}`,preferredParty:text(input.preferredParty,80),role:'member',status:'active',passwordHash:hashPassword(password),sessionVersion:0,mustChangePassword:false,createdAt:now(),updatedAt:now()};
  users[id]=user;await writeUsers(command,users);return {ok:true,user:publicUser(user)};
}
