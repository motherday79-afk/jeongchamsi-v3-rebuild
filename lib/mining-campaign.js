...
export function migrateCampaign(s,c,now){
 if(s.campaignId!==c.id){
  if(s.campaignId||c.id!==INITIAL_CAMPAIGN){
   const paidPicks=Array.isArray(s.paidPicks)?s.paidPicks:[],tool=paidPicks.includes(s.tool)?s.tool:'basic';
   const keep={cycle:s.cycle+1,character:s.character,chosen:s.chosen,paidPicks,tool,prizeHistory:s.prizeHistory||[],receipts:s.receipts||[]};
