...
export async function humanPollRequest(req,{service,user=null,url}={}){
 try{
  ...
  let body=req.body;if(Buffer.byteLength(typeof body==='string'?body:JSON.stringify(body||{}))>1024)return failure(413,'HUMAN_POLL_INPUT_INVALID');
  if(typeof body==='string'){try{body=JSON.parse(body);}catch{return failure(400,'HUMAN_POLL_INPUT_INVALID');}}
  if(body?.operation!=='collect')return failure(400,'HUMAN_POLL_INPUT_INVALID');
  return {status:200,data:await service.collect(user)};
 }catch(error){return failure(error?.message==='HUMAN_POLL_BUSY'?409:503,error?.message==='HUMAN_POLL_BUSY'?'HUMAN_POLL_BUSY':'HUMAN_POLL_UNAVAILABLE');}
}
