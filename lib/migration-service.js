...
const PROFILE_KEYS=new Set(['id','slot','type','roleLabel','groupLabel','jurisdictionLabel','connected','name','party','region','jurisdiction','terms','committee','termStart','termEnd','office','electionLabel','source','isVacant']);
...
export async function writePoliticianSeed(command,seed=POLITICIAN_SEED){
  ...
  for(const type of POLITICIAN_TYPES)await command(['SET',TARGET_KEYS.politicians(type),JSON.stringify({version:seed.version,type,items:seed.profiles[type]})]);
