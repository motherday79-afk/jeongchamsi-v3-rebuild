...
const TOPICS_BY_TYPE=Object.freeze({legal:['06','08'],election:['05','08'],policy:['01','09'],organization:['04','08'],crisis:['01','06'],local:['03','09']});
...
function topicsFor(row,types){
  const routed=unique(types.flatMap(type=>TOPICS_BY_TYPE[type]||[]));
  if(routed.length)return routed.slice(0,2);
  const refs=unique(row?.diagnosisRefs||[]).filter(id=>id==='07');
  return refs.length?refs:['07'];
}
...
export function extractPoliticalFacts(person,input={}){
  ...
  for(const [index,row] of cohorts.entries())facts.push(fact(`cohort-${person?.id||name}-${index}`,'cohort',`${clean(row?.age)||`${index+1}번째 세대`} 성별 반응`,`${clean(row?.age)||'해당 세대'}의 JCS 관심지수는 남성 ${Number(row?.male)||0}, 여성 ${Number(row?.female)||0}이다.`,{topicIds:['02','04'],basis:'derived',sourceTitle:'공식 연령×성별 자료·검색 보조 신호'}));
