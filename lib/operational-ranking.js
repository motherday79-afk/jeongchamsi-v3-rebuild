...
function searchVolume(searchAds){
  ...
  return {total:searchAds.keywordMode==='specified'?finite(searchAds.confirmedVolume?.total):pc+mobile,status:'DIRECT'};
}
...
function compareRows(a,b,weights){
  return b.score-a.score||(weights.news?b.newsScore-a.newsScore:0)||(weights.search?b.searchScore-a.searchScore:0)||(weights.news?b.metrics.sourceCount-a.metrics.sourceCount:0)||(weights.search?b.metrics.searchTotal-a.metrics.searchTotal:0)||a.id.localeCompare(b.id);
}
...
export function buildOperationalRankings(drafts,profileMap,snapshot,generatedAt=Date.now(),rankingWeights=DEFAULT_RANKING_WEIGHTS){
  const {news,search}=validateRankingWeights(rankingWeights),weights={search,news,newsArticles:news/3,newsSources:news/3,newsRecency:news/3};
  ...
  const scored=extracted.map((row,index)=>{
    ...
    const score=round1(searchScore*(weights.search/100)+articleScore*(weights.newsArticles/100)+sourceScore*(weights.newsSources/100)+recencyScore*(weights.newsRecency/100));
  ...
  }).sort((a,b)=>compareRows(a,b,weights));
  ...
  return {snapshot:text(snapshot),generatedAt:new Date(Number(generatedAt)).toISOString(),algorithm:'JCS_NOW_OPERATING_V1',weights,population:ranked.length,overall:ranked.slice(0,100),categories,byId};
}
...
export function rankingChanges(current,previous){
 if(previous&&!sameRankingWeights(current.weights,previous.weights))return {items:[],previousAt:previous.generatedAt||'',ready:false};
