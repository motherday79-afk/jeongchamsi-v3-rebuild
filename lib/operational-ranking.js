...
function extract(draft,profile={}){
  const search=searchVolume(draft?.raw?.searchAds),news=newsSignals(draft?.raw?.news);
  return {
    id:text(draft?.id),
    name:text(profile?.name||draft?.raw?.officialProfile?.name||draft?.id),
    type:TYPES.includes(profile?.type)?profile.type:(TYPES.includes(draft?.raw?.officialProfile?.type)?draft.raw.officialProfile.type:'assembly'),
    party:text(profile?.party||draft?.raw?.officialProfile?.party),
    jurisdiction:text(profile?.jurisdiction||draft?.raw?.officialProfile?.jurisdiction),
