...
// Keep every collected title in the 14-day comparison window; representative links remain bounded separately.
function compactKeywordCorpus(items,referenceAt){
 const at=Date.parse(referenceAt||''),rows=[];if(!Number.isFinite(at))return rows;
 const seen=new Set();for(const item of items){const stamp=Date.parse(item?.publishedAt||item?.date||''),age=at-stamp,title=boundedText(item?.title,280),source=boundedText(item?.source,100),key=title+'|'+source;if(!title||seen.has(key)||!Number.isFinite(age)||age<0||age>14*86400000)continue;seen.add(key);rows.push([title,source,new Date(stamp).toISOString()]);}return rows;
}
