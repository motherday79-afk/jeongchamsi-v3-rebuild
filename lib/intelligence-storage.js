...
function compactHistoricalAggregates(draft){
  ...
  const bars=(Array.isArray(consistency.bars)?consistency.bars:[]).slice(0,10).map((row,index)=>({index:Number.isFinite(Number(row?.index))?Number(row.index):index,label:boundedText(row?.label,10),value:finite(row?.value),count:finite(row?.count),observed:row?.observed===true}));
  const daily=(Array.isArray(persistence.daily)?persistence.daily:[]).slice(-30).map((row,index)=>({index:Number.isFinite(Number(row?.index))?Number(row.index):index,offset:Number.isFinite(Number(row?.offset))?Number(row.offset):index-29,date:boundedText(row?.date,10),count:finite(row?.count)})).filter(row=>row.date);
  ...
  if(daily.length===30)result.persistence={daily,maxDaily:Math.max(0,...daily.map(row=>row.count))};
