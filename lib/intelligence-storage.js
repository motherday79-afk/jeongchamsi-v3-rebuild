...
function compactSourceCounts(items){
  const counts=new Map();
  for(const item of Array.isArray(items)?items:[]){const name=boundedText(item?.source,100);if(name)counts.set(name,(counts.get(name)||0)+1);}
  return [...counts].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name,'ko')).slice(0,50);
}
...
export function compactIntelligenceDraft(draft,options={}){
  ...
  return {
    ...
    input:{searchAds:searchStatus==='DIRECT'?{volume:{pc,mobile,total:pc+mobile}}:null,news:{items:newsItems,...(sourceCounts.length?{sourceCounts}:{}),...(evidenceItems.length?{evidenceItems}: {})},officialContext:compactOfficialContext(source?.officialContext),sourceErrors:compactSourceErrors(source?.sourceErrors)},
