...
function compactYouTube(value){
  ...
  return {provider:'YOUTUBE_DATA_API',collectedAt:boundedText(value?.collectedAt,40),channel:{id:boundedText(value.channel.id,100),title:boundedText(value.channel.title,160),url:boundedText(value.channel.url,1000)},videos,uploads:{h24:finite(value?.uploads?.h24),d7:finite(value?.uploads?.d7),d30:finite(value?.uploads?.d30),daily,scanned:finite(value?.uploads?.scanned),truncated:value?.uploads?.truncated===true}};
}
...
export function compactIntelligenceDraft(draft,options={}){
  ...
  return {
    ...
    input:{collectedAt:text(source?.collectedAt),searchAds:searchStatus==='DIRECT'?{volume:{pc,mobile,total:pc+mobile}}:null,news:{items:newsItems,periodCounts,...(sourceCounts.length?{sourceCounts}:{}),...(evidenceItems.length?{evidenceItems}: {})},...(source?.youtube?.channel?.id?{youtube:compactYouTube(source.youtube)}:{}),officialContext:compactOfficialContext(source?.officialContext),sourceErrors:compactSourceErrors(source?.sourceErrors)},
