...
function compactNews(items){
  return (Array.isArray(items)?items:[]).slice(0,10).map(item=>({
    title:boundedText(item?.title,280),source:boundedText(item?.source,100),url:boundedText(item?.url,1000),publishedAt:boundedText(item?.publishedAt||item?.date,40)
...
export function compactIntelligenceDraft(draft,options={}){
  ...
  const pc=finite(volume.pc),mobile=finite(volume.mobile),newsItems=compactNews(source?.news?.items);
