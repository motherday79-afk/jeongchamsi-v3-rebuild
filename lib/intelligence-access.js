...
function safeYouTube(value,tier){
  ...
  return {provider:'YOUTUBE_DATA_API',collectedAt:text(value.collectedAt),channel:select(value.channel,['id','title','url']),videos:list(value.videos).slice(0,limit).map(row=>select(row,['id','title','url','publishedAt','viewCount'])),uploads:{...select(value.uploads,['h24','d7','d30','scanned','truncated']),daily:list(value?.uploads?.daily).slice(-30).map(row=>select(row,['date','count']))}};
}
