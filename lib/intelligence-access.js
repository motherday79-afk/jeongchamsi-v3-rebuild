export const DIAGNOSTIC_TOPICS=Object.freeze([
  {id:'01',title:'정치인 브랜드 진단'},{id:'02',title:'세대·성별 지지구조 분석'},{id:'03',title:'지역구 민심·메시지 진단'},{id:'04',title:'핵심 지지층 결집도 분석'},{id:'05',title:'경쟁 정치인 비교 분석'},{id:'06',title:'이슈·위기 위험도 진단'},{id:'07',title:'언론·온라인 영향력 분석'},{id:'08',title:'선거·캠페인 경쟁력 진단'},{id:'09',title:'정치 활동·미디어 전환 진단'},{id:'10',title:'JCS 종합해석'}
]);
...
const DISPLAY_KEYS=new Set(['agenda','lifecycle','issueObservations','observationBasis','firstAt','firstSources','unknownTimeCount','delayMinutes','kind','id','mode','nowSignal','indicators','brandClarity','imageConsistency','expansion','axes','bars','average','observed','collected','truncated','percent','scale','trend7','policyConnection','components','direct','action','local','recency','search','pc','mobile','mobileShare','news','newsPeriods','periods','framePeriods','pastRisks','tag','title','url','date','totalSign','risk','opportunity','cohorts','age','male','female','maleShare','femaleShare','maleContribution','femaleContribution','maleRank','femaleRank','total','totalCount','totalShare','composition','key','label','value','unit','basis','interpretation','elections','status','profileOnly','population','populationBasis','populationStatus','issues','count','ledCount','share','evidence','messageFit','messagePath','localJudgment','localShare','messageShare','gap','people','name','party','office','region','overallRank','categoryRank','newsCount','sourceCount','frames','positive','neutral','negative','metrics','competition','direction','velocity','persistence','from','to','durationDays','recurrences','reignitionCount','events','daily','index','offset','maxDaily','shape','source','articleCount','sourceSpread','allSources','newsBasis','newsCollectedAt','majorOutletNames','majorShare','nonMajorShare','frequencyBands','repeat','double','single','topSources','remainingSources','concentration','ownership','led','external','agendaPenetration','agendas','articles','outlets','youtube','policies','stage','stageIndex','isPolicy','evidenceTitle','evidenceUrl','evidenceSource','specificity','beneficiary','budget','deadline','owner','opponent','opponentRate','voteRate','margin','result','year','election','regions','currentSignals','trajectory','regionalStructure','foundations','visual','activityCount','activities','category','frame','agency','initiative','firstMover','joined','conversion','agendaConversion','sourceIds','sourceId','items','detail','totalScore','gender','support','resilience','mediaInfluence','media']);
...
function safeYouTube(value,tier){
  if(!value?.channel?.id)return undefined;
  const limit=tier==='admin'?10:tier==='member'?5:3;
  return {provider:'YOUTUBE_DATA_API',collectedAt:text(value.collectedAt),channel:select(value.channel,['id','title','url']),videos:list(value.videos).slice(0,limit).map(row=>select(row,['id','title','url','publishedAt','viewCount'])),uploads:{...select(value.uploads,['h24','d7','d30','averageViews','scanned','truncated']),daily:list(value?.uploads?.daily).slice(-30).map(row=>select(row,['date','count']))}};
}
...
function safeDiagnosis(item,tier){
  ...
  if(own(row,'display')){row.display=safeDisplay(row.display);if(tier!=='admin'&&row.display){delete row.display.issueObservations;delete row.display.observationBasis;}const youtube=safeYouTube(item?.display?.youtube,tier);if(youtube)row.display.youtube=youtube;else if(row.display)delete row.display.youtube;}
...
function compareProjection(topic,tier){
 const d=topic.display||{},member=tier==='member',out=select(topic,['id','title','headline','score','updatedAt']);
 out.trend=select(topic.trend,['direction']);
 out.display={kind:d.kind};
 if(d.kind==='brand')Object.assign(out.display,{indicators:list(d.indicators).slice(0,member?3:1).map(row=>select(row,['label','value'])),search:select(d.search,['pc','mobile'])});
 if(d.kind==='demographic')out.display.cohorts=list(d.cohorts).map(row=>select(row,['age','total']));
 if(d.kind==='local')Object.assign(out.display,{population:list(d.population).map(row=>select(row,['age','totalShare'])),agenda:list(d.agenda).slice(0,3).map(row=>select(row,['key','label','age','share']))});
 if(d.kind==='competitor')out.display.people=list(d.people).slice(0,1).map(row=>({...select(row,['overallRank','newsCount']),competition:select(row.competition,['index'])}));
 if(d.kind==='media')Object.assign(out.display,{articleCount:d.articleCount,sourceCount:d.sourceCount,topSources:list(d.allSources||d.topSources).slice(0,member?3:1).map(row=>select(row,['name','share']))});
 if(d.kind==='action')Object.assign(out.display,{activityCount:d.activityCount,composition:list(d.composition).slice(0,member?3:1).map(row=>select(row,['label','value']))});
 return out;
}
export function projectIntelligence(report,tier='public',scope='detail'){
  ...
  const diagnoses=list(report.diagnoses).filter(item=>allowed.has(item?.id)).map(item=>{const safe=safeDiagnosis(item,access);return scope==='compare'&&access!=='admin'?compareProjection(safe,access):safe;});
