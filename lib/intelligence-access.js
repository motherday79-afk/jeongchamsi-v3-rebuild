...
function safeDiagnosis(item,tier){
  ...
  if(own(row,'display')){row.display=safeDisplay(row.display);const youtube=safeYouTube(item?.display?.youtube,tier);if(youtube)row.display.youtube=youtube;else if(row.display)delete row.display.youtube;}
