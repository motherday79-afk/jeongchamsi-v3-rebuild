...
function volumeFact(raw){
  if(typeof raw==='number'&&Number.isFinite(raw))return {raw,value:raw>=0?raw:null,range:null};
  const text=clean(raw);
  if(!text)return {raw:raw??null,value:null,range:null};
  if(/^<\s*10$/.test(text))return {raw:text,value:null,range:{min:0,max:9}};
  const numeric=Number(text.replaceAll(',',''));
  if(Number.isFinite(numeric)&&numeric>=0)return {raw,value:numeric,range:null};
...
export async function fetchNaverKeywordVolume(person,options={}){
  ...
  const profile=options.collectionProfile,selected=profile?.mode==='specified'?profile.searchKeywords:null;
  ...
  const query=new URLSearchParams({hintKeywords:selected?selected.join(','):keyword,showDetail:'1'}),url=`https://api.searchad.naver.com${uri}?${query}`;
  const headers={
  ...
  if(selected){
    const keywords=selected.map(term=>{
      const row=rows.find(item=>normalize(item?.relKeyword)===normalize(term)),pc=volumeFact(row?.monthlyPcQcCnt),mobile=volumeFact(row?.monthlyMobileQcCnt);
      return {keyword:term,status:row?'reported':'unavailable',volume:{pc:pc.value,mobile:mobile.value,total:pc.value!==null&&mobile.value!==null?pc.value+mobile.value:null,pcRaw:pc.raw,mobileRaw:mobile.raw,...(pc.range?{pcRange:pc.range}:{}),...(mobile.range?{mobileRange:mobile.range}:{})}};
    });
    const exactSum=field=>keywords.every(row=>row.volume[field]!==null)?keywords.reduce((sum,row)=>sum+row.volume[field],0):null;
    const pc=exactSum('pc'),mobile=exactSum('mobile'),confirmedPc=keywords.reduce((sum,row)=>sum+(row.volume.pc??0),0),confirmedMobile=keywords.reduce((sum,row)=>sum+(row.volume.mobile??0),0);
    return {provider:'NAVER_SEARCH_ADS',personId:String(person?.id||''),keyword:selected.join(' · '),keywordMode:'specified',keywordMatched:true,keywords,collectedAt:new Date(Number(timestamp)).toISOString(),volume:{pc,mobile,total:pc!==null&&mobile!==null?pc+mobile:null},confirmedVolume:{pc:confirmedPc,mobile:confirmedMobile,total:confirmedPc+confirmedMobile,complete:pc!==null&&mobile!==null},source:{method,endpoint:uri,url:'https://api.searchad.naver.com/keywordstool'}};
  }
  const row=rows.find(item=>normalize(item?.relKeyword)===normalize(keyword));
  ...
  return {
    provider:'NAVER_SEARCH_ADS',personId:String(person?.id||''),keyword,keywordMatched:true,collectedAt:new Date(Number(timestamp)).toISOString(),
