...
export function validateCampaignPublication(data){
...
export function prepareCampaignMutation(previous,input={},options={}){
  ...
  if(['save','publish'].includes(operation))next.draft=normalizeCampaignContent(input);
  if(operation==='publish'){validateCampaignPublication(next.draft);next.published=clone(next.draft);next.publishedAt=stamp;next.visibility='public';}
