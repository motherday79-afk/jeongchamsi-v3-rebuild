...
export function projectItem(key,value){if(!key.startsWith('tile.')){const [dx,dy]=hudOffsets[key]||[0,0];return {...value,x:value.x+dx,y:value.y+dy};}const i=Number(key.split('.')[1]),p=remapPoint(i,value);if(key.endsWith('.label')&&[8,16,24].includes(i))p.y+=24;return {...value,...p,w:value.w*ITEM_SCALE,h:value.h*ITEM_SCALE,...('font'in value?{font:value.font*ITEM_SCALE}:{})};}
