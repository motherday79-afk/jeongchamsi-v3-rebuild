import {renderAiPanelPublic,renderAiPanelAdmin} from '../views/ai-panel-pages.js?v=0.0.31.181';
export async function loadAiPanelPage({admin=false,params=new URLSearchParams(),session={},client}={}){
 ...
 const list=await client.list({manage:admin});const id=params.get('id')||(!admin?list.items?.[0]?.id:'');const detail=id?await client.get(id,{edit:admin}):{};
 if(admin){if(params.get('panelId'))list.selectedPanel=await client.panel(params.get('panelId'));return renderAiPanelAdmin({session,data:list,detail,params});}
 ...
 return renderAiPanelPublic({list,detail,params,idHistory});
}
