...
function humanPoll(p){if(!isObject(p)||!validId(p.id)||!isObject(p.results)||typeof p.comparable!=='boolean')fail('HUMAN_INVALID');const out={id:p.id,comparable:p.comparable,institution:str(p.institution,200,true),sourceUrl:url(p.sourceUrl),question:str(p.question,2000,true)};for(const k of ['commissioner','title','method','comparisonNote'])out[k]=str(p[k],2000);for(const k of ['startDate','endDate','publishedDate'])out[k]=p[k]?date(p[k]):null;if(out.startDate&&out.endDate&&out.startDate>out.endDate)fail('HUMAN_INVALID');for(const k of ['sampleSize','responseRate','marginOfError']){const v=p[k];if(v!=null&&(typeof v!=='number'||!Number.isFinite(v)||v<0||(k==='sampleSize'?!Number.isInteger(v):v>100)))fail('HUMAN_INVALID');out[k]=v??null;}out.results={overall:humanBucket(p.results.overall)};for(const k of ['gender','age','region']){const v=p.results[k]||{};if(!isObject(v)||Object.keys(v).length>200)fail('HUMAN_INVALID');out.results[k]=Object.fromEntries(Object.entries(v).map(([name,b])=>[str(name,100,true),humanBucket(b)]));}return out;}
