...
const HUMAN_VALUE_STATUSES=['verified','unconfirmed','not_provided'];
function humanBucket(v){if(!isObject(v))fail('HUMAN_INVALID');const out={};for(const k of ['positive','negative','undecided']){const x=v[k];if(x!=null&&(typeof x!=='number'||!Number.isFinite(x)||x<0||x>100))fail('HUMAN_INVALID');out[k]=x??null;}if(['positive','negative','undecided'].every(k=>out[k]!==null)&&Math.abs(out.positive+out.negative+out.undecided-100)>1)fail('HUMAN_INVALID');for(const k of ['n','weightedN']){const x=v[k];if(x!=null&&(!Number.isInteger(x)||x<0))fail('HUMAN_INVALID');out[k]=x??null;}const status=isObject(v.status)?v.status:{};out.status={};for(const k of ['positive','negative','undecided','n','weightedN']){const x=status[k];if(x!=null&&!HUMAN_VALUE_STATUSES.includes(x))fail('HUMAN_INVALID');out.status[k]=x||((out[k]!==null&&out[k]!==undefined)?'verified':'unconfirmed');}return out;}
