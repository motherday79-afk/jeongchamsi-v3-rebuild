import {POLIMARBLE_BOARD as BOARD,POLIMARBLE_CARDS as CARDS} from '../core/polimable-data.js?v=0.0.31.208';

const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const number=v=>Number(v||0).toLocaleString('ko-KR');

/*
 * JCS POLIMARBLE 31.208 UI contract
 * --------------------------------
 * - Scenic background only: no baked-in board cells, player, dice, score, cards or ranking.
 * - Exactly 24 visible DOM tiles = exactly 24 engine positions.
 * - Current position is shown only by one moving Minime token.
 * - No active-tile glow, pulse, flashing border, locator badge or duplicated UI.
 * - Dice is a real DOM object and has a dedicated rolling animation before Minime movement.
 */
...
const PLAYER_ASSET='/assets/polimable/player-male-31-208.png';
const MASCOT_ASSET='/assets/polimable/mascot-31-208.png';
...
function delta(tile){
  if(tile.kind==='gain'||tile.kind==='bonus')return `+${number(tile.value)}`;
  ...
  if(tile.kind==='safe')return 'SAFE';
  if(tile.kind==='strategy_card')return 'CARD';
  if(tile.kind==='public_card')return 'EVENT';
  if(tile.kind==='choice')return 'CHOICE';
  return '';
}
function tileClass(tile,i){
  const palette=['mint','blue','yellow','pink'];
  if(tile.kind==='loss'||tile.kind==='percent_loss')return 'loss';
  if(tile.kind==='bonus')return 'bonus';
  if(tile.kind==='strategy_card'||tile.kind==='choice')return 'special';
...
function board(state){
  return `<div class="pm-board-track" aria-label="JCS 폴리마블 24칸 게임보드">${BOARD.map((tile,i)=>{
    const [r,c]=VISUAL_POSITIONS[i];
    const player=state?.position===i;
    const visibleNo=String(i+1).padStart(2,'0');
    return `<div class="pm-tile tone-${tileClass(tile,i)}" style="--r:${r};--c:${c}" data-pm-cell="${i}" aria-label="${visibleNo} ${esc(tile.title)} ${esc(delta(tile))}">
      <span class="pm-tile-no">${visibleNo}</span>
      <span class="pm-tile-icon" aria-hidden="true">${tile.icon}</span>
      <b>${tile.kind==='start'?'START':esc(tile.title)}</b>
      <em>${tile.kind==='start'?'출발':esc(delta(tile))}</em>
      ${player?`<span class="pm-player-token${state?._moving?' is-moving':''}" aria-label="현재 위치"><img src="${PLAYER_ASSET}" alt="폴리마블 미니미"></span>`:''}
    </div>`;
  }).join('')}</div>`;
}
...
function dicePanel(state,authenticated,busy,moving){
  ...
  return `<section class="pm-dice-panel${rolling?' is-rolling':''}">
    <header><span>🎲</span><div><b>주사위 굴리기</b><small>${rolling?'주사위가 굴러가고 있어요!':'지금, 한 걸음의 용기가 더 나은 정참시를 만듭니다.'}</small></div></header>
...
function cards(state,busy){
  ...
  return `<section class="pm-cards-panel">
    <header><div><span>⚡</span><b>전략카드 <small>(보유 ${held.length}/3)</small></b></div><em>똑똑한 선택이 더 큰 기회를 만듭니다!</em></header>
...
function ranking(leaderboard,scope){
  ...
  return `<section class="pm-ranking-panel">
    ...
    <ol>${entries.slice(0,5).map(e=>`<li class="${e.isMe?'me':''}"><i>${e.rank===1?'👑':e.rank}</i><span class="pm-rank-face"><img src="${PLAYER_ASSET}" alt=""></span><strong>${esc(e.initials||'JCS')}</strong><b>${number(e.score)}</b>${e.rank===1?'<em>TODAY KING</em>':e.isMe?'<em>나의 순위!</em>':''}</li>`).join('')||'<li class="pm-rank-empty">첫 기록을 기다리고 있어요!</li>'}</ol>
...
function status(state,message,busy,moving){
  ...
  const line=state?`${current.icon} ${current.title}`:'🎲 출발 준비';
  return `<div class="pm-status-bubble" aria-live="polite"><span>${current.icon}</span><div><b>${esc(line)}</b><p>${state?._diceRolling?'주사위를 굴리는 중...':busy&&!moving?'처리 중...':esc(message)}</p></div></div>`;
}
    ...
    <div class="pm-game-root" data-pm-root data-authenticated="${authenticated?'true':'false'}">
      <div class="pm-stage">
        <section class="pm-board-zone">
          ...
          ${board(state)}
          ${status(state,message,busy,moving)}
        </section>
        <aside class="pm-console">
          <div class="pm-console-head"><div class="pm-console-mascot"><img src="${MASCOT_ASSET}" alt=""><span>주사위를 굴려<br>정참시의 오늘을 만들어봐요! 💗</span></div><div class="pm-console-icons" aria-hidden="true"><span>🔊</span><span>?</span><span>⚙</span></div></div>
