...
export async function renderPoliticianCompare(service,route='/compare',session=null){
  ...
  const slotMarkup=slots.map((record,index)=>record?(record.result?.ok&&record.result.item?selectedSlot(record.result.item,index,ids,searchState):failedSlot(record.id,index,ids,run)):selectedSlot(null,index,ids,searchState)).join('');
  ...
  return `<main class="subpage politician-compare-page compare-capacity-${limit}" data-compare-role="${role}" data-compare-limit="${limit}" data-compare-executed="${run}"><section class="page-hero politician-compare-hero"><span class="eyebrow">${eyebrow}</span><h1>${title}</h1><p>${description}</p><div class="politician-compare-capacity"><strong>${ids.length}</strong><span>/ ${limit}명 선택</span><b>${isAdmin?'최대 5명':'1:1 전용'}</b></div></section><section class="content-card politician-compare-selection"><div class="section-title"><div><span class="eyebrow">SELECTED PROFILES</span><h2>비교 대상</h2></div><span>검색하여 선택 · ${isAdmin?'2~5명 비교':'1:1 비교'}</span></div><div class="politician-compare-slots">${slotMarkup}</div>${runButton}</section>${analysis}</main>`;
}
