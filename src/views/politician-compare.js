...
function compareCompetitor(data){
  ...
  return `<div data-jcs-period-scope data-jcs-competitor-period-scope><div class="jcs-periods" aria-label="경쟁 기사 기간">${periods.map(row=>`<button type="button" data-jcs-period="${row.label}" aria-pressed="${row.label==='30D'}">${row.label}</button>`).join('')}</div><div class="jcs-cmp-stat-grid">${compactStat('전체 NOW',person.overallRank?`${person.overallRank}위`:'—')}${compactStat('분야 NOW',person.categoryRank?`${person.categoryRank}위`:'—')}${compactStat('PC 검색',compactNumber(person.pc))}${compactStat('모바일 검색',compactNumber(person.mobile))}</div>${periods.map(row=>`<div class="jcs-cmp-period-value" data-jcs-period-panel="${row.label}"${row.label==='30D'?'':' hidden'}>${compactStat(`${row.label} 기사`,row.value,'건')}</div>`).join('')}</div><div class="jcs-cmp-frames"><span class="is-positive">긍정 <b>${frames.positive??'—'}</b></span><span class="is-neutral">중립 <b>${frames.neutral??'—'}</b></span><span class="is-negative">부정 <b>${frames.negative??'—'}</b></span></div><div class="jcs-cmp-election"><b>${esc(election.election||'현재 경쟁 신호')}</b><span>${election.voteRate!==null&&election.voteRate!==undefined?`${election.voteRate}%`:`JCS ${person.competition?.index??'—'}`}</span>${election.margin!==null&&election.margin!==undefined?`<em>${election.margin>=0?'+':''}${esc(election.margin)}p</em>`:''}</div>`;
}
