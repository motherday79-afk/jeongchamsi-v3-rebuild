...
function publicCompareCell(topic,position){return `<article class="jcs-compare-topic-cell"><strong>${esc(topic?.headline)}</strong>${compareField('핵심 사건',topic?.coreEvent)}${compareField('정치적 의미',topic?.politicalMeaning,'is-meaning')}${compareField('상대 비교',position.label)}${compareField('최근 흐름',topic?.trend)}${compareField('JCS 상대지수',topic?.score)}</article>`;}

function memberCompareCell(topic,position){return `<article class="jcs-compare-topic-cell">${compareField('현재 평가',topic?.currentPosition,'is-summary')}${compareField('핵심 사건',topic?.coreEvent)}${compareField('정치적 의미',topic?.politicalMeaning,'is-meaning')}${compareField('변화 원인',topic?.changeReason)}${compareField('과거와 현재',topic?.pastPresentConnection)}${compareField('핵심 수치',`${topic?.score} · ${topic?.percentile}`)}${compareField('최근 변화',topic?.trend)}${compareField('현재 우위·열세·경합',position.label)}${compareField('격차 요약',position.value)}${compareField('동급·경쟁 비교',topic?.benchmark)}${compareField('정참시 비교 해석',topic?.interpretation,'is-interpretation')}${compareField('서브데이터',topic?.supportingData)}${compareField('기준일·출처',[topic?.updatedAt,...(topic?.sourceTypes||[])])}</article>`;}

function adminCompareCell(topic,position){return `<article class="jcs-compare-topic-cell">${compareField('핵심 사건',topic?.coreEvent)}${compareField('정치적 의미',topic?.politicalMeaning,'is-meaning')}${compareField('현재 위치',topic?.currentPosition,'is-position')}${compareField('변화 원인',topic?.changeReason)}${compareField('과거와 현재',topic?.pastPresentConnection)}${compareField('점수·상대 위치',`${position.label} · ${topic?.score}`)}${compareField('직군 위치',topic?.percentile)}${compareField('최근 변화',topic?.trend)}${compareField('비교 기준',topic?.benchmark)}${compareField('근거 데이터',topic?.evidence)}${compareField('서브데이터',topic?.supportingData)}${compareField('정참시 해석',topic?.interpretation,'is-interpretation')}${compareField('활용 가능한 기회',topic?.opportunity)}${compareField('관리해야 할 위험',topic?.risk)}</article>`;}
...
function renderDiagnosticComparison(entries,role,strategyId=''){
  ...
  return `<section class="jcs-compare-report jcs-compare-report-${role}"><header class="jcs-compare-report-heading"><span>${en}</span><h2>${ko}</h2><p>${role==='admin'?esc(entries[0].intelligence?.stInterpretation):'상세페이지와 동일한 분석값을 같은 항목과 기준으로 직접 비교합니다.'}</p><small class="jcs-representative-news-guide">대표 뉴스는 핵심 이슈 주제에서 벗어난 관련 기사 집계를 의미합니다.</small></header>${role==='admin'?adminComparisonSummary(entries,topicIds):''}<div class="jcs-compare-matrix" style="--compare-count:${entries.length}">${header}${sections}</div>${adminRx}${role==='public'?'<footer><button type="button" class="primary-btn" data-layout-route="/login">로그인하고 상세 비교 보기</button></footer>':''}</section>`;
}
