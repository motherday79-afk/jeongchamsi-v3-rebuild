...
function renderPublicComparison(entries){
  ...
  const sources=entries.map(entry=>`<article><h3>${esc(entry.item.name)}</h3><div>${(entry.intelligence?.sources||[]).map(source=>`<span><b>${esc(source.type)}</b><small>${esc(source.grade||'SOURCE')}</small></span>`).join('')}</div></article>`).join('');
  return `<div class="politician-compare-intelligence public-comparison-intelligence"><section class="content-card">${sectionHead('NOW OPERATING INDEX','NOW 운영지수 비교','네이버 검색광고 40% · Google 뉴스 60%')}<div class="politician-compare-index-grid" style="--compare-count:${entries.length}">${nowCards}</div></section><section class="content-card">${sectionHead('CORE INDICATORS','핵심 지표 비교','동일 운영 스냅샷')}${metricColumns(entries,'core')}</section><section class="content-card">${sectionHead('ACTIVITY & MEDIA','활동·미디어 비교','공식 역할·Google 뉴스') }<div class="politician-compare-activity-grid" style="--compare-count:${entries.length}">${activityMedia}</div></section><section class="content-card">${sectionHead('SOURCE CLASSES','원자료 범주','허용 원자료만 사용')}<div class="politician-compare-source-grid" style="--compare-count:${entries.length}">${sources}</div></section></div>`;
}

function cohortRows(entries){
  const ages=[...new Set(entries.flatMap(entry=>(entry.intelligence?.cohorts||[]).map(row=>row.age)))];
  return ages.map(age=>`<div class="politician-compare-cohort-row"><b>${esc(age)}</b>${entries.map(entry=>{const row=(entry.intelligence?.cohorts||[]).find(item=>item.age===age);return `<span><em style="--heat:${clamp(row?.male)}%">남 ${available(row?.male)?Number(row.male):''}</em><em style="--heat:${clamp(row?.female)}%">여 ${available(row?.female)?Number(row.female):''}</em></span>`;}).join('')}</div>`).join('');
}
...
function renderAdminComparison(entries){
  ...
  const radars=entries.map(entry=>{const data=entry.intelligence?.support||{},values=[data.loyalty,data.action,data.stability,data.scalability].filter(available);return `<article><h3>${esc(entry.item.name)}</h3><svg viewBox="0 0 180 180" role="img" aria-label="${esc(entry.item.name)} 지지 품질 레이더"><circle cx="90" cy="90" r="74"></circle><circle cx="90" cy="90" r="48"></circle>${values.length>=3?`<polygon points="${radarPoints(values)}"></polygon>`:''}</svg><div><span>충성 ${available(data.loyalty)?data.loyalty:''}</span><span>행동 ${available(data.action)?data.action:''}</span><span>안정 ${available(data.stability)?data.stability:''}</span><span>확장 ${available(data.scalability)?data.scalability:''}</span></div></article>`;}).join('');
  ...
  const evidence=entries.map(entry=>`<article><h3>${esc(entry.item.name)}</h3>${(entry.intelligence?.sources||[]).map(source=>`<div><b>${esc(source.type)}</b><span>${esc(source.title||source.grade||'DIRECT')}</span><p>${esc(source.detail||'')}</p></div>`).join('')}</article>`).join('');
