...
import {renderSearchDiscovery} from './search-discovery.js?v=0.0.31.162';
...
export async function renderSearchPage({query='',page=1,personId='',publisher='',period='latest',politicians,content}={}){
 ...
 try{
  ...
  if(data.matches.length||data.target.kind!=='none')return `<main class="subpage search-page jcs-spread" data-search-query="${esc(term)}">${renderMediaSpread(data,params)}${renderSearchDiscovery(term)}</main>`;
  const posts=await matchingPosts(term,content);
  return `<main class="subpage search-page jcs-spread" data-search-query="${esc(term)}">${hero}<p class="spread-query">‘${esc(term)}’ 검색 결과</p>${posts||'<div class="content-card spread-empty"><h2>일치하는 결과가 없습니다</h2><p>정치인 이름이나 정당 이름으로 다시 검색해 주세요.</p></div>'}${renderSearchDiscovery(term)}</main>`;
 }catch{
  const retry=spreadRoute(params);return `<main class="subpage search-page jcs-spread" data-search-query="${esc(term)}">${hero}<div class="content-card spread-empty" role="status"><h2>검색 결과를 불러오지 못했습니다</h2><p>잠시 후 다시 시도해 주세요.</p><a href="${esc(retry)}" data-layout-route="${esc(retry)}">다시 불러오기</a></div>${renderSearchDiscovery(term)}</main>`;
 }
}
