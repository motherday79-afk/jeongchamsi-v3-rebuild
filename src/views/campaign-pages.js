import { campaignUrl, campaignVideoId } from '../core/campaign-model.js';
...
const dateRange=item=>[item?.startDate,item?.endDate].filter(Boolean).map(esc).join(' — ');
const personMeta=item=>[item?.office,item?.party,item?.region].filter(Boolean).map(esc).join(' · ');
const image=item=>{const src=campaignUrl(item?.photoUrl,{local:true});return src?`<img data-politician-photo src="${esc(src)}" alt="${esc(item?.name?`${item.name} 캠페인 사진`:'캠페인 사진')}" loading="lazy">`:'';};
...
function featuredCard(item){
  ...
  return `<a class="jcd-feature${photo?' has-photo':''}" href="${esc(route)}" data-layout-route="${esc(route)}"><div class="jcd-feature-copy"><div class="jcd-feature-edition"><span class="jcd-feature-label">대표 캠페인</span>${numberLabel(item.number)?`<span>JCS ${esc(numberLabel(item.number))}</span>`:''}</div><h2 class="jcd-feature-title">${text(item.headline)}${item.accentLine?`<br><em>${text(item.accentLine)}</em>`:''}</h2>${item.topic?`<p class="jcd-feature-topic">${esc(item.topic)}</p>`:''}${item.name?`<p class="jcd-feature-person"><strong>${esc(item.name)}</strong>${meta?`<span>${meta}</span>`:''}</p>`:''}<span class="jcd-feature-cta">이야기를 만나보세요 ${icon('right')}</span></div>${photo?`<div class="jcd-feature-photo">${photo}${dateRange(item)?`<span class="jcd-feature-period">${dateRange(item)}</span>`:''}</div>`:''}</a>`;
}

function gridCard(item){
  const route=routeFor(item.id),photo=image(item),meta=[item.office,item.region].filter(Boolean).map(esc).join(' · ');
  return `<a class="jcd-item" href="${esc(route)}" data-layout-route="${esc(route)}">${photo?`<div class="jcd-portrait">${photo}${numberLabel(item.number)?`<span class="jcd-number">${esc(numberLabel(item.number))}</span>`:''}${item.topic?`<span class="jcd-topic">${esc(item.topic)}</span>`:''}</div>`:''}<div class="jcd-item-body"><h3 class="jcd-item-title">${titleOf(item)}</h3>${item.name?`<div class="jcd-name-line"><strong>${esc(item.name)}</strong>${item.party?`<span class="jcd-party">${esc(item.party)}</span>`:''}</div>`:''}${meta?`<span class="jcd-role">${meta}</span>`:''}<div class="jcd-item-bottom">${dateRange(item)?`<span>${dateRange(item)}</span>`:''}<span class="jcd-item-link">이야기 만나보기 ${icon('upRight')}</span></div></div></a>`;
}
...
function manageCard(item){
  ...
  return `<article class="jcd-manage-row"><div><span class="jcd-manage-state">${state}</span><h2>${esc(item.draftHeadline||item.headline||'제목 없는 초안')}</h2><p>${item.version!=null?`버전 ${esc(item.version)}`:''}</p></div>${routeLink(route,`수정 ${icon('right')}`,'jcd-admin-link')}</article>`;
}

export function renderCampaignBoard(result,session={},view='current'){
  ...
  if(!result?.ok)return `<main id="jcs-campaign-directory"><div class="jcd-top"><span class="jcd-crumb">정치후원 / <b>CAMPAIGN 전체보기</b></span>${controls}</div><section class="jcd-state" role="alert"><h1>캠페인을 불러오지 못했습니다.</h1><p>${esc(result?.error||'잠시 후 다시 시도해주세요.')}</p></section></main>`;
  ...
  const pageRoute=number=>{const query=new URLSearchParams();if(selected!=='current')query.set('view',selected);query.set('page',String(number));return `/campaigns?${query}`;};
  ...
  const tabs=`<nav class="jcd-tabs" aria-label="캠페인 목록 선택">${routeLink('/campaigns',`진행 중 <span class="jcd-count">${currentCount}</span>`,selected==='current'?'jcd-tab is-active':'jcd-tab')}${routeLink('/campaigns?view=archive',`ARCHIVE <span class="jcd-count">${archiveCount}</span>`,selected==='archive'?'jcd-tab is-active':'jcd-tab')}</nav>`;
  let body='';
  if(selected==='archive'){
  ...
  else body=`${featuredCard(featured)}${items.length?`<div class="jcd-section-head"><h2>함께 살펴볼 캠페인</h2><span>진행 중인 다른 이야기 ${items.length}건</span></div><div class="jcd-grid">${items.map(gridCard).join('')}</div>`:featured?'':'<div class="jcd-state"><p>공개된 캠페인이 아직 없습니다.</p></div>'}${archiveCount?`<div class="jcd-archive-invite"><div><small>CAMPAIGN ARCHIVE</small><h2>캠페인은 끝나도,<br>기록은 남습니다.</h2></div>${routeLink('/campaigns?view=archive',`지난 이야기 ${archiveCount}건 ${icon('right')}`,'jcd-archive-link')}</div>`:''}`;
  return `<main id="jcs-campaign-directory"><div class="jcd-top"><span class="jcd-crumb">정치후원 / <b>CAMPAIGN 전체보기</b></span>${controls}</div><header class="jcd-introduction"><span class="jcd-eyebrow">POLICY · PEOPLE · POSSIBILITY</span><h1 class="jcd-heading">정참시 <em>CAMPAIGN</em></h1><p class="jcd-lead">한 사람의 생각에서, 우리 사회의 다음 가능성으로.<br>정책과 이야기를 살펴보고, 여러분의 기준으로 판단해주세요.</p></header>${selected==='manage'?'':tabs}<div class="jcd-content">${body}${pagination}</div><footer class="jcd-footer"><p class="jcd-principle">정참시는 정당을 보고 사람을 선택하지 않습니다.<br><strong>아직 충분한 기회를 얻지 못한 정치와 정책을 발견합니다.</strong></p></footer></main>`;
}
...
export function renderCampaignDetail(item,session={}){
  ...
  const route=routeFor(item.id),meta=personMeta(item),photo=image(item),admin=isAdmin(session);
  ...
  const story=item.quote||item.storyBody||item.needsBody?`<section class="jcs-section" id="jcs-campaign-story"><div class="jcs-kicker"><span>02</span> POLITICIAN STORY</div>${item.quote?`<blockquote class="jcs-quote">${text(item.quote)}${item.name?`<cite>${esc(item.name)}</cite>`:''}</blockquote>`:''}${item.storyBody?`<p class="jcs-body-copy">${text(item.storyBody)}</p>`:''}${item.needsBody?`<details class="jcs-story-fold"><summary>이 제안에 지금 필요한 것은 무엇인가요?</summary><p>${text(item.needsBody)}</p></details>`:''}</section>`:'';
  const policies=Array.isArray(item.policies)?item.policies.filter(row=>row?.title||row?.body):[];
  const policy=item.policyTitle||policies.length?`<section class="jcs-section" id="jcs-campaign-policy"><div class="jcs-kicker"><span>03</span> POLICY</div>${item.policyTitle?`<h2>${text(item.policyTitle)}</h2>`:''}${policies.length?`<p class="jcs-body-copy">${policies.length===3?'이번 캠페인에서 함께 살펴볼 정책의 세 가지 방향입니다.':'이번 캠페인에서 함께 살펴볼 정책의 방향입니다.'}</p><ol class="jcs-policy-list">${policies.map((row,index)=>`<li><span class="jcs-policy-number" aria-hidden="true">${String(index+1).padStart(2,'0')}</span><div>${row.title?`<h3>${text(row.title)}</h3>`:''}${row.body?`<p>${text(row.body)}</p>`:''}</div></li>`).join('')}</ol>`:''}<div class="jcs-judgment"><strong>정책을 살펴보고, 판단은 시민이 합니다.</strong>정참시는 소개된 정치인의 모든 견해에 동의를 요구하지 않습니다. 이번 캠페인의 정책과 활동을 살펴보고, 여러분의 기준으로 판단해주세요.</div></section>`:'';
  ...
  const support=item.support||{},official=campaignUrl(support.officialUrl),supportSource=campaignUrl(support.sourceUrl),showSupport=item.state!=='archive'&&support.public===true&&support.verified===true&&official&&supportSource&&support.associationName;
  const supportBlock=showSupport?`<section class="jcs-support" id="jcs-campaign-support" aria-label="공식 후원회 안내"><div class="jcs-kicker">SUPPORT THE POSSIBILITY</div><h2>이 제안에 공감하셨나요?</h2><p class="jcs-support-intro">정책과 활동을 살펴본 뒤,<br>후원 여부는 직접 결정해주세요.</p><div class="jcs-official-info"><strong>${esc(support.associationName)}</strong><a href="${esc(supportSource)}" target="_blank" rel="noopener noreferrer">공식 정보 출처</a></div><a class="jcs-support-action" href="${esc(official)}" target="_blank" rel="noopener noreferrer">공식 후원회에서 후원하기 ${icon('upRight')}</a><p class="jcs-direct-note">후원은 정치인의 공식 후원회를 통해 진행합니다.</p></section>`:'';
  return `<main id="jcs-campaign-preview"><div class="jcs-topline"><div class="jcs-crumb">${routeLink('/campaigns',`${icon('left')} 캠페인 전체보기`)}</div>${admin?routeLink(`${route}/edit`,'캠페인 수정','jcd-admin-link'):''}</div>${item.state==='archive'?'<p class="jcd-detail-ended">종료된 캠페인의 기록입니다. 당시의 정책과 이야기를 보존합니다.</p>':''}<section class="jcs-hero" aria-label="캠페인 소개"><div class="jcs-hero-copy">${numberLabel(item.number)?`<div class="jcs-edition"><span class="jcs-edition-line" aria-hidden="true"></span>JCS CAMPAIGN ${esc(numberLabel(item.number))}</div>`:''}<h1>${text(item.headline)}${item.accentLine?`<br><em>${text(item.accentLine)}</em>`:''}</h1>${item.intro?`<p class="jcs-intro">${text(item.intro)}</p>`:''}${item.name?`<div class="jcs-person"><strong>${esc(item.name)}</strong>${meta?`<span>${meta}</span>`:''}</div>`:''}${why?`<a class="jcs-read" href="#jcs-campaign-why">정책과 이야기 읽기 ${icon('down')}</a>`:''}</div>${photo?`<figure class="jcs-photo">${photo}</figure>`:''}</section>${item.topic||dateRange(item)?`<div class="jcs-meta">${item.topic?`<span class="jcs-meta-topic">${esc(item.topic)}</span>`:''}${dateRange(item)?`<span>${dateRange(item)}</span>`:''}</div>`:''}<div class="jcs-article">${why}${story}${policy}${videoEmbed(item.videoUrl)}${sourceBlock}${supportBlock}<p class="jcs-principle">정참시는 정당을 보고 사람을 선택하지 않습니다.<br><b>아직 충분한 기회를 얻지 못한 정치와 정책을 발견합니다.</b></p></div><div class="jcs-bottomline"><span>정참시 CAMPAIGN</span></div></main>`;
}
