...
export function footer(info={},memberCount=0){const rows=FOOTER_FIELDS.filter(([key])=>String(info?.[key]||'').trim()).map(([key,label])=>`<span class="footer-business-item"><b>${label}</b>${footerValue(key,info[key])}</span>`).join(''),disclaimer=String(info?.disclaimer||'').trim();return `<footer class="footer footer-expanded"><div class="footer-brand"><b>${esc(info?.siteName||'정참시')}</b><span>정치에 참여할 시간</span></div><nav class="footer-links"><a href="#/inquiry" data-layout-route="/inquiry">1:1 문의하기</a><a href="#/privacy" data-layout-route="/privacy">개인정보처리방침</a><a href="#/policy" data-layout-route="/policy">운영정책</a></nav>${rows?`<div class="footer-business-grid">${rows}</div>`:''}${disclaimer?`<p class="footer-disclaimer">${esc(disclaimer)}</p>`:''}<div class="footer-member-count"><b>${Number(memberCount).toLocaleString('ko-KR')}</b>명이 정참시와 함께합니다</div></footer>`;}
