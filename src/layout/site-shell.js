import { SERVICE_CATALOG, serviceIconSvg, serviceNavIconSvg, brandMarkSvg } from '../ui/service-icons.js?v=0.0.31.144';
...
export function siteHeader(memberCount=0,session={authenticated:false,user:null}){
  ...
  return `<header class="site-header product-header"><div class="product-head-main"><a class="header-brand-home" href="/" data-layout-route="/" aria-label="정참시 홈">${brandMarkSvg('header-brand-icon')}<span class="header-brand-slogan"><em>정</em>치에 <em>참</em>여할 <em>시</em>간</span></a><div class="product-account-tools">${fontSizeControl()}${account}</div></div><div class="product-head-search"><form class="product-search product-search-expanded" data-layout-search><a class="search-home" href="/" data-layout-route="/" aria-label="정참시 홈">${brandMarkSvg('search-brand-icon')}</a><input name="q" type="search" enterkeyhint="search" aria-label="통합 검색" placeholder="정치인, 정당, 이슈를 검색하세요" autocomplete="off" data-politician-autocomplete data-politician-select-mode="route"></form></div></header>`;
}
...
export function footer(info={},memberCount=0){const rows=FOOTER_FIELDS.filter(([key])=>String(info?.[key]||'').trim()).map(([key,label])=>`<span class="footer-business-item"><b>${label}</b>${footerValue(key,info[key])}</span>`).join(''),disclaimer=String(info?.disclaimer||'').trim();return `<footer class="footer footer-expanded"><div class="footer-brand"><b class="footer-brand-wordmark">${brandMarkSvg('footer-brand-icon')}${esc(info?.siteName||'정참시')}</b><span>정치에 참여할 시간</span></div><nav class="footer-links"><a href="#/inquiry" data-layout-route="/inquiry">1:1 문의하기</a><a href="#/privacy" data-layout-route="/privacy">개인정보처리방침</a><a href="#/policy" data-layout-route="/policy">운영정책</a></nav>${rows?`<div class="footer-business-grid">${rows}</div>`:''}${disclaimer?`<p class="footer-disclaimer">${esc(disclaimer)}</p>`:''}<div class="footer-member-count">정참시는 <b>${Number(memberCount).toLocaleString('ko-KR')}</b>명의 정참시민과 함께합니다.</div></footer>`;}
