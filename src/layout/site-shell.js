import { SERVICE_CATALOG, serviceIconSvg } from '../ui/service-icons.js?v=0.0.31.39';
...
export function siteHeader(memberCount=0,session={authenticated:false,user:null}){
  ...
  return `<header class="site-header product-header"><div class="product-head-main"><button class="product-menu" type="button" aria-label="전체 메뉴 열기" aria-expanded="false" data-drawer-open><svg viewBox="0 0 24 24"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button><a class="product-wordmark" href="#/" data-layout-route="/"><b>정참시</b><span>JEONGCHAMSI</span></a><form class="product-search product-search-expanded" data-layout-search><input name="q" aria-label="통합 검색" placeholder="정치인, 정당, 이슈를 검색하세요" autocomplete="off" data-politician-autocomplete data-politician-select-mode="route"><button type="submit" aria-label="검색"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="6.5"/><path d="m16 16 4 4"/></svg></button></form><div class="product-account-tools">${account}</div></div><div class="product-service-bar live-community-bar"><div class="live-community-inner"><div class="live-community-count"><b>${Number(memberCount).toLocaleString('ko-KR')}</b><span>명이 정참시와 함께합니다</span></div><div class="live-community-actions">${fontSizeControl()}</div></div></div></header>`;
}

export function fontSizeControl(){
  return `<div class="jcs-font-scale-control" data-font-scale-control data-font-scale="0" role="group" aria-label="페이지 글자 크기"><span>글자 크기</span><button type="button" data-font-scale-decrease aria-label="글자 크기 줄이기" disabled>−</button><button type="button" data-font-scale-increase aria-label="글자 크기 키우기">＋</button></div>`;
}
...
export function drawer(session={authenticated:false,user:null}){
  ...
  return `<div class="drawer-backdrop" data-drawer-close hidden></div><aside class="app-drawer" data-drawer hidden aria-hidden="true" aria-label="정참시 전체 메뉴"><div class="drawer-head"><div><span>JEONGCHAMSI</span><b>정참시 전체메뉴</b></div><button type="button" aria-label="전체 메뉴 닫기" data-drawer-close>×</button></div>${account}<section class="drawer-block drawer-service-block"><div class="drawer-block-head"><b>전체 서비스</b><span>정참시 서비스 메뉴</span></div><div class="drawer-service-grid">${services}</div></section>${personal}<section class="drawer-block drawer-utility"><div class="drawer-utility-links">${adminLink}<a href="#/about" data-layout-route="/about">정참시 소개</a><a href="#/inquiry" data-layout-route="/inquiry">1:1 문의하기</a><a href="#/privacy" data-layout-route="/privacy">개인정보처리방침</a><a href="#/policy" data-layout-route="/policy">운영정책</a><a href="#/request-politician" data-layout-route="/request-politician">정치인 등록 요청</a><a href="#/partners" data-layout-route="/partners">파트너스 신청</a></div></section></aside>`;
}
const FOOTER_FIELDS=Object.freeze([
  ['businessName','운영·사업자명'],['businessRegistrationNumber','사업자등록번호'],['mailOrderRegistrationNumber','통신판매업 신고번호'],['representative','대표자'],['phone','대표전화'],['fax','팩스'],['address','주소'],['privacyOfficer','개인정보 보호책임자'],['privacyEmail','개인정보 문의'],['adEmail','광고 문의'],['customerEmail','고객센터'],['partnerEmail','파트너 문의'],['managementEmail','경영지원 문의']
]);
const footerValue=(key,value)=>{const safe=esc(value);return key.endsWith('Email')?`<a href="mailto:${safe}">${safe}</a>`:safe;};
export function footer(info={}){const groups=[['businessName','representative'],['businessRegistrationNumber','mailOrderRegistrationNumber'],['phone','fax','address'],['privacyOfficer','privacyEmail'],['adEmail','customerEmail'],['partnerEmail','managementEmail']],rows=groups.map(keys=>{const items=FOOTER_FIELDS.filter(([key])=>keys.includes(key)&&String(info?.[key]||'').trim()).map(([key,label])=>`<span class="footer-business-item"><b>${label}</b>${footerValue(key,info[key])}</span>`).join('');return items?`<div class="footer-business-row">${items}</div>`:'';}).join(''),disclaimer=String(info?.disclaimer||'').trim();return `<footer class="footer footer-expanded"><div class="footer-brand"><b>${esc(info?.siteName||'정참시')}</b><span>정치에 참여할 시간</span></div><nav class="footer-links"><a href="#/inquiry" data-layout-route="/inquiry">1:1 문의하기</a><a href="#/privacy" data-layout-route="/privacy">개인정보처리방침</a><a href="#/policy" data-layout-route="/policy">운영정책</a></nav>${rows?`<div class="footer-business-grid">${rows}</div>`:''}${disclaimer?`<p class="footer-disclaimer">${esc(disclaimer)}</p>`:''}</footer>`;}
