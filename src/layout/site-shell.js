import { SERVICE_CATALOG, serviceIconSvg, serviceNavIconSvg, brandMarkSvg } from '../ui/service-icons.js?v=0.0.31.194';
...
export function siteHeader(memberCount=0,session={authenticated:false,user:null}){
  ...
  const account=authenticated
    ? `<button class="product-user-chip" type="button" data-layout-route="/mypage" aria-label="${nickname}님 마이페이지"><span>마이페이지</span></button>`
    : `<button class="product-login-chip" type="button" data-layout-route="/login">로그인</button>`;
  return `<header class="site-header product-header"><div class="product-head-main"><a class="header-brand-home" href="/" data-layout-route="/" aria-label="정참시 홈"><span class="header-brand-slogan">정치에 참여할 시간</span></a><div class="product-account-tools">${fontSizeControl()}${account}</div></div><div class="product-head-search"><form class="product-search product-search-expanded" data-layout-search><button type="button" class="search-home" data-page-reload aria-label="현재 페이지 새로고침" title="새로고침">${brandMarkSvg('search-brand-icon')}</button><input name="q" type="search" enterkeyhint="search" aria-label="통합 검색" placeholder="정치인, 정당, 이슈를 검색하세요" autocomplete="off" data-politician-autocomplete data-politician-select-mode="route"></form></div></header>`;
}
...
export function drawer(session={authenticated:false,user:null}){
  const services=SERVICE_CATALOG.map(item=>`<a class="drawer-service-item drawer-service-${item.key} drawer-tone-${item.tone}" href="#${item.href}" data-layout-route="${item.href}"><span class="drawer-service-icon">${serviceNavIconSvg(item.key)}</span><span class="drawer-service-copy"><b>${item.label}</b><small>${item.description}</small></span><em>→</em></a>`).join('');
  const authenticated=session?.authenticated===true;
  const nickname=esc(session?.user?.nickname||'회원');
...
const FOOTER_FIELDS=Object.freeze([
  ['businessName','운영·사업자명'],['representative','대표자'],['businessRegistrationNumber','사업자등록번호'],['mailOrderRegistrationNumber','통신판매업 신고번호'],['phone','대표전화'],['fax','팩스'],['address','주소'],['privacyOfficer','개인정보 보호책임자'],['privacyEmail','개인정보 문의'],['adEmail','광고 문의'],['customerEmail','고객센터'],['partnerEmail','파트너 문의'],['managementEmail','경영지원 문의']
]);
// Contact addresses are selectable text. They must not launch another app.
const footerValue=(key,value)=>{const safe=esc(value);return key.endsWith('Email')?`<span class="footer-email" data-allow-copy>${safe}</span>`:safe;};
export function footer(info={},memberCount=0){const rows=FOOTER_FIELDS.filter(([key])=>String(info?.[key]||'').trim()).map(([key,label])=>`<span class="footer-business-item"><b>${label}</b>${footerValue(key,info[key])}</span>`).join(''),disclaimer=String(info?.disclaimer||'').trim();return `<footer class="footer footer-expanded"><div class="footer-brand"><b class="footer-brand-wordmark">${brandMarkSvg('footer-brand-icon')}<span class="footer-brand-name">${esc(info?.siteName||'정참시')}</span></b><span>정치에 참여할 시간</span></div><nav class="footer-links"><a href="#/inquiry" data-layout-route="/inquiry">1:1 문의하기</a><a href="#/privacy" data-layout-route="/privacy">개인정보처리방침</a><a href="#/policy" data-layout-route="/policy">운영정책</a></nav>${rows?`<div class="footer-business-grid">${rows}</div>`:''}${disclaimer?`<p class="footer-disclaimer">${esc(disclaimer)}</p>`:''}<div class="footer-member-count">정참시는 <b>${Number(memberCount).toLocaleString('ko-KR')}</b>명의 정참시민과 함께합니다.</div></footer>`;}
