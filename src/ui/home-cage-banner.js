...
export function renderCageBanner({title,titleLayout=null,blue=0,red=0}={}){
 ...
 const titleArt=`<g clip-path="url(#${id}-title)"><g class="cage-promo-title" filter="url(#${id}-brush)" mask="url(#${id}-grain-mask)" fill="url(#${id}-ink)" stroke="#fff1c9" stroke-width="2" stroke-linejoin="round" paint-order="stroke fill" font-family="JCS Cage Rock,JCS Cage Brush,cursive" font-weight="400" text-anchor="start" style="font-kerning:none;font-variant-ligatures:none" transform="translate(897.5 0) skewX(-${CAGE_BRUSH_SKEW})">${geometry.lines.map((line,i)=>`<text transform="scale(${line.widthScale.toFixed(5)} 1)" data-cage-title-line="${i+1}" x="${line.x.toFixed(2)}" y="${line.baseline.toFixed(2)}" font-size="${line.size.toFixed(2)}" letter-spacing="${(CAGE_BRUSH_TRACKING*line.size).toFixed(2)}">${esc(line.text)}</text>`).join('')}</g></g>`;
 return `<svg class="cage-promo-art" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1774 887" aria-hidden="true" focusable="false">
 <defs><pattern id="${id}-grain" width="94" height="81" patternUnits="userSpaceOnUse"><rect width="94" height="81" fill="white"/><path d="M15 4l-2 28 1-2 3-25z M54 18l-4 49 2-6 4-41z M78 2l-1 16 2-5z M27 49l-3 26 2-8 2-15z" fill="black" opacity=".62"/></pattern><mask id="${id}-grain-mask" maskUnits="userSpaceOnUse" x="-1000" y="0" width="2000" height="450"><rect x="-1000" y="0" width="2000" height="450" fill="url(#${id}-grain)"/></mask><clipPath id="${id}-title"><rect x="390" y="84" width="1015" height="282"/></clipPath><linearGradient id="${id}-ink" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fffdf6"/><stop offset=".55" stop-color="#fff7dd"/><stop offset=".8" stop-color="#f4d28a"/><stop offset="1" stop-color="#c88b36"/></linearGradient><filter id="${id}-brush" x="-5%" y="-10%" width="110%" height="125%"><feTurbulence type="fractalNoise" baseFrequency=".055 .22" numOctaves="2" seed="8" result="grain"/><feDisplacementMap in="SourceGraphic" in2="grain" scale="3.6" xChannelSelector="R" yChannelSelector="G" result="ink"/><feDropShadow in="ink" dx="1.5" dy="2.5" stdDeviation="1" flood-color="#352009" flood-opacity=".95"/></filter></defs>
