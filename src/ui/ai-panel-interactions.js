...
export async function simpleOperatorUpload({client,panelId,responses,onProgress=()=>{}}={}){
 ...
 if(profiles){
  ...
  const population={...selectedPanel.population,notes:[selectedPanel.population?.notes,'통합 AI 파일 업로드로 프로필 1,000개 자동 갱신'].filter(Boolean).join(' · ')};onProgress('통합파일의 프로필 1,000명을 새 패널로 저장하고 있습니다…');
 ...
 if(!run.environments?.EXPOSED){onProgress('AI 정보환경을 자동 저장하고 있습니다…');const sources=polls.map(p=>({title:`${p.institution} 최신 대통령 직무평가`,url:p.sourceUrl,containsPollNumbers:true,publishedAt:null}));result=await client.save({operation:'environment',id:run.id,version,input:{mode:'EXPOSED',notes:'관리자 간편 업로드 · 최신 HUMAN 조사 확인 후 ChatGPT에서 생성한 통합 AI 파일',sources}});if(!result?.ok)throw Error(result?.error||'AI_PANEL_NETWORK_FAILED');run=result.item;version=Number(run.version)||version+1;}
 onProgress('AI 응답 1,000개를 저장하고 자동 집계하고 있습니다…');result=await client.save({operation:'responses',id:run.id,version,input:{mode:'EXPOSED',model:'JCS AI ID INTELLIGENT AGENT · N1000',executedAt:new Date().toISOString(),responses}});if(!result?.ok)throw Error(result?.error||'AI_PANEL_NETWORK_FAILED');return {...result,integratedProfiles:!!profiles};
}
export function bindAiPanelInteractions(root,{client,onSaved=()=>{},navigate=()=>{}}={}){
 ...
 root.addEventListener('submit',async event=>{
  const manual=event.target.closest('[data-human-manual]');if(manual){event.preventDefault();const s=state(manual);if(s.busy)return;const text=String(new FormData(manual).get('text')||'').trim(),button=manual.querySelector('button');if(!text){notice(manual,'붙여넣을 조사 내용을 입력해 주세요.');return;}s.busy=true;if(button)button.disabled=true;notice(manual,'JCS HUMAN POLL 데이터를 검증하고 저장하고 있습니다…');try{const result=await client.importHumanPoll(text);if(!result?.ok){notice(manual,result?.error==='HUMAN_POLL_INPUT_INVALID'?'JCS HUMAN POLL JSON의 기관·발표일·조사기간·표본수·긍정·부정·유보·출처 URL을 확인해 주세요.':(messages[result?.error]||`등록하지 못했습니다. (${result?.error||'연결 오류'})`));return;}notice(manual,`JCS HUMAN POLL ${Number(result?.importedCount)||1}건을 등록했습니다. AI 통합파일 업로드 시 최신 HUMAN 자료에 자동 연결됩니다.`);await onSaved(result,'human-manual');}catch{notice(manual,'조사 저장 결과를 확인하지 못했습니다. 잠시 후 다시 시도해 주세요.');}finally{s.busy=false;if(button)button.disabled=false;}return;}
  const deleting=event.target.closest('[data-ai-delete]');if(deleting){event.preventDefault();const s=state(deleting);if(s.busy)return;if(typeof globalThis.confirm==='function'&&!globalThis.confirm('이 과거조사 기록을 완전히 삭제할까요? 삭제 후에는 복구할 수 없습니다.'))return;const button=deleting.querySelector('button');s.busy=true;if(button)button.disabled=true;notice(deleting,'과거조사 기록을 삭제하고 있습니다…');try{const result=await client.save({operation:'delete',id:deleting.dataset.id||'',version:Number(deleting.dataset.version)||0,input:{}});if(!result?.ok){notice(deleting,messages[result?.error]||`삭제하지 못했습니다. (${result?.error||'연결 오류'})`);return;}notice(deleting,'삭제했습니다.');await onSaved(result,'delete');}catch{notice(deleting,'삭제 결과를 확인하지 못했습니다. 새로고침 후 확인해 주세요.');}finally{s.busy=false;if(button)button.disabled=false;}return;}
 ...
 root.addEventListener('change',async event=>{
  ...
  if(simple){
   ...
   s.busy=true;simple.disabled=true;notice(form,`AI 통합파일을 확인하고 있습니다… (${file.name})`);
   try{
    if(file.size>maxBytes)throw Error('FILE');const text=await file.text();const responses=parse(text,true);
    const result=await simpleOperatorUpload({client,panelId:form.dataset.panelId,responses,onProgress:text=>notice(form,text)});
 ...
 root.addEventListener('click',event=>{
  const reasonMore=event.target.closest('[data-ai-reason-more]');if(reasonMore){event.preventDefault();const category=reasonMore.closest('[data-ai-reason-category]');if(!category)return;const hidden=[...category.querySelectorAll('[data-ai-reason-item][hidden]')];hidden.slice(0,20).forEach(item=>item.hidden=false);const left=Math.max(0,hidden.length-20);if(!left)reasonMore.remove();else reasonMore.innerHTML=`더보기 +20 <span>(${left}명 남음)</span>`;return;}
