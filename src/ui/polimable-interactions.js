import {projectItem} from '../core/polimable-board-geometry.js?v=0.0.31.273';
...
import {mountHudEditor} from './polimable-hud-editor.js?v=0.0.31.273';
...
function bindResponsiveStage(root){
  const page=root.closest('.pm-board-stage-page');if(!page)return;
  const controller=new AbortController(),signal=controller.signal;
  const sync=()=>{
    if(!root.isConnected||root.dataset.pmHudEditing==='1')return;
    const vv=window.visualViewport,vw=vv?.width||innerWidth,vh=vv?.height||innerHeight;
    Object.assign(page.style,{left:`${vv?.offsetLeft||0}px`,top:`${vv?.offsetTop||0}px`,width:`${vw}px`,height:`${vh}px`});
    const cs=getComputedStyle(page),fit=calculatePoliMarbleStage(vw,vh,{left:readPx(cs.paddingLeft),right:readPx(cs.paddingRight),top:readPx(cs.paddingTop),bottom:readPx(cs.paddingBottom),gutter:0});
    root.style.width=`${fit.width}px`;root.style.height=`${fit.height}px`;root.dataset.pmFitMode=fit.mode;root.style.setProperty('--pm-stage-scale',String(fit.scale));
  };
  const fullscreen=page.querySelector('[data-pm-fullscreen]');
  if(fullscreen&&page.requestFullscreen&&document.fullscreenEnabled){fullscreen.hidden=false;fullscreen.addEventListener('click',async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await page.requestFullscreen();}catch{fullscreen.textContent='전체화면 불가';}},{signal});}
  document.addEventListener('fullscreenchange',()=>{if(fullscreen)fullscreen.textContent=document.fullscreenElement?'전체화면 해제':'전체화면';sync();},{signal});
  window.addEventListener('resize',sync,{passive:true,signal});window.addEventListener('orientationchange',sync,{passive:true,signal});window.visualViewport?.addEventListener('resize',sync,{passive:true,signal});window.visualViewport?.addEventListener('scroll',sync,{passive:true,signal});
  const observer=new MutationObserver(()=>{if(!root.isConnected){controller.abort();observer.disconnect();}});observer.observe(document.body,{childList:true,subtree:true});
  root._pmViewportSync=sync;sync();
}
