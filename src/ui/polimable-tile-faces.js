...
import {boardTile,projectPlane,triangleMatrix} from '../core/polimable-board-geometry.js?v=0.0.31.272';
...
export function mountTileFaces(canvas){
 ...
 gradient('pm271-base',[[0,'#b8d0dc'],[.12,'#dbe6e7'],[.24,'#7795aa'],[.8,'#536a83'],[1,'#40596c']]);
 ...
 for(const[a,b]of [[outer[3],outer[2]],[outer[2],outer[1]]]){
  svg.append(node('path',{d:path([a,b,[b[0],b[1]+31],[a[0],a[1]+31]]),fill:'url(#pm271-base)',stroke:'#7892a0','stroke-width':1}));
  svg.append(node('path',{d:`M${a[0]},${a[1]+25}L${b[0]},${b[1]+25}`,fill:'none',stroke:'#bdcdd3','stroke-width':1.5,opacity:.8}));
 }
 const baseClip=clip('pm271-backboard',roundPolygon(outer,12)),base=node('g',{'clip-path':baseClip});
 base.append(node('image',{href:'/assets/polimable/design272/plaza.png',width:1,height:1,preserveAspectRatio:'none',transform:`matrix(782 402.73 -782 402.73 836 ${453-782*.515})`}));svg.append(base);
 svg.append(node('path',{d:roundPolygon(outer,12),fill:'none',stroke:'#d2e2e4','stroke-width':2}));
 const inner=[[123,123],[649,123],[649,649],[123,649]].map(p=>projectPlane(...p));svg.append(node('path',{d:roundPolygon(inner,8),fill:'none',stroke:'#d9e1cf','stroke-width':1.5,opacity:.7}));
 for(const t of [...TILES].sort((a,b)=>boardTile(a.index).center[1]-boardTile(b.index).center[1])){
  ...
  const cap=node('g'),topArt=node('g',{'clip-path':topClip});topArt.append(node('path',{d:geo.top,fill:`url(#pmcolor-${i})`}));
  {
   ...
   for(const tri of [[0,1,2],[0,2,3]]){const d=tri.map(n=>dst[n]),g=node('g',{'clip-path':clip(`pm-art-${i}-${tri[1]}`,path(d))});g.append(node('image',{href:silver?'/assets/polimable/editor/strategy-silver-266.png':t.corner?`/assets/polimable/design271/corner-${i}.png`:'/assets/polimable/design272/tile.png',width:1,height:1,preserveAspectRatio:'none',transform:`matrix(${triangleMatrix(tri.map(n=>src[n]),d).join(' ')})`}));topArt.append(g);}
  }
  let finish=null;
