import { renderCagePosts, renderCageArena, renderCageHits, cagePageData } from './views/community-ui.js?v=0.0.31.77';
import { HOME_FIXTURE } from './fixtures/home.js?v=0.0.31.56';
import { siteHeader, drawer, footer, renderInitialLoading } from './layout/site-shell.js?v=0.0.31.77';
import { renderCheerShop, renderCheerProduct, renderTrendingPage, renderKeywordsPage, renderNowRankCard, renderHomeLayout, renderBadgeShowcase } from './layout/home-layout.js?v=0.0.31.77';
import { setupLayoutInteractions } from './ui/interactions.js?v=0.0.31.77';
...
import * as views from './views/stage1.js?v=0.0.31.77';
...
import { renderParticipationAdminSettings, generationVoteConfirmation, renderPollBoard, renderGenerationPresident, renderNationalEvaluationPage } from './views/participation-pages.js?v=0.0.31.77';
...
document.addEventListener('click',async event=>{
 ...
 if(unpin||commentLike){event.preventDefault();const button=unpin||commentLike;if(button.disabled)return;button.disabled=true;try{const result=unpin?await content.update('community',button.dataset.postId,{pinKind:''}):await content.likeComment(button.dataset.domain,button.dataset.postId,button.dataset.commentId);if(result?.status===401){button.disabled=false;navigation.navigate('/login');return;}if(result?.error){let state=button.closest('.jc49').querySelector('[data-jc-error]');if(!state){state=document.createElement('p');state.dataset.jcError='';state.setAttribute('role','alert');button.closest('.jc49').append(state);}state.textContent=formErrors[result.error]||result.error;return;}await render({preserveScroll:true});}catch{button.title='처리하지 못했습니다. 다시 시도해 주세요.';}finally{button.disabled=false;}return;}
  ...
  if(bannerEdit){event.preventDefault();const placement=bannerEdit.dataset.homeBannerEdit==='hero'?'hero':'sidebar',dialog=document.createElement('dialog');dialog.className='home-banner-dialog';dialog.innerHTML=`<form data-home-banner-form><h2>${placement==='hero'?'히어로 하단 가로 배너':'메인 사이드 배너'} 등록</h2><input type="hidden" name="placement" value="${placement}"><p>PC·모바일·폴드 펼침/태블릿 이미지 모두 필수 · JPG·PNG·WEBP·GIF<br>각각 최대 2MB, 전체 합계 최대 3MB</p><p data-banner-storage-status data-state="checking">배너 저장소 연결 확인 중…</p><label>PC 이미지 · ${placement==='hero'?'848 × 150':'640 × 450'}px<input type="file" name="pc" accept="image/jpeg,image/png,image/webp,image/gif" required></label><label>모바일 이미지 · ${placement==='hero'?'720 × 300':'720 × 540'}px<input type="file" name="mobile" accept="image/jpeg,image/png,image/webp,image/gif" required></label><label>폴드 펼침·태블릿 이미지 · ${placement==='hero'?'1200 × 300':'1200 × 400'}px<input type="file" name="tablet" accept="image/jpeg,image/png,image/webp,image/gif" required></label><label>클릭 이동 URL<input type="url" name="targetUrl" placeholder="https://" required></label><label>대체 텍스트<input name="alt" maxlength="120" placeholder="배너 설명" required></label><span data-form-state role="status"></span><div class="home-banner-actions"><button type="button" class="ghost-btn" data-home-banner-close>취소</button><button type="submit" class="primary-btn">저장</button></div></form>`;document.body.append(dialog);dialog.addEventListener('close',()=>dialog.remove(),{once:true});dialog.showModal();const state=dialog.querySelector('[data-banner-storage-status]'),result=await auth.homeBannerStorageStatus?.().catch(()=>null),configured=!!result?.storage?.configured;if(state){state.dataset.state=configured?'ready':'missing';state.textContent=configured?'배너 저장소 연결됨':'배너 저장소 미연결 · Vercel Blob 연결 및 재배포 필요';}return;}
