import { HOME_FIXTURE } from './fixtures/home.js?v=0.0.31.53';
import { siteHeader, drawer, footer, renderInitialLoading } from './layout/site-shell.js?v=0.0.31.53';
import { renderTrendingPage, renderKeywordsPage, renderNowRankCard, renderHomeLayout, renderBadgeShowcase } from './layout/home-layout.js?v=0.0.31.53';
import { setupLayoutInteractions } from './ui/interactions.js?v=0.0.31.53';
import { createAuthService, photoUploadMessage } from './core/auth.js?v=0.0.31.53';
import { createContentService } from './core/content.js?v=0.0.31.53';
import { createPoliticianService } from './core/politicians.js?v=0.0.31.53';
import { createNavigation, adminRouteState, adminRouteWith } from './core/navigation.js?v=0.0.31.53';
import { createIntelligenceAutoResumeGuard, runIntelligenceAction } from './core/intelligence-runner.js?v=0.0.31.53';
import { buildRoleNarratives } from './ui/intelligence-narratives.js?v=0.0.31.53';
import * as views from './views/stage1.js?v=0.0.31.53';
import { renderPoliticianDirectory, renderPoliticianDetail } from './views/politicians.js?v=0.0.31.53';
import { renderPoliticianCompare } from './views/politician-compare.js?v=0.0.31.53';
import { generationVoteConfirmation, renderPollBoard, renderGenerationPresident, renderNationalEvaluationPage } from './views/participation-pages.js?v=0.0.31.53';
import { renderPresidentPage } from './views/president.js?v=0.0.31.53';
import { renderSearchPage } from './views/search-page.js?v=0.0.31.53';
import { loadRecentPoliticians, recordRecentPolitician } from './ui/recent-politicians.js?v=0.0.31.53';
import { regionDistrictOptions, regionSubdistrictOptions } from './data/korean-regions.js?v=0.0.31.53';
...
document.addEventListener('submit',async event=>{
  ...
  if(bannerForm){
    event.preventDefault();const data=new FormData(bannerForm),pc=data.get('pc'),mobile=data.get('mobile'),state=bannerForm.querySelector('[data-form-state]'),submit=bannerForm.querySelector('[type="submit"]');
    if([pc,mobile].some(file=>!(file instanceof File)||!file.size)){state.textContent='PC와 모바일 이미지를 모두 선택해 주세요.';return;}
    if([pc,mobile].some(file=>file.size>2097152||!['image/jpeg','image/png','image/webp'].includes(file.type))){state.textContent='JPG·PNG·WEBP, 이미지별 최대 2MB입니다.';return;}
    if(pc.size+mobile.size>3145728){state.textContent='PC·모바일 이미지의 합계 용량은 3MB 이하로 선택해 주세요.';return;}
    ...
    try{const encode=async file=>{const bytes=new Uint8Array(await file.arrayBuffer());let binary='';for(let start=0;start<bytes.length;start+=32768)binary+=String.fromCharCode(...bytes.subarray(start,start+32768));return {contentType:file.type,dataBase64:btoa(binary)};};
      const result=await content.saveHomeBanner({placement:String(data.get('placement')||'sidebar'),pc:await encode(pc),mobile:await encode(mobile),targetUrl:String(data.get('targetUrl')||''),alt:String(data.get('alt')||'정참시 배너')});
      const messages={BANNER_STORAGE_NOT_CONFIGURED:'배너 저장소가 연결되지 않았습니다.',BANNER_BOTH_REQUIRED:'PC와 모바일 이미지를 모두 선택해 주세요.',BANNER_PAIR_TOO_LARGE:'두 이미지의 합계 용량은 3MB 이하입니다.',BANNER_TOO_LARGE:'이미지별 최대 2MB입니다.',BANNER_TYPE_INVALID:'JPG·PNG·WEBP 이미지만 업로드할 수 있습니다.',BANNER_SIGNATURE_INVALID:'손상되었거나 지원하지 않는 이미지입니다.',BANNER_TARGET_URL_INVALID:'클릭 이동 주소는 https://로 시작해야 합니다.'};
