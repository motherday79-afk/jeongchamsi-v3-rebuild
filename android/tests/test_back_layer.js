...
(async () => {
  const [{default: fs}, {default: vm}, {default: assert}, {default: path}] = await Promise.all([
    import('node:fs'),
    import('node:vm'),
    import('node:assert/strict'),
    import('node:path')
  ]);
  const script = fs.readFileSync(path.join(path.dirname(process.argv[1]), '../assets/back-layer.js'), 'utf8');
  let count = 0;
